...
import { enableEmergencyPad, isEmergencyPadActive } from "./vault-crypto.js?v=6bb1e228";
...
import {
  ...
  fetchKeyslotBlob,
  lookupKeyslotTxid,
} from "./client-auth.js?v=5d3f1f4f";  // sec-2026-07: 絶対パス化で cache-bust(?v=)対象にし、SW キャッシュに旧署名が残る罠を解消（app-main と同一モジュール実体に統一）
...
export async function refreshTierQualifier() {
  ...
  if (_session.rmat) {
    try {
      const { deriveAppNameTagFromHkdf } = await import("./vault-crypto.js?v=6bb1e228");
...
export async function createVault(password, userDisplayName, captchaToken = null, opts = {}) {
  ...
  const { generateRecoverySecret } = await import("./vault-crypto.js?v=6bb1e228");
...
export async function currentSigningPrivateKeyRaw() {
  ...
  const { deriveSigningKeyFromHkdf } = await import("./vault-crypto.js?v=6bb1e228");
