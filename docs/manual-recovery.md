...
暗号は Rust WASM が無くても **pure-JS (noble) にフォールバック**して動くため、
Arpass のサーバも Rust のビルドも不要です。
...
- ツールが公開 Arweave からあなたの envelope を直接取得し、ブラウザ内で復号（サーバ不要・Rust 不要）。
...
2. envelope を取得して **ブラウザ内で復号**します（Arpass サーバ不要・Rust ビルド不要。
   Rust WASM が無ければ noble にフォールバック）。
