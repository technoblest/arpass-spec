...
### 個人 Vault を CLI で復旧する（ブラウザ不要）

個人（Master + Recovery）Vault は、ブラウザを使わずネイティブ CLI でも復旧できます
（`tools/recover-personal.py`）。ブラウザの配信コードに依存せず、自分で確認したコードで
復旧したい場合に推奨です。
...
# マスターパスワードとリカバリーシークレット (RS1-...) を入力
```

公開 Arweave から自分の Vault を直接取得・復号し、パスワード表示（既定でマスク）・`vault.json` 出力・
添付ファイル保存まで行います（Arpass サーバ非経由）。YubiKey 専用 Vault は `recover-yubikey.py`、
法人 Vault は K1 ファイルが必要なのでブラウザツールを使ってください。
...
## セキュリティ上の注意（復旧ツールを使う前に）

- **正規の入手元だけを使う（フィッシング対策）**: マスターパスワード / リカバリーを入力してよいのは、
  ブラウザのアドレスバーが **`arpass.io`** または **`technoblest.github.io/arpass-spec`** の時だけです。
  類似ドメインや他人の fork には**絶対に入力しない**でください。復旧ツールはあなたの最重要秘密を扱うため、
  改ざん版を使うとすべて盗まれます。
- **最高機密は「あなたがコードを支配する」経路で**: ホスティングされた配信版（Web）は、その配信コードを
  支配する者があなたの秘密を支配できます（arpass.io / GitHub Pages のいずれも）。したがって最も機密性の高い
  復旧は、**ネイティブ CLI（`tools/recover-yubikey.py`）** か、**リポジトリを clone してローカル配信**で行い、
  自分でコードを確認・管理することを推奨します。
- **平文の取り扱い**: 復号結果（画面表示・`vault.json` / `vault_N.json`・エクスポート CSV/JSON・保存ファイル）は
  **パスワードやファイルの平文**です。クラウド同期/バックアップ対象のフォルダに置かず、使用後は削除してください。
- **設計上の補足（pure-JS）**: ブラウザ版は意図的に pure-JS（WebCrypto + noble、WASM バイナリ非依存、
  ソースだけで検証可能）です。本体アプリのように鍵を WASM 内に隠す設計ではないため、配信コードが改ざんされた場合の
  耐性は本体より低くなります。ただし復旧ツールは平文を出力するのが目的なので、上記「コードを支配する経路」の方が
  本質的な守りです。
