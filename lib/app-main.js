console.log("[arpass] build: provision intent:// button 2026-06-29b");
...
import {
  ...
  currentVault,
  currentLatestTxId,
  currentRestoredFromHistory,
  retryLatestForRestoredSession,
  currentCredIdHash,
  ...
  hasAndroidProvision,
  fireAndroidProvision,
  iapVerifyApple,
  iapVerifyGoogle,
} from "/lib/vault-client.js?v=c6d50826";
import { enableEmergencyPad } from "/lib/vault-client.js?v=c6d50826";
import {
  generatePassword,
  passwordStrength,
  isPasskeySupported,
...
} from "/lib/client-auth.js?v=7220a552";
import {
  initI18n,
  mountLangPicker,
  t as i18n_t,
...
} from "/lib/i18n.js?v=a97271a3";
// Phase 7.5L: app.html でも SW 登録 + 自動更新検知 (= cache クリア不要化)
import { registerServiceWorker } from "/lib/pwa-install.js?v=310ff450";
import { displayLabel } from "/lib/profiles.js?v=69cb395d";
...
// 2026-07-12: initI18n は辞書 fetch が失敗しても resolve する (dict 空のまま) ため、
//   ロード成否を t() の key-echo で検査し、失敗時は 8 秒間隔でリトライする。
//   不安定な回線で「辞書が空のまま永遠に英語フォールバック」になるのを防ぐ。
// 2026-07-12: profile picker は JS がラベルを文字列として焼き込む方式 (data-i18n では
//   ない) ため、辞書ロード前に描画されるとフォールバック表示のまま残る。辞書ロード
//   完了・言語切替のタイミングで picker が表示中なら再描画する。
async function _rerenderPickerIfVisible(reason) {
  try {
    const v = document.getElementById("view-profile-picker");
    if (!v || v.classList.contains("hidden")) return;
    const profMod = await import("/lib/profiles.js?v=69cb395d");
    console.log("[i18n] " + reason + " — re-rendering profile picker labels");
    _renderProfilePicker(profMod.listProfiles(), !!_inviteCode);
  } catch (e) {
    console.warn("[i18n] picker re-render failed:", e?.message);
  }
}

function _initI18nWithRetry(attempts = 6) {
  initI18n()
    .then(() => {
      mountLangPicker("#header-lang", { compact: false });
      const probe = "app.toast.vault_created";
      if (i18n_t(probe) === probe && attempts > 0) {
        console.warn("[i18n] dict empty after init — retrying in 8s (" + attempts + " left)");
        setTimeout(() => _initI18nWithRetry(attempts - 1), 8000);
      } else {
        console.log("[i18n] dictionary ready (lang=" + (i18n_getLang?.() || "?") + ")");
        _rerenderPickerIfVisible("dictionary loaded");
        try { _applyIOSShellDeviceText(); } catch (_) {}
      }
    })
    .catch(err => {
      console.warn("[i18n] init failed:", err);
      if (attempts > 0) setTimeout(() => _initI18nWithRetry(attempts - 1), 8000);
    });
}
_initI18nWithRetry();
onLangChange(() => { _rerenderPickerIfVisible("language changed"); try { _applyIOSShellDeviceText(); } catch (_) {} });
...
async function removeAuthorizedDevice(id) {
  ...
  const { removeCredentialFromEnvelopeUI } = await import("/lib/vault-client.js?v=c6d50826");
...
async function fetchVaultStatus() {
  ...
  try {
    ...
    return {
      ...
      // Postpaid: 未払 (ご利用額) と与信上限、書込可否 (docs/postpaid-billing-design.md §2.3)
      owedUsdMicro: typeof j.owedUsdMicro === "number" ? j.owedUsdMicro : null,
      owedJpyApprox: typeof j.owedJpyApprox === "number" ? j.owedJpyApprox : null,
      stopLimitJpy: typeof j.stopLimitJpy === "number" ? j.stopLimitJpy : null,
      registrationPaid: !!j.registrationPaid,
      canWrite: j.canWrite !== false,
      writeBlockedReason: j.writeBlockedReason ?? null,
      trialUsageUsdMicro: typeof j.trialUsageUsdMicro === "number" ? j.trialUsageUsdMicro : null,
      trialWritesRemaining: typeof j.trialWritesRemaining === "number" ? j.trialWritesRemaining : null,
      tier: j.tier ?? null,
      corpBilling: !!j.corpBilling,   // true = 社員 (請求は admin — 支払い UI を出さない)
      corpSubscriptionActive: !!j.corpSubscriptionActive,
      corpMonthUsageUsdMicro: typeof j.corpMonthUsageUsdMicro === "number" ? j.corpMonthUsageUsdMicro : null,
      corpIncludedJpy: typeof j.corpIncludedJpy === "number" ? j.corpIncludedJpy : null,
...
const state = {
  ...
  owedUsdMicro: null,      // Postpaid: 未払 (= ご利用額) USD × 1e6
  trialUsageUsdMicro: null,// Postpaid: お試し中の参考額 (請求されない、登録で owed=0 スタート)
  owedJpyApprox: null,     // Postpaid: 未払の JPY 概算 (サーバ計算)
  stopLimitJpy: null,      // Postpaid: 書込停止となる未払上限 (JPY)
  registrationPaid: false, // Postpaid: 登録料支払い済みか
  canWrite: true,          // Postpaid: サーバ判定の書込可否
  writeBlockedReason: null,// Postpaid: 不可理由 (registration_required | insufficient_credits | frozen)
...
function showView(name) {
  ...
  // Phase ZZ-LOCK: グローバルロックボタン — vault view のみ visible
  try { document.getElementById("lock-btn-global")?.classList.toggle("hidden", name !== "vault"); } catch {}
  if (name === "vault") {
    ...
    // Phase 7.5ZZ: Android TWA の「📲 自動入力に登録」ボタン。直前の解錠で provision URL が
    //   用意できているとき(= TWA)だけ表示し、タップ(=user gesture)で credId+uh を native へ渡す。
    //   custom scheme(arpass-provision://)起動には activation が必須なので、確実なタップで発火する。
    try {
      const _abtn = document.getElementById("android-autofill-register");
      if (_abtn) {
        const _ahint = document.getElementById("android-autofill-hint");
        if (hasAndroidProvision()) {
          _abtn.classList.remove("hidden");
          _ahint?.classList.remove("hidden");
          _abtn.onclick = () => {
            const ok = fireAndroidProvision();
            try { toast(ok ? (i18n_t("app.toast.autofill_registered") || "この端末の自動入力に登録しました") : "登録に失敗しました", ok ? "ok" : "err"); } catch {}
          };
        } else {
          _abtn.classList.add("hidden");
          document.getElementById("android-autofill-hint")?.classList.add("hidden");
        }
      }
    } catch {}
  }
...
async function applyVaultModeUI() {
  ...
  // Phase 7.x #427: admin 解錠時に K1 自動配布を fire (= 30s throttle 共有)
  //   旧: admin が「管理」 tab を開いた瞬間にだけ走る (= renderAdminConsole の setTimeout)
  //   新: admin が vault を解錠した時点で fire — social 通知 (Slack/メール等) で
  //       社員 signup を知った admin は arpass を解錠するだけで K1 配布完了。
  //   #426 (k1Pending write block) との combo で、 社員側の待ち時間最小化。
  if (inAdmin) {
    // #428: badge を初期表示 (非同期、 unlock 直後の他処理と並行)
    _updateAdminTabBadge().catch(() => {});
    setTimeout(async () => {
      try {
        if (Date.now() - _lastAutoK1DistributeAt < 30_000) return;
        const status = currentAdminK1Status();
        if (!status?.hasK1) return;  // admin K1 未生成は対象外 (admin が手動で生成すべき)
        ...
        console.log(`[admin-unlock-auto-k1] ${pending.length} 名の K1 未配布社員を自動配布 (unlock trigger)`);
        const results = await distributeK1ToAllPendingSafe(_offboarded);
        const okCount = results.filter(r => r.ok).length;
        const failCount = results.length - okCount;
        if (okCount > 0) {
          try { toast(`🎉 新規社員 ${okCount} 名に K1 を自動配布しました` + (failCount > 0 ? ` (${failCount} 名失敗)` : ""), "ok", 10000); } catch {}
          // 配布完了で badge も refresh (= count 0 で消える想定)
          _updateAdminTabBadge().catch(() => {});
        } else if (failCount > 0) {
          console.warn("[admin-unlock-auto-k1] 全配布失敗:", results);
        }
      } catch (e) {
        const msg = String(e?.message || "");
        if (msg.includes("429") || msg.toLowerCase().includes("rate")) {
          console.log("[admin-unlock-auto-k1] rate-limited; retry on admin tab open");
        } else {
          console.warn("[admin-unlock-auto-k1] failed (non-fatal):", msg);
        }
      }
    }, 3000);  // 3 秒待ち = unlock 直後の他処理 (= corp/info, balance fetch 等) が落ち着いた頃
  }
}
...
// =====================================================================
// 2026-07-12: 保存先ストレージ (Turbo/Arweave) の一時障害を判定。
//   server 由来: upload_failed(502)/storage 系 503/プラットフォーム 502 (HTML page)。
//   client 由来: Load failed / Failed to fetch (= 応答なしで接続断)。
//   これらは全て「まだ保存されていない・後で再試行すれば直る」性質なので、
//   生のエラー文字列ではなく専用の分かりやすいメッセージで案内する。
function _isStorageOutageError(err) {
  if (!err) return false;
  if (err.code === "upload_failed" || err.code === "storage_outage" ||
      err.code === "private_pool_unavailable" || err.code === "free_pool_unavailable") return true;
  const s = err.status;
  if (s === 502 || s === 503 || s === 504) return true;
  return /HTTP 50[234]|Bad gateway|Load failed|Failed to fetch|NetworkError/i.test(String(err.message || ""));
}
...
function updateSaveStatusBadge(saveState, info = {}) {
  ...
  // Phase ZZ-SAVE: dirty / saving 時は TX badge (#header-account) を隠して
  // save badge が同位置 (= 行 2 / desktop は inline) を占有する。 これで
  // 「未保存 — クリックで今すぐ保存」 が wrap せず 1 line に収まる。
  const txEl = document.getElementById("header-account");
  const _hideTx = () => { if (txEl) txEl.style.display = "none"; };
  const _showTx = () => { if (txEl) txEl.style.display = ""; };
  switch (saveState) {
    case "dirty": {
    ...
    case "saved": {
      ...
      try { maybeShowRateNudge(); } catch (_) {}   // 2026-07-13: 評価ナッジ (リンク版)
      // Phase ZZ-SAVE-2: saved 中も TX hide 継続 (= layout 競合防止)。
      _hideTx();
      // Auto-fade after 4 seconds when idle
      setTimeout(() => {
        if (el && el.textContent.startsWith("✓")) {
          el.style.opacity = "0";
          // fade transition 完了 (= 0.3s) 後に badge を DOM から完全削除し TX を復帰。
          // display:none ではなく remove() を使う理由: mobile CSS の
          // '#save-status-badge:not(:empty) { display:inline-flex !important }' が
          // inline display:none を上書きしてしまうため。 next dirty/saving で再生成。
          setTimeout(() => {
      ...
      // Refresh header (credits display) — TX は hidden だが innerHTML は更新
    ...
    case "error": {
      ...
      } else if (_isStorageOutageError(err)) {
        el.textContent = i18n_t("app.save_status.error_outage");
        el.title = i18n_t("app.save_status.error_outage_tooltip");
      } else {
        el.textContent = i18n_t("app.save_status.error");
        el.title = i18n_t("app.save_status.error_tooltip");
      }
      _showTx();  // Phase ZZ-SAVE: error 時は TX も表示で context 提供
  ...
  try {
    ...
    //   完全新規の場合は前段の routing で profile が未作成のため、ここで補完作成する。
    {
      ...
      if (!profMod.getActiveProfileId()) {
        ...
        const prof = profMod.createProfile({ kind });
    ...
    // 2026-07-12: Turbo Payment Service 障害中 (= server が直近5分に upload_failed を記録)
    //   なら、最初から <100KiB sponsored パディングで作成する。createVault は途中で
    //   再暗号化すると signing 鍵が変わり register 済み pubkey と不整合になるため、
    //   事前判定方式 (saveVault と違い書き込み失敗後の retry は不可)。
    try {
      const _hr = await fetch("/api/status", { signal: AbortSignal.timeout(4000) });
      const _hj = await _hr.json();
      if (_hj?.storage_down) enableEmergencyPad();
    } catch (_) { /* status 取得失敗は通常パディングで続行 */ }
    ...
    // Stage G9 hotfix (2026-06-06): _businessJoinAndDeposit 呼び出しを撤去。
    //   旧 α-pre design では (a) /api/corp/join で会社加入 + (b) Recovery を admin に
    //   deposit していたが、 α 以降は (a) は /api/write の inviteCode option で
    //   atomic 化 (Phase 7.2-B/D hotfix)、 (b) は廃止 (= 社員自身が Recovery 保管)。
    //   よってここで join+deposit を再実行する必要は無い。 残しておくと invite が
    //   消費済のため必ず "Invite code not found" 404 になり、 "Recovery 送信失敗"
    //   という存在しない動作のエラー文言で user を混乱させていた。
    const businessDepositOk = !!_inviteCode;  // signup 成功 = atomic 加入も成功
    ...
    // PRF 非対応端末 (iOS/iPadOS < 18.4 等): 生の "PRF extension required..." では
    //   何が問題か伝わらないので、 OS 更新を促す分かりやすい案内に差し替える。
    if (e?.code === "prf_unsupported" || /PRF extension required/i.test(e?.message || "")) {
      err.textContent = i18n_t("app.error.prf_unsupported");
      err.classList.remove("hidden");
      document.getElementById("passkey-unavailable")?.classList.remove("hidden");
    } else if (_isStorageOutageError(e)) {
      // 2026-07-12: Turbo/Arweave 一時障害。生の "Upload failed"/"HTTP 502" ではなく
      //   「未保存・後で再試行」を明確に伝える。
      err.textContent = i18n_t("app.error.storage_outage");
      err.classList.remove("hidden");
    } else {
...
document.getElementById("unlock-btn").addEventListener("click", async () => {
  ...
  try {
    ...
    // 2026-07-12: refreshHeader (= /api/balance) を await しない。遅い回線では
    //   数十秒かかり、解錠済みなのに vault 画面が出ないのはこれが原因だった。
    //   失敗時は retry (利用額行が空のままにならないように)。
    refreshHeaderWithRetry();
    renderList();
    showView("vault");
  ...
  try {
    ...
    if (window.confirm(i18n_t("app.confirm.add_passkey_after_recovery"))) {
      ...
      try {
        ...
        // PRF 非対応端末なら OS 更新を促す案内。 それ以外は従来の汎用メッセージ。
        if (passkeyErr?.code === "prf_unsupported" || /PRF extension required/i.test(passkeyErr?.message || "")) {
          toast(i18n_t("app.error.prf_unsupported"), "warn", 12000);
        } else {
...
document.getElementById("unlock-pk-rs-btn")?.addEventListener("click", async () => {
  ...
  try {
    ...
    try {
      const profMod = await import("/lib/profiles.js?v=69cb395d");
      if (!profMod.getActiveProfileId()) {
        const prof = profMod.createProfile({ kind: "personal" });
        profMod.setActiveProfileId(prof.id);
      }
    } catch (e) { /* non-fatal */ }
...
function openSettings() {
  ...
  try { showAppBuildId(); } catch (e) { /* 表示のみ、失敗しても無害 */ }
}

// 設定画面に「アプリが実際に動かしている shell の版 (BUILD_ID)」を表示する。
//   controlling Service Worker に GET_BUILD_ID を postMessage で問い合わせ、
//   返ってきた BUILD_ID を表示する。 controlling SW が古く GET_BUILD_ID 未対応
//   の場合は返答が来ないので、 1.2s で「旧バージョン（要更新）」にフォールバック。
//   これでユーザーが「更新が反映されたか」を自分の目で確認できる。
function _askSwBuildId(worker, timeoutMs) {
  return new Promise((resolve) => {
    try {
      if (!worker) { resolve(null); return; }
      const ch = new MessageChannel();
      let settled = false;
      ch.port1.onmessage = (ev) => { settled = true; resolve((ev && ev.data && ev.data.buildId) || null); };
      worker.postMessage({ type: "GET_BUILD_ID" }, [ch.port2]);
      setTimeout(() => { if (!settled) resolve(null); }, timeoutMs || 1000);
    } catch { resolve(null); }
  });
}

// 実際に動いている shell の版 (BUILD_ID) を解決する。
//   1) controlling SW → 2) active SW (controller 未確立でも取れる) →
//   3) /sw.js を直接 fetch して parse。 SW が page を制御していない WKWebView
//   でも 3) で必ず版が出る (「不明（SW 未登録）」を出さない)。
async function resolveBuildId() {
  const sw = navigator.serviceWorker;
  if (sw) {
    let id = await _askSwBuildId(sw.controller, 1000);
    if (id) return id;
    try {
      const reg = await sw.getRegistration();
      id = await _askSwBuildId(reg && reg.active, 1000);
      if (id) return id;
    } catch {}
  }
  try {
...
// 設定画面 / ログイン前画面の「.app-build-id」全てに版を表示する。
//   ログインできない人でも版を確認できるよう pre-auth 画面にも出す。
async function showAppBuildId() {
  const els = document.querySelectorAll(".app-build-id");
  if (!els.length) return;
  els.forEach((el) => { el.textContent = "…"; });
  const id = await resolveBuildId();
  const text = id || ((typeof i18n_t === "function" && i18n_t("app.settings.build_unknown")) || "不明");
  document.querySelectorAll(".app-build-id").forEach((el) => { el.textContent = text; });
}

// ページ表示直後 (= まだログイン前) にも版を出す。
if (document.readyState === "loading") {
  document.addEventListener("DOMContentLoaded", () => { try { showAppBuildId(); } catch {} });
} else {
  try { showAppBuildId(); } catch {}
}
...
document.getElementById("hwkey-devadd-input-submit")?.addEventListener("click", async () => {
  ...
  try {
    ...
    const { createProfile, setActiveProfileId, getActiveProfileId } = await import("/lib/profiles.js?v=69cb395d");
    if (!getActiveProfileId()) {
      setActiveProfileId(createProfile({ kind: "personal" }).id);
    }
...
document.getElementById("sec-pk-btn")?.addEventListener("click", async () => {
  ...
  try {
    // task16 (2026-07-17): rs (Recovery Secret) を addCredentialOnThisDevice に渡す。
    //   passkey 解錠 session は recovery を持たないため、これが無いと
    //   「Recovery を要する操作です」で失敗していた。backend 側で appNameTag 一致を
    //   検証し、誤 Recovery で孤児パスキーを作らない。v8 なので他端末の passkey は
    //   上書きされず coexist する (新規追加として登録)。
    // 2026-07-17: パスキー追加も Arweave 書込み(新 txid)を伴うので、ヘッダの
    //   保存ステータスバッジにも saving→saved を出して一貫させる (従来は toast のみで
    //   「保存された表示が出ない」と分かりにくかった)。
    updateSaveStatusBadge("saving");
    const _pkRes = await addCredentialOnThisDevice(pw, _defaultPasskeyDisplayName(), { recoverySecret: rs });
    updateSaveStatusBadge("saved", { txid: _pkRes?.txid ?? currentLatestTxId() });
    await refreshHeader();
    toast(i18n_t("app.toast.passkey_registered_new"), "ok");
    closeSettings();
  } catch (e) {
...
document.getElementById("lock-btn-global")?.addEventListener("click", async () => {
...
// =====================================================================
// Android TWA autofill 保存 → deep link 取り込み
// native の onSaveRequest が arpass.io/app#arpass_add=<b64url(JSON{site,user,pw})> で開く。
// URL fragment はサーバに送られない。解錠後に「追加」モーダルを事前入力で開き、
// ユーザーが確認して通常保存する (native 書込みコード不要・成否が UI で見える)。
// =====================================================================
let _pendingAddEntry = null;
let _pendingAddApplied = false;
function _parsePendingAdd() {
  try {
    const m = (location.hash || "").match(/arpass_add=([A-Za-z0-9_-]+)/);
    if (!m) return;
    const b64 = m[1].replace(/-/g, "+").replace(/_/g, "/");
    // NOTE: モジュール内に HTML エスケープ用の local function escape() があり
    // 組み込み escape() を shadow するため、従来の decodeURIComponent(escape(atob()))
    // 方式は JSON を壊して必ず parse 失敗していた。TextDecoder で UTF-8 復号する。
    const _bin = atob(b64);
    const _bytes = Uint8Array.from(_bin, (c) => c.charCodeAt(0));
    const o = JSON.parse(new TextDecoder().decode(_bytes));
    if (o && (o.site || o.user || o.pw)) {
      _pendingAddEntry = {
        site: String(o.site || ""), url: String(o.url || ""), user: String(o.user || ""), pw: String(o.pw || ""),
      };
      _pendingAddApplied = false; // 新しい取り込みは未適用に戻す (warm/2回目以降も動く)
    }
  } catch (e) { console.warn("[pending-add] parse failed:", e?.message); }
  // 機微情報を URL/履歴から即時消す
  try { history.replaceState(null, "", location.pathname + location.search); } catch {}
  // 既に解錠済みなら即適用 (warm: arpass が開いた状態で deep link が来たケース)
  try { _maybeApplyPendingAdd(); } catch {}
}
_parsePendingAdd();
// arpass が既に開いている状態で deep link が来ると hash だけ変わり再ロードされず
// _parsePendingAdd が再実行されない (解錠済みで「画面遷移だけ」になる不具合)。hashchange で拾う。
window.addEventListener("hashchange", _parsePendingAdd);
function _maybeApplyPendingAdd() {
  if (!_pendingAddEntry || _pendingAddApplied || !isUnlocked()) return;
  _pendingAddApplied = true;
  const data = _pendingAddEntry; _pendingAddEntry = null;
  try {
    openModal(null);
    const s = document.getElementById("m-site"); if (s) s.value = data.site;
    // url にもドメインを入れる。空だと詳細モーダルの「開く」ボタン (d-url-open) が
    // 機能しないため。ボタン側が http 始まりでなければ https:// を補完する。
    const ul = document.getElementById("m-url"); if (ul) ul.value = data.url ? String(data.url) : (data.site ? "https://" + String(data.site).replace(/^https?:\/\//, "") : "");
    const u = document.getElementById("m-user"); if (u) u.value = data.user;
    const p = document.getElementById("m-pw");   if (p) p.value = data.pw;
  } catch (e) { console.warn("[pending-add] apply failed:", e?.message); }
}

function renderHistoryFallbackBanner() {
  const el = document.getElementById("history-fallback-banner");
  if (!el) return;
  const info = (typeof currentRestoredFromHistory === "function") ? currentRestoredFromHistory() : null;
  if (!info) { el.classList.add("hidden"); el.innerHTML = ""; return; }
  const sh = (x) => x ? (String(x).slice(0, 10) + "…") : "?";
  el.innerHTML = "⚠ 最新版が読めなかったため、 直前の正常版 (" + sh(info.restoredTxid) + ") で表示しています。"
    + " 最新 (" + sh(info.latestTxid) + ") は壊れている可能性があります。"
    + " <button id=\"history-retry-btn\" type=\"button\" style=\"margin-left:8px;\">最新を再試行</button>"
    + " <span style=\"display:block;margin-top:6px;font-size:12px;\">この版のまま保存すれば、 これを新しい最新として続行できます。</span>";
  el.classList.remove("hidden");
  const btn = document.getElementById("history-retry-btn");
  if (btn) btn.onclick = async () => {
    // 2026-07-13: 単純 reload だと wrap 欠落端末で無限ループ。 まず最新を検査する。
    btn.disabled = true;
    const _orig = btn.textContent;
    btn.textContent = i18n_t("app.history_retry.checking") || "確認中…";
    let res;
    try { res = await retryLatestForRestoredSession(); }
    catch (e) { res = { status: "fetch_failed", message: e?.message }; }
    if (res?.status === "retryable" || res?.status === "refreshed") {
      // この端末の wrap が最新にある (= 伝播完了)。 解錠し直せば最新が開ける。
      location.reload();
      return;
    }
    btn.disabled = false;
    btn.textContent = _orig;
    if (res?.status === "device_missing") {
      // reload しても永久に開けない → 復旧導線を案内 (ループ回避)。
      toast(i18n_t("app.history_retry.device_missing") ||
        "この端末の解錠情報が最新版にありません。 マスターパスワード + リカバリーコードで解錠し、 この端末の Passkey を登録し直してください。",
        "warn", 14000);
    } else if (res?.status === "nochange") {
      toast(i18n_t("app.history_retry.nochange") || "サーバ上の最新は変わっていません。", "info", 6000);
    } else {
      toast(i18n_t("app.history_retry.fetch_failed") ||
        "最新版をまだ取得できません (配信待ちの可能性)。 数分後にもう一度お試しください。",
        "warn", 10000);
    }
  };
}
...
function renderList() {
  // 履歴フォールバックで古い版を開いている時は常設バナーで明示する。
  renderHistoryFallbackBanner();
  ...
  if (!entries.length) {
    ...
    _maybeApplyPendingAdd();  // 空 / フィルタ0件でも、ロック→解錠後の保留登録は適用する
    return;
  }
...
function syncSaveButtonToCredits() {
  ...
  // Postpaid: サーバ判定 canWrite が false (お試し使い切り / 利用上限 / 凍結) の
  // 時だけ購入 CTA に切替。**お試し枠内 (canWrite=true) は未登録でも保存できる**。
  if (!state.canWrite) {
    saveBtn.textContent = state.registrationPaid
      ? i18n_t("app.button.pay_usage_emoji")
      : i18n_t("app.button.register_emoji");
    saveBtn.dataset.purchaseMode = "1";
    saveBtn.title = state.registrationPaid
      ? i18n_t("app.toast.usage_limit_click_pay")
      : i18n_t("app.toast.registration_required_click");
  } else {
  ...
  // records (ファイル) modal の保存ボタンも同じ状態に同期する
  const recBtn = document.getElementById("records-modal-save");
  if (recBtn) {
    if (!state.canWrite) {
      recBtn.textContent = state.registrationPaid
        ? i18n_t("app.button.pay_usage_emoji")
        : i18n_t("app.button.register_emoji");
      recBtn.dataset.purchaseMode = "1";
    } else {
      recBtn.textContent = i18n_t("app.records.btn_save");
      delete recBtn.dataset.purchaseMode;
    }
  }
}
...
function showSaveError(el, e, prefix = i18n_t("app.error.failed")) {
  ...
  if (e?.code === "registration_required") {
    // Postpaid: 登録料未払い。登録 CTA を出す。
    _absorbPostpaidAccount(e?.account);
    el.innerHTML = i18n_t("app.error.registration_required_cta", { cta: purchaseCtaHtml() });
  } else if (e?.code === "insufficient_credits") {
    // Postpaid: ご利用額が上限到達 (旧: 残高不足)。402 response は sanitized account 同梱。
    _absorbPostpaidAccount(e?.account);
 ...
 */
/**
 * Postpaid: 402 response 同梱の sanitized account を state に反映する共通処理。
 */
function _absorbPostpaidAccount(account) {
  if (!account || typeof account !== "object") return;
  if (typeof account.estimatedWrites === "number") state.credits = account.estimatedWrites;
  if (typeof account.perWriteUsd === "number") state.perWriteUsd = account.perWriteUsd;
  if (typeof account.perWriteUsdBase === "number") state.perWriteUsdBase = account.perWriteUsdBase;
  if (typeof account.owedUsdMicro === "number") state.owedUsdMicro = account.owedUsdMicro;
  if (typeof account.owedJpyApprox === "number") state.owedJpyApprox = account.owedJpyApprox;
  if (typeof account.stopLimitJpy === "number") state.stopLimitJpy = account.stopLimitJpy;
  state.registrationPaid = !!account.registrationPaid;
  if (account.canWrite !== undefined) state.canWrite = account.canWrite !== false;
  if (account.writeBlockedReason !== undefined) state.writeBlockedReason = account.writeBlockedReason;
  refreshHeader().catch(() => {});
}
...
function toastSaveError(e, prefix = i18n_t("app.error.failed")) {
  if (e?.code === "registration_required") {
    _absorbPostpaidAccount(e?.account);
    toast(i18n_t("app.toast.registration_required"), "err");
...
// =====================================================================
// 2026-07-12: balance (10s timeout) が失敗したままだと header の利用額行が
//   出っぱなしで空になるため、失敗時は 15 秒間隔で数回再試行する。
//   遅い回線でも接続が温まれば 2 回目以降は数秒で返る。
function refreshHeaderWithRetry(attempts = 5) {
  refreshHeader().catch(() => {
    if (attempts > 0) setTimeout(() => refreshHeaderWithRetry(attempts - 1), 15_000);
  });
}
...
async function refreshHeader() {
  ...
  try {
    ...
    if (typeof s.perWriteUsdBase === "number") state.perWriteUsdBase = s.perWriteUsdBase;
      ...
      // Postpaid: 利用額・上限・書込可否
      if (typeof s.owedUsdMicro === "number") state.owedUsdMicro = s.owedUsdMicro;
      if (typeof s.owedJpyApprox === "number") state.owedJpyApprox = s.owedJpyApprox;
      if (typeof s.stopLimitJpy === "number") state.stopLimitJpy = s.stopLimitJpy;
      state.registrationPaid = !!s.registrationPaid;
      state.canWrite = s.canWrite !== false;
      state.writeBlockedReason = s.writeBlockedReason ?? null;
      state.trialUsageUsdMicro = (typeof s.trialUsageUsdMicro === "number") ? s.trialUsageUsdMicro : null;
      state.tier = s.tier ?? state.tier ?? null;
      state.corpEmployee = !!s.corpBilling;
      state.corpSubscriptionActive = !!s.corpSubscriptionActive;
      state.corpMonthUsageUsdMicro = (typeof s.corpMonthUsageUsdMicro === "number") ? s.corpMonthUsageUsdMicro : null;
      state.corpIncludedJpy = (typeof s.corpIncludedJpy === "number") ? s.corpIncludedJpy : null;
      ...
      // 行3: ご利用額 + 単価 (postpaid — 「残高」表示は廃止)
      const balanceEl = document.getElementById("header-balance");
      if (balanceEl) {
        balanceEl.innerHTML = renderUsageHtml();
      }
...
function renderUsageHtml() {
  // Postpaid (docs/postpaid-billing-design.md §2.3):
  //   「今月のご利用額 ¥X」— 使った分だけ増えていく表示。マイナス値・「残高」
  //   という語・「残り◯回」型の残数表示は使わない (回数券=数量型前払式の外観回避)。
  //
  // Color-codes:
  //   • 未登録 (registration_required) → red, 「登録して利用開始」CTA
  //   • 書込停止 (owed ≥ limit)        → red, 「お支払い」CTA
  //   • 利用額が上限の 6 割超          → orange, 「お支払い」CTA
  //   • それ以外                       → muted purple, 利用額 + 単価 subscript
  // 表示する金額の選択 (2026-07-09 法人表示を 1 本化 — ユーザー指定のモデル):
  //   お試し中 → 参考額 (請求されない)
  //   法人契約中 → 「今月のご利用額」= 枠内利用 + 超過 (単一の正しい金額)。
  //                月替わりで枠内分 (≤¥1,000) は自動リセット、超過分だけ翌月に残る。
  //                お支払いできるのは超過分 (owed) のみ — 台帳側で既に強制されている。
  //   それ以外 → 未払 (ご利用額)
  const isTrialDisplay = !state.registrationPaid && (typeof state.trialUsageUsdMicro === "number");
  const isCorpDisplay = !isTrialDisplay && state.corpSubscriptionActive;
  const JPY_RATE_HDR = 154;
  const corpIncludedMicro = (typeof state.corpIncludedJpy === "number")
    ? Math.round(state.corpIncludedJpy / JPY_RATE_HDR * 1_000_000) : 0;
  const corpDisplayMicro = ((typeof state.owedUsdMicro === "number") ? state.owedUsdMicro : 0)
    + Math.min((typeof state.corpMonthUsageUsdMicro === "number") ? state.corpMonthUsageUsdMicro : 0, corpIncludedMicro);
  const owedUsdMicro = isTrialDisplay
    ? state.trialUsageUsdMicro
    : (isCorpDisplay
        ? corpDisplayMicro
        : ((typeof state.owedUsdMicro === "number") ? state.owedUsdMicro : 0));
  const owedUsd = owedUsdMicro / 1_000_000;
  const limitJpy = (typeof state.stopLimitJpy === "number") ? state.stopLimitJpy : 500;
  const JPY_PER_USD = 154;
  const owedJpy = owedUsd * JPY_PER_USD;

  let color = "var(--accent)";
  let cta = "";
  let title = i18n_t("app.label.usage_click_to_pay");

  if (!state.registrationPaid) {
    // 無償お試し枠内 (canWrite=true) は黄色の案内、使い切ったら赤
    color = state.canWrite ? "#FBBF24" : "var(--red)";
    cta = ' <span style="text-decoration:underline; white-space:nowrap;">' + i18n_t("app.misc.register_link_html") + '</span>';
    title = i18n_t("app.label.registration_required_click");
  } else if (!state.canWrite) {
    color = "var(--red)";
    cta = ' <span style="text-decoration:underline; white-space:nowrap;">' + i18n_t("app.misc.pay_link_html") + '</span>';
    // ja は「¥{limit}」表記、他言語は USD 換算の通貨込み文字列を渡す
    const limitStr = (i18n_getLang() === "ja")
      ? limitJpy.toLocaleString()
      : `$${(limitJpy / JPY_PER_USD).toFixed(2)}`;
    title = i18n_t("app.label.usage_limit_reached_click_to_pay", { limit: limitStr });
  } else if (owedJpy >= limitJpy * 0.6) {
    color = "#FBBF24";
    cta = ' <span style="text-decoration:underline; white-space:nowrap;">' + i18n_t("app.misc.pay_link_html") + '</span>';
  }

  // 利用額を「整数部を主に、小数部を従に」visual hierarchy で表示 (旧残高表示と同 pattern)
  const lang = i18n_getLang();
  let usageLabel;
  if (lang === "ja") {
    const formatted = owedJpy.toLocaleString("ja-JP", {
      minimumFractionDigits: 2,
      maximumFractionDigits: 2,
    });
    const [intPart, decPart] = formatted.split(".");
    usageLabel = `<span style="white-space:nowrap;">¥${intPart}<span style="font-size:0.7em; opacity:0.65; vertical-align:baseline;">.${decPart}</span></span>`;
  } else {
    const formatted = owedUsd.toLocaleString("en-US", {
  ...
  const usagePrefix = isTrialDisplay
    ? `<span style="font-size:12px; opacity:0.8; margin-right:3px; white-space:nowrap;">${i18n_t("app.label.trial_usage_prefix")}</span>`
    : (isCorpDisplay
        ? `<span style="font-size:12px; opacity:0.8; margin-right:3px; white-space:nowrap;">${i18n_t("app.label.corp_usage_prefix")}</span>`
        : `<span style="font-size:12px; opacity:0.8; margin-right:3px; white-space:nowrap;">${i18n_t("app.label.usage_prefix")}</span>`);
  if (isCorpDisplay) {
    // tooltip で「支払いは超過分のみ」と現在の超過額を明示 (ja=¥ / 他=USD の整形済み文字列を渡す)
    const isJaHdr = (i18n_getLang() === "ja");
    const includedJpy = state.corpIncludedJpy ?? 1000;
    const payableUsd = (state.owedUsdMicro || 0) / 1_000_000;
    title = i18n_t("app.label.corp_usage_title", {
      included: isJaHdr ? `¥${includedJpy.toLocaleString()}` : `$${(includedJpy / JPY_RATE_HDR).toFixed(2)}`,
      payable: isJaHdr ? `¥${Math.round(payableUsd * JPY_RATE_HDR).toLocaleString()}` : `$${payableUsd.toFixed(2)}`,
    });
  }
  const perWriteCost = perWriteCostDisplay();
  const subLabel = perWriteCost
    ? `<span class="usage-sub" style="font-size:13px; opacity:0.85; margin-left:4px; white-space:nowrap;">(${perWriteCost})</span>`
    : "";
  return `<button type="button" data-action="open-purchase-modal" title="${title}" style="background:transparent; border:none; padding:2px 6px; cursor:pointer; color:${color}; text-decoration:none; font-weight:600; font-size:14px; display:inline-flex; flex-wrap:wrap; align-items:center; justify-content:flex-start; text-align:left; column-gap:4px; row-gap:2px; max-width:100%;">
            <span class="usage-l1" style="display:inline-flex; align-items:center; gap:4px; white-space:nowrap;"><span aria-hidden="true">💳</span>${usagePrefix}</span><span class="usage-l2" style="display:inline-flex; align-items:center; column-gap:4px; row-gap:2px;"><span style="white-space:nowrap;">${usageLabel}${subLabel}</span>${cta}</span>
          </button>`;
}
...
function renderTxStatusHtml(txid) {
  ...
  return `
    <button type="button" data-action="open-tx-detail" data-txid="${txid}" title="${titleText}" style="background:transparent; border:1px solid var(--line); border-radius:6px; padding:2px 8px; cursor:pointer; color:${badgeColor}; font-size:13px; display:inline-flex; align-items:center; gap:4px; word-break:keep-all;">
...
async function _routeOnStartup({ forcePicker = false } = {}) {
  ...
  const { listProfiles, getActiveProfileId, setActiveProfileId, dedupeProfiles, relabelAllAuto } = await import("/lib/profiles.js?v=69cb395d");
  // Phase 7.1-AE: 起動時に同一 drive の重複 profile を統合 (localStorage 整理のみ)。
  try { dedupeProfiles(); relabelAllAuto(); } catch (e) { console.warn("[startup] dedupe/relabel failed:", e?.message); }
...
function _renderProfilePicker(profiles, hasInvite) {
  ...
  for (const p of profiles) {
    ...
    const emptyBadge = isEmpty
      ...
      : "";
    const labelHtml = `<span style="font-weight:600; font-size:14px;">${icon} ${escapeHtml(displayLabel(p))}${emptyBadge}</span>`;
...
function _showUnlockOrCreate() {
  if (!hasVaultIdentity()) {
    ...
    showView("unlock");
    _applyNormalUnlockView();   // hwkey 画面からの復帰時に DOM を必ず通常表示へ戻す
...
// hwkey 専用ビューからの復帰。 _applyHwkeyUnlockView() の変更 (pw 欄非表示・
// subtitle/ボタン文言の YubiKey 化) を確実に元へ戻す。 これが無いと、一度
// YubiKey ドライブを選ぶと normal ドライブを選び直しても画面が YubiKey の
// まま固定されてしまう (= 全部 YubiKey で開く バグ)。
function _applyNormalUnlockView() {
  const pw = document.getElementById("unlock-pw");
  if (pw) {
  ...
  const sub = document.getElementById("unlock-subtitle");
  if (sub) sub.textContent = i18n_t("app.unlock.subtitle_password");
  const btn = document.getElementById("unlock-btn");
  if (btn) btn.textContent = i18n_t("app.unlock.btn_short");
}
...
async function _hwkeyUnlockFlow(btn, err, idleLabel, opts = {}) {
  ...
  try {
    ...
    const a = await hwkeyAuthenticateForUnlock(opts);
    _phase("phase_lookup");
    const { createProfile, setActiveProfileId, getActiveProfileId } = await import("/lib/profiles.js?v=69cb395d");
    if (!getActiveProfileId()) {
      setActiveProfileId(createProfile({ kind: "personal" }).id);
    ...
    // Android TWA autofill の provision は vault-client.unlockWithHwkeyAuthed 内で
    //   provisionAndroidAutofill により credId+userHandle を直行キャッシュに上書きするため、
    //   ここでの個別 provision は不要(統一: 最後に解錠した Vault を hwkey/AB 問わず追従)。
...
document.getElementById("goto-hwkey-unlock")?.addEventListener("click", (e) => {
  ...
  _hwkeyUnlockFlow(
    ...
    { forcePicker: true },   // 別端末導線は iOS/PC で一覧表示。Android は内部で specific 維持。
  );
});
...
document.getElementById("hwkey-create-btn")?.addEventListener("click", async () => {
  ...
  try {
    const { createProfile, setActiveProfileId, getActiveProfileId } = await import("/lib/profiles.js?v=69cb395d");
    if (!getActiveProfileId()) {
      const p = createProfile({ kind: "personal" });
      setActiveProfileId(p.id);
    }
...
document.getElementById("unlock-switch-profile-link")?.addEventListener("click", async (e) => {
  e.preventDefault();
  const { clearActiveProfile } = await import("/lib/profiles.js?v=69cb395d");
  clearActiveProfile();
...
async function _renderCreateBackToPicker() {
  const { listProfiles } = await import("/lib/profiles.js?v=69cb395d");
  const profiles = listProfiles();
...
async function _pruneEmptyProfiles() {
  const { listProfiles, deleteProfile } = await import("/lib/profiles.js?v=69cb395d");
  const profiles = listProfiles();
  let pruned = 0;
  for (const p of profiles) {
    const metaRaw = localStorage.getItem(`arpass_vault_meta_v5__${p.id}`);
...
async function _startBusinessJoinFlow() {
  try {
    ...
    if (isInviteFormat) {
      ...
      const { createProfile, setActiveProfileId } = await import("/lib/profiles.js?v=69cb395d");
      const profile = createProfile({ kind: "corp" });
    ...
    while (Date.now() - startedAt < TIMEOUT_MS && !cancelled) {
      ...
      try {
        ...
        if (grant) {
          ...
          const { eciesDecrypt } = await import("/lib/vault-crypto.js?v=6bb1e228");
...
document.getElementById("picker-delete-mode-btn")?.addEventListener("click", async () => {
  ...
  const labels = profiles.map((p, i) => `${i + 1}. ${displayLabel(p)} (${p.kind}${p.companyId ? " — " + p.companyId.slice(0,8) : ""})`).join("\n");
...
(async function init() {
  // Phase 7.5: 起動時の await fetch("/api/status") を削除。
  //   bundlerReadBase は client のどこからも参照されない死にコード (self-hosted
  //   bundler 時代の遺物) であり、 この await が unlock UI の表示をゲートしていた。
  //   ネットワークがストールする端末では白画面が ~60 秒続く実害 (2026-07-12 実機で確認)。
  //   初期 view の表示は network に依存させないこと。
...
async function tryConsumePendingPurchase() {
  ...
  // Postpaid: pricing.html からの pack deep-link は廃止。互換のため、
  // hash が残っていた場合は後払いの購入モーダル (登録 or 精算) を開くだけ。
  window.openPurchasePackModal?.();
  return true;
}

// =====================================================================
// IAP (アプリ内課金) ブリッジ — iOS StoreKit / Android Play Billing
// =====================================================================
// 購入は環境で経路が分かれる:
//   - iOS 本体アプリ(WKWebView): StoreKit を native で実行(webkit.messageHandlers.arpassPurchase)。
//       Apple 規約上、アプリ内のデジタル課金は IAP 必須。Stripe は使わない。
//   - Android TWA: Digital Goods API + PaymentRequest(play.google.com/billing) を web で実行。
//   - それ以外(実 web): 従来どおり Stripe。
function _inIosApp() {
  try { return !!globalThis.webkit?.messageHandlers?.arpassAutofill; } catch { return false; }
}
function _hasIosPurchase() {
  try { return !!globalThis.webkit?.messageHandlers?.arpassPurchase; } catch { return false; }
}
function _hasAndroidBilling() {
  try {
    const mm = window.matchMedia;
    const inApp = mm && (mm("(display-mode: standalone)").matches || mm("(display-mode: fullscreen)").matches);
    return !!inApp && /Android/i.test(navigator.userAgent || "") && typeof window.getDigitalGoodsService === "function";
  } catch { return false; }
}

// iOS StoreKit: reqId で native へ依頼 → __arpassPurchaseResolve(reqId, {ok,signedTransaction,error}) で返る。
const _iosPurchasePending = new Map();
if (!globalThis.__arpassPurchaseResolve) {
  globalThis.__arpassPurchaseResolve = (reqId, result) => {
    const pend = _iosPurchasePending.get(reqId);
    if (!pend) return;
    _iosPurchasePending.delete(reqId);
    pend(result || {});
  };
}
function _iosStoreKitPurchase(productId) {
  return new Promise((resolve, reject) => {
    const reqId = "p" + Date.now() + Math.random().toString(36).slice(2, 8);
    _iosPurchasePending.set(reqId, (r) => {
      if (r.ok && r.signedTransaction) resolve({ signedTransaction: r.signedTransaction, transactionId: r.transactionId || null });
      else reject(new Error(r.error || "StoreKit purchase failed/cancelled"));
    });
    try { globalThis.webkit.messageHandlers.arpassPurchase.postMessage({ reqId, productId }); }
    catch (e) { _iosPurchasePending.delete(reqId); reject(e); }
    setTimeout(() => { if (_iosPurchasePending.has(reqId)) { _iosPurchasePending.delete(reqId); reject(new Error("StoreKit timeout")); } }, 180000);
  });
}

// iOS: web のサーバ付与が成功したら native に finish を依頼(消耗型を確定)
function _iosFinishPurchase(transactionId) {
  if (!transactionId) return;
  try { globalThis.webkit.messageHandlers.arpassPurchase.postMessage({ action: "finish", transactionId: String(transactionId) }); } catch (_) {}
}
// iOS: 起動時/割込の未完了取引を native が再配達 → 検証(冪等)＋付与成功で finish。
//   未解錠(署名不可)なら何もしない(次回再配達される)。
if (!globalThis.__arpassPurchaseRedeliver) {
  globalThis.__arpassPurchaseRedeliver = async (signedTransaction, transactionId) => {
    try {
      if (!isUnlocked || !isUnlocked()) return;     // 解錠後でないと signedFetch 不可
      await iapVerifyApple(signedTransaction);
      _iosFinishPurchase(transactionId);
      try { startBalancePolling(); } catch (_) {}
      try { toast(i18n_t("app.toast.purchase_ok") || "購入が完了しました", "ok", 8000); } catch (_) {}
    } catch (e) { console.warn("[iap] redeliver 付与失敗(次回再試行):", e?.message); }
  };
}

// Android Play Billing (Digital Goods API + PaymentRequest)
async function _androidPlayPurchase(pack) {
  const sku = pack.androidProductId;
  const svc = await window.getDigitalGoodsService("https://play.google.com/billing");
  // 前回の検証失敗などで未消費のまま Play 側に残った購入を先に処理する。
  // サーバ検証は付与後に consume するため、成功すれば「すでに所有」状態が解消し再購入可能になる。
  if (typeof svc.listPurchases === "function") {
    let owned = [];
    try { owned = await svc.listPurchases(); } catch (e) { console.warn("[iap] listPurchases 失敗:", e?.message); }
    let redeemedTarget = false;
    for (const p of (owned || [])) {
      try {
        await iapVerifyGoogle(p.itemId, p.purchaseToken); // 検証＋付与＋consume(サーバ側・冪等)
        if (p.itemId === sku) redeemedTarget = true;
      } catch (e) { console.warn("[iap] 未消費購入の再検証に失敗:", p.itemId, e?.message); }
    }
    if (redeemedTarget) return "redelivered"; // 既に所有分を処理済み → 新規購入は不要
  }
  const details = await svc.getDetails([sku]);
  if (!details || !details.length) throw new Error("Play 商品情報が取得できません: " + sku);
  const item = details[0];
  const req = new PaymentRequest(
    [{ supportedMethods: "https://play.google.com/billing", data: { sku } }],
    { total: { label: item.title || sku, amount: { currency: item.price.currency, value: item.price.value } } },
  );
  const resp = await req.show();
  const purchaseToken = resp.details?.purchaseToken;
  // 2026-07-12: complete() はブラウザ仕様で 60 秒以内必須。サーバ検証を待ってから
  //   呼ぶ旧実装は、遅い回線で InvalidStateError (complete too late) になっていた。
  //   購入は Play 側で確定済みで complete("fail") でも返金されない = 先に閉じてよい。
  //   検証が失敗/中断しても未消費購入として残り、次回の listPurchases 再検証で
  //   救済される (冪等)。
  try { await resp.complete("success"); } catch (_) {}
  await iapVerifyGoogle(sku, purchaseToken);   // サーバ検証＋付与(消耗は検証側で実施)
  return purchaseToken;
}

// Postpaid: 旧 pack 確認モーダルの「続ける」は後払い購入モーダルへ委譲 (deep-link 互換)
document.getElementById("purchase-confirm-go")?.addEventListener("click", () => {
...
// Postpaid (docs/postpaid-billing-design.md §2.4): 前払いパックは廃止。
// 商品は「登録料」と「精算 (IAP 固定額)」のみ。Web (Stripe) の精算は
// 任意額 (未払額ちょうど) なので商品定義不要。
const POSTPAID_REGISTRATION = {
  kind: "registration", jpy: 300,
  iosProductId: "io.arpass.registration", androidProductId: "registration_300",
};
const POSTPAID_SETTLE_PRODUCTS = [
  { jpy: 100, iosProductId: "io.arpass.settle.100", androidProductId: "settle_100" },
  { jpy: 200, iosProductId: "io.arpass.settle.200", androidProductId: "settle_200" },
  { jpy: 300, iosProductId: "io.arpass.settle.300", androidProductId: "settle_300" },
  { jpy: 400, iosProductId: "io.arpass.settle.400", androidProductId: "settle_400" },
  { jpy: 500, iosProductId: "io.arpass.settle.500", androidProductId: "settle_500" },
];

/** 未払額 (JPY) 以下の最大の IAP 精算商品を 1 つ返す (= 切り捨て保証)。 */
function _settleProductForOwed(owedJpy) {
  let best = null;
  for (const p of POSTPAID_SETTLE_PRODUCTS) {
    if (p.jpy <= owedJpy && (!best || p.jpy > best.jpy)) best = p;
  }
  return best;
}
...
window.openPurchasePackModal = function () {
  ...
  // 法人の社員: 支払いは管理者のみ。購入カードは一切出さず案内のみ (steering 設計とも整合)
  if (state.corpEmployee) {
    const p = document.createElement("p");
    p.style.cssText = "font-size:13px; line-height:1.8; padding:14px; background:var(--paper); border-radius:10px; border:1px solid var(--line);";
    p.textContent = i18n_t("app.purchase.corp_employee_note");
    wrap.appendChild(p);
    document.getElementById("purchase-pack-bg").classList.remove("hidden");
    return;
  }
  const owedJpy = (typeof state.owedJpyApprox === "number")
    ? state.owedJpyApprox
    : Math.round(((state.owedUsdMicro || 0) / 1_000_000) * 154);
  const isNative = _inIosApp() || _hasAndroidBilling();

  const mkCard = ({ title, desc, price, disabled, onClick }) => {
    ...
    card.style.cssText = `
      ...
      border-radius: 10px; cursor: ${disabled ? "default" : "pointer"}; text-align: left;
      opacity: ${disabled ? "0.55" : "1"};
      transition: background 0.15s, border-color 0.15s;
    `;
    if (!disabled) {
      card.onmouseenter = () => { card.style.background = "#F1F5F9"; };
      card.onmouseleave = () => { card.style.background = "var(--paper)"; };
    }
    card.innerHTML = `
      <div>
      ...
      <div style="font-size: 18px; color: var(--accent);">${disabled ? "" : "→"}</div>
    `;
    if (!disabled && onClick) card.addEventListener("click", () => onClick(card));
    wrap.appendChild(card);
    return card;
  };

  // 金額表示: 日本語のみ ¥、それ以外は USD (2026-07-08 決定)
  const JPY_RATE = 154;
  const fmtJpyAmount = (jpy) => isJa
    ? `¥${jpy.toLocaleString()}`
    : `$${(jpy / JPY_RATE).toFixed(2)}`;
  const owedAmountStr = isJa
    ? `¥${owedJpy.toLocaleString()}`
    : `$${((state.owedUsdMicro || 0) / 1_000_000).toFixed(2)}`;

  if (!state.registrationPaid) {
    // --- 登録料 (アカウント有効化) ---
    mkCard({
      title: i18n_t("app.purchase.registration_title"),
      desc: i18n_t("app.purchase.registration_desc"),
      price: isJa ? "¥300" : "$2",
      onClick: (card) => initiatePostpaidPurchase("registration", card),
    });
  } else {
    // --- 精算 (ご利用料金のお支払い) ---
    if (isNative) {
      const p = _settleProductForOwed(owedJpy);
      if (p) {
        mkCard({
          title: i18n_t("app.purchase.settle_title"),
          desc: i18n_t("app.purchase.settle_desc", { amount: owedAmountStr }),
          price: fmtJpyAmount(p.jpy),
          onClick: (card) => initiatePostpaidPurchase("settlement", card),
        });
      } else {
        mkCard({
          title: i18n_t("app.purchase.settle_title"),
          desc: i18n_t("app.purchase.settle_below_min_iap"),
          price: "—",
          disabled: true,
        });
      }
    } else {
      const MIN_WEB_JPY = 50;
      // 法人: 都度払いは「今月の枠を超えた分」のみ。枠内 (または繰越のみ) の間は
      // カードを無効表示し「次回の月額請求に合算」と案内する。
      const corpIncMicro = state.corpSubscriptionActive && (typeof state.corpIncludedJpy === "number")
        ? Math.round(state.corpIncludedJpy / JPY_RATE * 1_000_000) : 0;
      const corpManualBlocked = state.corpSubscriptionActive
        && ((state.corpMonthUsageUsdMicro || 0) < corpIncMicro);
      if (state.corpSubscriptionActive && corpManualBlocked && (state.owedUsdMicro || 0) > 0) {
        mkCard({
          title: i18n_t("app.purchase.settle_overage_title"),
          desc: i18n_t("app.purchase.corp_overage_on_invoice"),
          price: "—",
          disabled: true,
        });
      } else if (owedJpy >= MIN_WEB_JPY) {
        mkCard({
          // 法人 (admin) の未払 = 込み枠を超えた超過分のみ、という位置づけを明示
          title: state.corpSubscriptionActive
            ? i18n_t("app.purchase.settle_overage_title")
            : i18n_t("app.purchase.settle_title"),
          desc: i18n_t("app.purchase.settle_desc", { amount: owedAmountStr }),
          price: owedAmountStr,
          onClick: (card) => initiatePostpaidPurchase("settlement", card),
        });
        ...
        mkCard({
          title: i18n_t("app.purchase.settle_title"),
          desc: i18n_t("app.purchase.settle_below_min_web"),
          price: "—",
          disabled: true,
        });
      }
  ...
  // --- 法人 / チーム (2d-lite): Web のみ申込可 (IAP なし — Enterprise Services 例外)。
  //     native では表示しない (steering 回避)。既に corp (契約済み/所属) なら出さない。
  const isCorpAlready = state.tier === "corp" || state.corpSubscriptionActive || !!(state._corpInfo?.member);
  if (!isNative && state.registrationPaid && !isCorpAlready) {
    mkCard({
      title: i18n_t("lp.pricing.corp_name"),
      desc: `${i18n_t("lp.pricing.corp_sub")} · ${i18n_t("lp.pricing.corp_desc_html").replace(/<br\s*\/?>/g, " ")}`,
      price: isJa ? "月額 ¥1,000" : "$6.50 / mo",
      onClick: (card) => initiatePostpaidPurchase("corp_subscription", card),
    });
  }

  // 「お支払いはいつでも」+「反映の目安時間」note
  const note = document.createElement("p");
  note.style.cssText = "font-size:12px; color:var(--muted); line-height:1.7; margin:12px 0 0; padding:10px 12px; background:var(--paper); border-radius:8px; border-left:3px solid var(--accent);";
  note.innerHTML = i18n_t("app.purchase.pay_anytime_html")
    + "<br><br>⏱ " + i18n_t("app.purchase.reflect_note_html");
...
/**
 * Postpaid 購入 (登録料 / 精算)。
 *   - iOS 殻: StoreKit 消耗型 → /api/iap/apple 検証
 *   - Android TWA: Play Billing → /api/iap/google 検証
 *   - Web: Stripe Checkout (金額はサーバ計算 — client は kind のみ送る)
 */
async function initiatePostpaidPurchase(kind, originBtnEl) {
  const owedJpy = (typeof state.owedJpyApprox === "number") ? state.owedJpyApprox : 0;
  const iapProduct = (kind === "registration")
    ? POSTPAID_REGISTRATION
    : (kind === "settlement" ? _settleProductForOwed(owedJpy) : null);

  // 法人サブスクは Web 専用 (native では modal に表示されないが二重ガード)
  if (kind === "corp_subscription" && (_inIosApp() || _hasAndroidBilling())) return;

  // --- IAP 分岐: iOS 本体 = StoreKit / Android TWA = Play Billing ---
  if (kind !== "corp_subscription" && _inIosApp()) {
    if (!_hasIosPurchase() || !iapProduct?.iosProductId) {
      toast(i18n_t("app.toast.update_app_for_purchase") || "購入にはアプリの更新が必要です", "err", 8000);
      return;
    }
    try {
      const { signedTransaction, transactionId } = await _iosStoreKitPurchase(iapProduct.iosProductId);
      await iapVerifyApple(signedTransaction);
      _iosFinishPurchase(transactionId);
      document.getElementById("purchase-pack-bg")?.classList.add("hidden");
      startBalancePolling(kind);
      toast(i18n_t("app.toast.purchase_ok") || "お支払いが完了しました", "ok", 8000);
    } catch (err) {
      console.error("StoreKit purchase failed:", err);
      toast(i18n_t("app.toast.purchase_failed", { reason: err.message ?? err }) || ("お支払いに失敗: " + (err.message ?? err)), "err", 8000);
    }
    return;
  }
  if (_hasAndroidBilling() && iapProduct?.androidProductId) {
    try {
      await _androidPlayPurchase(iapProduct);
      document.getElementById("purchase-pack-bg")?.classList.add("hidden");
      startBalancePolling(kind);
      toast(i18n_t("app.toast.purchase_ok") || "お支払いが完了しました", "ok", 8000);
    } catch (err) {
  ...
  try {
    const j = await checkoutSessionUI(kind, { locale: getStripeLocale(), currency: getStripeCurrency() });
    ...
    if (e?.code === "already_paid") {
      ...
      refreshHeader().catch(() => {});
      toast(i18n_t("app.toast.registration_already_paid"), "ok", 6000);
    } else if (e?.code === "below_minimum") {
      toast(i18n_t("app.purchase.settle_below_min_web"), "ok", 6000);
    } else {
      console.error("postpaid purchase failed:", e);
...
function startBalancePolling(goal = null) {
  ...
  try {
    import("/lib/vault-client.js?v=c6d50826").then(({ invalidateBalanceCache }) => {
  ...
  // Postpaid: 決済反映は goal ごとに観測する。
  //   registration → registrationPaid が true に変わる
  //   settlement   → owedUsdMicro が初期値より減る
  const initialOwed = (typeof state.owedUsdMicro === "number") ? state.owedUsdMicro : 0;
  const initialRegPaid = !!state.registrationPaid;
  // 法人サブスク申込 (2d-lite) は webhook の createCompany 完了 = admin 昇格を待つ
  const expectAdmin = (goal === "corp_subscription");
  ...
  const _stopped = () =>
    ...
    || (expectAdmin ? adminPromoted : creditsReflected);       // 目標達成
  const _stop = () => {
  ...
  const tick = async () => {
    ...
    let hadError = false;
    try {
      // --- 目標 A: 決済反映 (postpaid — 1 回だけ) ---
      //   registration: registrationPaid の flip を観測
      //   settlement:   owedUsdMicro の減少を観測
      if (!creditsReflected) {
        const s = await getBalanceUI({ force: true });
        const newOwed = (typeof s.owedUsdMicro === "number") ? s.owedUsdMicro : null;
        const regPaidNow = !!s.registrationPaid;
        let reflected = false;
        if (goal === "registration") {
          reflected = !initialRegPaid && regPaidNow;
        } else if (goal === "settlement") {
          reflected = (newOwed !== null && newOwed < initialOwed);
        } else {
          reflected = (!initialRegPaid && regPaidNow) || (newOwed !== null && newOwed < initialOwed);
        }
        if (reflected) {
          creditsReflected = true;
          if (typeof s.estimatedWrites === "number") state.credits = s.estimatedWrites;
          if (newOwed !== null) state.owedUsdMicro = newOwed;
          if (typeof s.owedJpyApprox === "number") state.owedJpyApprox = s.owedJpyApprox;
          state.registrationPaid = regPaidNow;
          state.canWrite = s.canWrite !== false;
          state.writeBlockedReason = s.writeBlockedReason ?? null;
          await refreshHeader();
          if (goal === "registration") {
            toast(i18n_t("app.toast.registration_reflected"), "ok", 8000);
          } else {
            const settledUsd = Math.max(0, initialOwed - (newOwed ?? 0)) / 1_000_000;
            const lang = i18n_getLang();
            const amountStr = (lang === "ja")
              ? `¥${Math.round(settledUsd * 154).toLocaleString("ja-JP")}`
              : `$${settledUsd.toFixed(2)}`;
            toast(i18n_t("app.toast.settlement_reflected", { amount: amountStr }), "ok", 8000);
          }
        }
      }
...
/**
 * Phase 7.x #428: 管理 tab nav の右側に pending 社員数 badge を表示。
 * admin 解錠時 + auto-distribute 後 + manual K1 配布後 などで refresh する。
 */
async function _updateAdminTabBadge() {
  const tab = document.getElementById("tab-admin");
  if (!tab || tab.classList.contains("hidden")) return;  // 非 admin or DOM 不在
  let count = 0;
  try {
    const _offboarded = _offboardedPkSet();
    const pending = (await listPendingEmployees())
    ...
    console.warn("[admin-tab-badge] listPendingEmployees failed:", e?.message);
    return;
  }
  // 既存 badge 削除 (= idempotent)
  ...
  if (count > 0) {
    ...
    badge.style.cssText = "display:inline-flex; align-items:center; justify-content:center; min-width:18px; height:18px; padding:0 5px; margin-left:6px; background:#DC2626; color:#fff; border-radius:9px; font-size:11px; font-weight:700; vertical-align:middle;";
    badge.title = `${count} ${i18n_t("app.admin.pending_badge_title") || "名の未配布社員"}`;
...
async function _onAdminK1Rotate() {
  ...
  if (!btn) return;
  if (!confirm("K1 を新 version に rotate します。 既存 K1 は履歴に保管され、 全 active 社員に新 K1 が自動で再配布されます。\n\n続行しますか？\n(rotate 後、 各社員は次回ログイン時に最新 K1 を取得します。 admin 自身の過去レコードは別途「再暗号化」が必要です)")) return;
  errEl?.classList.add("hidden");
  resEl?.classList.add("hidden");
  btn.disabled = true;
  ...
  try {
    ...
    if (resEl) {
      ...
      let _msg = `🔄 K1 を v${r.k1Version} に rotate しました (履歴サイズ: ${r.historySize})。`;
      if (r.distributedCount > 0) {
        _msg += ` ${r.distributedCount} 名の active 社員に新 K1 を再配布済。`;
      } else if (r.activeMemberCount === 0) {
        _msg += ` active 社員が居ないためローカルのみ更新 (社員加入時に最新 K1 が配布されます)。`;
      }
      if (r.skippedNoPubkey > 0) {
        _msg += ` ⚠️ ${r.skippedNoPubkey} 名は emp_pubkey 未登録で配布スキップ。`;
      }
...
async function _auditPushEvent(eventObj) {
  try {
    const { eciesEncrypt } = await import("/lib/vault-crypto.js?v=6bb1e228");
...
document.getElementById("admin-emergency-export-btn")?.addEventListener("click", async () => {
  ...
  try {
    ...
    // iOS 殻でも保存できるよう共通ヘルパー経由 (共有シート / download を自動振分)
    arpassSaveBlob(blob, `arpass-business-k1-${cid || "company"}-${Date.now()}.json`);
...
document.getElementById("admin-audit-refresh-btn")?.addEventListener("click", async () => {
  ...
  try {
    ...
    const { currentSigningPrivateKeyRaw } = await import("/lib/vault-client.js?v=c6d50826");
    const adminPrivKeyRaw = await currentSigningPrivateKeyRaw();
...
document.getElementById("admin-netpolicy-whoami-btn")?.addEventListener("click", async () => {
  ...
  try {
    ...
    if (resultEl) {
      // Phase 7.2-A UX fix: IPv6 は Privacy Extensions で下位 64 bit が定期的に変動するため、
      //   /128 (端末 1 つだけ) を suggest すると数日で動かなくなる。 /64 を suggest 推奨。
      const ipv4Comment = i18n_t("app.admin.netpolicy_whoami_cidr_comment_ipv4") || "# この端末 1 つだけ";
      const ipv6Comment = i18n_t("app.admin.netpolicy_whoami_cidr_comment_ipv6") || "# このネットワーク全体 (= 推奨)";
...
async function _renderAdminNetPolicy() {
  ...
  try {
    ...
    if (!info?.admin) {
      try {
        const { corpInfoUI } = await import("/lib/vault-client.js?v=c6d50826");
...
function renderRecordsList() {
  ...
  for (const r of sorted) {
    ...
    const typeLabel = i18n_t(`app.records.type_${r.type}`) || r.type;
    ...
    const costStr = formatRecordAttachmentCost(att);   // trial は「¥X (お試し中・請求されません)」
    ...
    card.innerHTML = `
      ...
      <div>
        ...
        <div style="font-size:12px; color:var(--muted); margin-top:2px;">
          ${escapeHtml(r.date)} · ${escapeHtml(typeLabel)}
...
/**
 * Postpaid (2026-07-08): attachment のコスト表示を一元化。
 *   - 実費 > 0            → 「¥4.2」
 *   - trial 参考額 > 0    → 「¥4.2 (お試し中・請求されません)」
 *   - 0 円 (sponsored 等) → 非表示 (「¥0 (無料)」の clutter 廃止 — ユーザー指摘)
 */
function formatRecordAttachmentCost(att) {
  if (!att) return "";
  if (typeof att.consumedUsdMicro === "number" && att.consumedUsdMicro > 0) {
    return formatRecordConsumedCost(att.consumedUsdMicro);
  }
  if (typeof att.referenceUsdMicro === "number" && att.referenceUsdMicro > 0) {
    const s = formatRecordConsumedCost(att.referenceUsdMicro);
    return s ? `${s} (${i18n_t("app.label.trial_usage_prefix")})` : "";
  }
  return "";
}

function formatRecordConsumedCost(usdMicro) {
  if (usdMicro == null || usdMicro === 0) return "";
  const lang = i18n_getLang();
...
document.getElementById("records-add-btn")?.addEventListener("click", () => {
  ...
  // 保存ボタンを canWrite 状態に同期 (未登録/上限なら「登録して利用開始」等に)
  syncSaveButtonToCredits();
...
document.getElementById("records-modal-save")?.addEventListener("click", async () => {
  ...
  // 購入 CTA モード (未登録 / 利用上限) → 最新状態を確認してから購入モーダルへ
  const recBtnCheck = document.getElementById("records-modal-save");
  if (recBtnCheck?.dataset.purchaseMode) {
    try { await refreshHeader(); } catch {}
    if (recBtnCheck.dataset.purchaseMode) {
      window.openPurchasePackModal?.();
      return;
  ...
  try {
    ...
    if (e?.code === "registration_required" || e?.code === "insufficient_credits" || e?.status === 402) {
      const errEl2 = document.getElementById("records-modal-error");
      if (errEl2) {
        _absorbPostpaidAccount(e?.account);   // 最新の owed/canWrite を state へ + header 更新
        errEl2.innerHTML = (e?.code === "registration_required")
          ? i18n_t("app.error.registration_required_cta", { cta: purchaseCtaHtml() })
          : i18n_t("app.error.insufficient_credits_cta", { cta: purchaseCtaHtml() });
        errEl2.classList.remove("hidden");
        syncSaveButtonToCredits();   // ボタンも「登録して利用開始」等に切替
      }
    } else if (_isStorageOutageError(e)) {
      // 2026-07-12: 保存先ストレージの一時障害。ファイルは <100KiB に縮小できないので
      //   縮小フォールバック対象外 = 正直に「一時障害・未保存・後で再試行」を伝える。
      showRecordsModalError(i18n_t("app.error.storage_outage"));
    } else {
...
async function openRecordDetail(recordId) {
  ...
  if (meta) {
    ...
    meta.innerHTML = `
      <div><strong>${escapeHtml(i18n_t("app.records.field_type"))}:</strong> ${escapeHtml(i18n_t(`app.records.type_${r.type}`) || r.type)}</div>
      ...
      <div style="margin-top:8px; padding:8px 10px; background:var(--paper); border-radius:6px; font-size:12px; color:var(--muted); border:1px solid var(--line);">
        <div style="display:flex; gap:14px; flex-wrap:wrap; align-items:center;">
          ${formatRecordAttachmentCost(r.attachments?.[0]) ? `<span style="color:var(--accent); font-weight:600;">💸 ${escapeHtml(i18n_t("app.records.field_cost") || "Cost")}: ${escapeHtml(formatRecordAttachmentCost(r.attachments[0]))}</span>` : ""}
  ...
  try {
    ...
    _currentDetailBlob = blob;            // 保存で fetch を挟まず直接 share するため保持
    _currentDetailFilename = safeFilename;

    if (previewEl) {
      // Download は <button> にして、クリック直後に同期で arpassSaveBlob を呼ぶ。
      //   (iOS の navigator.share は「操作直後に同期呼出」が必須。 fetch を挟むと
      //    権限が切れて共有シートが出ず「何も起こらない」状態になっていた。)
      const downloadBtnHtml = `
        <div style="display:flex; gap:8px; margin-bottom:12px; flex-wrap:wrap;">
          <button type="button" id="records-detail-save-btn" class="btn-outline"
             style="padding:8px 14px; display:inline-flex; align-items:center; gap:6px; cursor:pointer;">
      ...
      const _saveBtn = previewEl.querySelector("#records-detail-save-btn");
      if (_saveBtn) {
        _saveBtn.addEventListener("click", () => {
          // 同期呼出を保つため fetch を挟まず保持済み Blob を直接渡す
          arpassSaveBlob(_currentDetailBlob, _currentDetailFilename);
        });
      }
      try { if (typeof applyTranslations === "function") applyTranslations(previewEl); } catch {}
    }
  } catch (e) {
...
// Detail modal の blob URL / Blob 本体 / ファイル名を保持 (close 時に revoke, 保存で使用)
let _currentDetailBlobUrl = null;
let _currentDetailBlob = null;
let _currentDetailFilename = "download";
...
// =====================================================================
// Native/standalone shell の「新しいタブで開く」罠を回避 (2026-07-10)
// -----------------------------------------------------------------
// インストール済み PWA / iOS ネイティブ殻 (WKWebView) には「新しいタブ」が
// 無いため、blob: を target="_blank" で開くと単一 WebView がその blob に遷移
// し、CSV 等が小さな生テキストで全画面表示され戻れなくなる (閉じるボタン無し)。
// standalone のときは、こうしたクリックを同コンテナ内の Download リンクと同じ
// ダウンロードに変換する (ブラウザでは従来どおり新規タブ/DL のまま)。
// =====================================================================
// =====================================================================
// 全ダウンロードの共通保存ヘルパー (2026-07-10)
// -----------------------------------------------------------------
// iOS ネイティブ殻 (WKWebView) は <a download> を「無視」して blob へ画面遷移
// してしまう (CSV/JSON 等が全画面 raw 表示になり戻れない)。 iOS では Web Share
// API (共有シート →「ファイルに保存」) で保存する。 それ以外 (desktop / Android
// TWA / PWA) は従来どおり <a download>。 アプリ内の全ダウンロードはこの
// arpassSaveBlob() を通すこと (= 「ダウンロード系は全部同じ症状」への集約対策)。
// =====================================================================
function _isIOSNativeShell() {
  try {
    const mh = window.webkit && window.webkit.messageHandlers;
    return !!(mh && (mh.arpassPurchase || mh.arpassHwkey || mh.arpassAutofill));
  } catch { return false; }
}
// 2026-07-16: App Store 審査 2.3.10 対応。 iOS 殻の中では他プラットフォーム(Android/Windows)
//   への言及を消し、 対応端末表記を Apple 系のみに差し替える。 i18n 適用後 + 言語変更時に呼ぶ。
function _applyIOSShellDeviceText() {
  if (!_isIOSNativeShell()) return;
  try {
    const el = document.querySelector('[data-i18n-html="app.create.hint_devices"]');
    if (el && typeof i18n_t === "function") { const v = i18n_t("app.create.hint_devices_ios"); if (v && v !== "app.create.hint_devices_ios") el.innerHTML = v; }
  } catch (_) {}
  try {
    const el2 = document.querySelector('[data-i18n="app.create.unsupported_browser_desc"]');
    if (el2 && typeof i18n_t === "function") { const v = i18n_t("app.create.unsupported_browser_desc_ios"); if (v && v !== "app.create.unsupported_browser_desc_ios") el2.textContent = v; }
  } catch (_) {}
}
function _isAppShell() {
  if (_isIOSNativeShell()) return true;
  try {
    const mm = window.matchMedia;
    if (mm && (mm("(display-mode: standalone)").matches || mm("(display-mode: fullscreen)").matches)) return true;
  } catch {}
  return !!window.navigator.standalone;
}
// =====================================================================
// 2026-07-13: 「気に入ったら評価を」ナッジ (リンク版・審査不要)
//   ネイティブのアプリ内レビューシートは TWA から呼べないため、 ストアページを
//   開くリンク方式。 成功体験直後 (保存成功) にだけ、 戻ってきた満足層に限定して
//   最大 30 日に 1 回。 desktop / 通常ブラウザには出さない。 フォールバック中は出さない。
// ENABLED: ストア公開までは false。 公開確認後に true にして staging→main で解禁する
//   (未公開だと Play/App Store のリンクが 404 になるため、 それまで一切表示しない)
const _RATE = { ENABLED: false, KEY: "arpass.rate.v1", ANDROID_PKG: "io.arpass.twa", IOS_APP_ID: "6768276918", CAP_MS: 30 * 864e5, MIN_ENTRIES: 3 };
function _rateLoad() { try { return JSON.parse(localStorage.getItem(_RATE.KEY) || "{}"); } catch { return {}; } }
function _rateSave(o) { try { localStorage.setItem(_RATE.KEY, JSON.stringify(o)); } catch {} }
function _rateToday() { return new Date().toISOString().slice(0, 10); }
function _isAndroidTWA() { try { return String(document.referrer || "").startsWith("android-app://" + _RATE.ANDROID_PKG); } catch { return false; } }
function _ratePlatform() { if (_isIOSNativeShell()) return "ios"; if (_isAndroidTWA()) return "android"; return null; }
function _rateOpenStore(platform) {
  try {
    if (platform === "android") window.open("https://play.google.com/store/apps/details?id=" + _RATE.ANDROID_PKG, "_blank", "noopener");
    else window.open("https://apps.apple.com/app/id" + _RATE.IOS_APP_ID, "_blank", "noopener");
  } catch (_) {}
}
function _rateRemoveBanner() { try { document.getElementById("rate-nudge")?.remove(); } catch (_) {} }
function _showRateBanner(platform) {
  _rateRemoveBanner();
  const b = document.createElement("div");
  b.id = "rate-nudge";
  b.style.cssText = "position:fixed;left:50%;bottom:16px;transform:translateX(-50%);z-index:99999;max-width:92vw;width:420px;background:#fff;color:#111;border:1px solid #e5e7eb;border-radius:12px;box-shadow:0 8px 30px rgba(0,0,0,.18);padding:14px 16px;font:14px/1.5 system-ui,sans-serif";
  const msg = document.createElement("div");
  msg.textContent = i18n_t("app.rate.prompt") || "arpass を気に入っていただけましたか？ ストアでの評価が開発の大きな励みになります。";
  msg.style.marginBottom = "10px";
  const row = document.createElement("div");
  row.style.cssText = "display:flex;gap:8px;flex-wrap:wrap;justify-content:flex-end";
  const mk = (label, kind) => { const el = document.createElement("button"); el.type = "button"; el.textContent = label;
    el.style.cssText = "font:13px system-ui,sans-serif;padding:7px 12px;border-radius:8px;cursor:pointer;border:" + (kind === "cta" ? "0;background:#0a84ff;color:#fff" : "1px solid #d1d5db;background:#f9fafb;color:#374151"); return el; };
  const cta = mk(i18n_t("app.rate.cta") || "評価する", "cta");
  const later = mk(i18n_t("app.rate.later") || "後で", "sub");
  const never = mk(i18n_t("app.rate.never") || "今後表示しない", "sub");
  cta.onclick = () => { const st = _rateLoad(); st.state = "done"; _rateSave(st); _rateOpenStore(platform); _rateRemoveBanner(); };
  later.onclick = () => { _rateRemoveBanner(); };  // snoozeUntil は表示時に既に 30 日設定済み
  never.onclick = () => { const st = _rateLoad(); st.state = "never"; _rateSave(st); _rateRemoveBanner(); };
  row.appendChild(never); row.appendChild(later); row.appendChild(cta);
  b.appendChild(msg); b.appendChild(row);
  (document.body || document.documentElement).appendChild(b);
}
function maybeShowRateNudge() {
  if (!_RATE.ENABLED) return;                              // ストア公開まで無効
  const platform = _ratePlatform();
  if (!platform) return;                                   // desktop / 通常ブラウザ → 出さない
  const st = _rateLoad();
  if (st.state === "never" || st.state === "done") return; // 既に評価 or 拒否
  const now = Date.now();
  if (st.snoozeUntil && now < st.snoozeUntil) return;      // スヌーズ中
  if (st.shownAt && now - st.shownAt < _RATE.CAP_MS) return; // 30 日上限
  if (!st.firstSeenDay) { st.firstSeenDay = _rateToday(); _rateSave(st); return; } // 初日は記録のみ
  if (st.firstSeenDay === _rateToday()) return;            // 戻りセッション ゲート (別日に再訪した層に限定)
  const entries = state.vault?.entries?.length ?? 0;
  if (entries < _RATE.MIN_ENTRIES) return;                 // 価値を体験した層に限定
  try { if (typeof currentRestoredFromHistory === "function" && currentRestoredFromHistory()) return; } catch {} // フォールバック中は出さない
  _showRateBanner(platform);
  st.shownAt = now; st.snoozeUntil = now + _RATE.CAP_MS; _rateSave(st);
}
function _downloadBlobViaAnchor(blob, filename) {
  const url = URL.createObjectURL(blob);
  const a = document.createElement("a");
  a.href = url;
  a.download = filename;
  a.style.display = "none";
  a.setAttribute("data-arpass-native-dl", "1"); // interceptor 再入防止マーク
  document.body.appendChild(a);
  a.click();
  setTimeout(() => { try { a.remove(); } catch {} try { URL.revokeObjectURL(url); } catch {} }, 3000);
}
// アプリ内のあらゆる「保存/ダウンロード」はこれを通す。
async function arpassSaveBlob(blob, filename) {
  const name = filename || "download";
  if (_isIOSNativeShell()) {
    // iOS WKWebView: Web Share API (共有シート →「ファイルに保存」)。
    //   ※ navigator.share は「ユーザー操作直後に同期呼出」が必須なので、
    //     呼出元は fetch 等の await を挟まず in-memory Blob を渡すこと。
    try {
      const file = new File([blob], name, { type: (blob && blob.type) || "application/octet-stream" });
      if (navigator.canShare && navigator.canShare({ files: [file] })) {
        await navigator.share({ files: [file], title: name });
        return;
      }
      // canShare 非対応: この WKWebView は Web Share (files) 未対応。
      //   純 Web では保存手段が無いため、 ネイティブ保存ブリッジが必要な旨を通知。
      try { toast(i18n_t("app.records.save_unsupported") || "この端末では共有シートからの保存に未対応です。アプリの更新をお待ちください。", "warn", 6000); } catch {}
      return;
    } catch (e) {
      if (e && e.name === "AbortError") return; // ユーザーがシートを閉じた
      // NotAllowedError 等 → 通知して終了 (download にしても iOS では表示になる)
      try { toast(i18n_t("app.records.save_failed") || "保存に失敗しました。もう一度お試しください。", "warn", 5000); } catch {}
      return;
    }
  }
  _downloadBlobViaAnchor(blob, name);
}
// rendered な blob: アンカー (records-detail の Download / 新しいタブで開く、
//   ファイル一覧の Download 等) を アプリ殻で横取りして arpassSaveBlob へ回す。
//   自前で発行した download (data-arpass-native-dl) は素通しして再入を防ぐ。
(function () {
  document.addEventListener("click", function (ev) {
    if (!_isAppShell()) return;
    const t = ev.target;
    const a = t && t.closest && t.closest('a[href^="blob:"]');
    if (!a) return;
    if (a.getAttribute("data-arpass-native-dl") === "1") return;
    ev.preventDefault();
    ev.stopPropagation();
    const container = a.closest("#records-detail-preview, #records-file-preview") || document;
    const dlSibling = container.querySelector("a[download]");
    const name = a.getAttribute("download")
      || (dlSibling && dlSibling.getAttribute("download"))
      || "download";
    (async () => {
      try {
        const resp = await fetch(a.href);
        const blob = await resp.blob();
        await arpassSaveBlob(blob, name);
      } catch (e) { /* noop */ }
    })();
  }, true); // capture phase: 他 handler より先に横取り
})();
...
function _openHistoryModal() {
  ...
  if (entries.length === 0) {
    ...
    const r = (state.vault ? getCurrentRecords(state.vault) : []).find(x => x.id === _currentDetailRecordId);
    const createdCostStr = formatRecordAttachmentCost(r?.attachments?.[0]);
    ...
    listEl.innerHTML = entries.map(e => {
      ...
      const cost = (e.action === "create" && createdCostStr)
        ? `<div style="margin-top:4px; font-size:11px;"><span style="color:var(--accent); font-weight:600;">💸 ${escapeHtml(i18n_t("app.records.field_cost") || "Cost")}: ${escapeHtml(createdCostStr)}</span>${createdSize ? ` <span style="color:var(--muted);"> · 📎 ${escapeHtml(formatFileSize(createdSize))}</span>` : ""}</div>`
        : "";
...
function _exportRecordsCsv() {
  ...
  // iOS 殻でも保存できるよう共通ヘルパー経由 (共有シート / download を自動振分)
  arpassSaveBlob(blob, `arpass-files-${today}.csv`);
}
...
// ── arpass ブラウザ拡張ブリッジ (Phase3 Z: デスクトップ拡張 ⇄ 本体 web) ──────────
//   拡張の content script が arpass.io タブ内で window.postMessage({__arpass:"req",...})
//   を送り、解錠済みセッションが応答する。ロック中は {locked:true}。
//   受理は「同一オリジン かつ ev.source===window」のみ → 他サイト/iframe 親からは到達不可。
//   通常利用では何も起きない (タグ付き要求が来た時だけ反応)。
(function initArpassExtBridge() {
  if (typeof window === "undefined" || window.__arpassExtBridgeInit) return;
  window.__arpassExtBridgeInit = true;
  const hostOf = (s) => { try { return new URL(String(s).includes("://") ? s : "https://" + s).host.toLowerCase(); } catch { return ""; } };
  const matchHost = (e, host) => {
    if (!host) return false;
    const cands = [e.url, e.site].map(hostOf).filter(Boolean);
    return cands.some((h) => h === host || host.endsWith("." + h) || h.endsWith("." + host));
  };
  window.addEventListener("message", async (ev) => {
    if (ev.source !== window || ev.origin !== location.origin) return;
    const d = ev.data;
    if (!d || d.__arpass !== "req" || !d.reqId) return;
    const reply = (p) => window.postMessage(Object.assign({ __arpass: "res", reqId: d.reqId }, p), location.origin);
    try {
      if (!state.vault) { reply({ locked: true }); return; }
      if (d.action === "status") { reply({ ok: true, unlocked: true }); return; }
      if (d.action === "getCredentials") {
        // 一覧は pw を含めない (平文パスワードを拡張側に滞留させない)。pw は getPassword で都度取得。
        const host = String(d.host || "").toLowerCase();
        const entries = (state.vault.entries || [])
          .filter((e) => matchHost(e, host))
          .map((e) => ({ id: e.id, user: e.user || "", site: e.site || "", url: e.url || "" }));
        reply({ ok: true, entries });
        return;
      }
      if (d.action === "getPassword") {
        // 選択された1件の pw のみ返す (just-in-time)。
        const id = d.payload && d.payload.id;
        const e = (state.vault.entries || []).find((x) => x.id === id);
        reply({ ok: !!e, pw: e ? (e.pw || "") : "" });
        return;
      }
      if (d.action === "saveCredential") {
        const p = d.payload || {};
        const now = Date.now();
        const host = hostOf(p.url || "");
        state.vault.entries = state.vault.entries || [];
        const idx = host ? state.vault.entries.findIndex(
          (e) => (e.user || "") === (p.user || "") && hostOf(e.url || e.site || "") === host) : -1;
        if (idx >= 0) state.vault.entries[idx] = { ...state.vault.entries[idx], pw: p.pw || "", updatedAt: now };
        else state.vault.entries.push({ id: newEntryId(), site: host, url: p.url || "", user: p.user || "", pw: p.pw || "", notes: "", createdAt: now, updatedAt: now });
        await saveVault(state.vault);
        // 拡張からの保存でも本体 UI を最新化 (一覧再描画 + 保存バッジ)
        try { renderList(); updateSaveStatusBadge("saved", { txid: currentLatestTxId() }); } catch (e) {}
        reply({ ok: true, updated: idx >= 0 });
        return;
      }
      reply({ error: "unknown action" });
    } catch (e) { reply({ error: String((e && e.message) || e) }); }
  });
  try { console.log("[arpass] ext bridge ready"); } catch {}
})();

// ── アイドル自動ロック (放置時に in-memory secret を破棄) ──────────────────────
//   一定時間 無操作で lockSession()+reload (手動ロックと同じ後処理)。操作で都度リセット。
//   解錠中のみ作動。拡張(Phase3)が掴む解錠タブを放置しても自動的にロックされる。
(function initIdleAutoLock() {
  if (typeof window === "undefined" || window.__arpassIdleLockInit) return;
  window.__arpassIdleLockInit = true;
  const IDLE_MS = 10 * 60 * 1000; // 10 分
  let timer = null;
  async function lockNow() {
    if (!state.vault) return; // 未解錠/既ロックなら何もしない
    try {
      const st = (typeof getSaveStatus === "function") ? getSaveStatus() : null;
      if (st && (st.state === "dirty" || st.state === "saving")) { await flushSaveDebounce(); }
    } catch (_) {}
    try { clearSavePending(); } catch (_) {}
    try { lockSession(); } catch (_) {}
    try { console.log("[arpass] idle auto-lock"); } catch (_) {}
    try { location.reload(); } catch (_) {}
  }
  function reset() { if (timer) clearTimeout(timer); timer = setTimeout(lockNow, IDLE_MS); }
