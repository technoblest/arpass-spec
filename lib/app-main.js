...
} from "/lib/vault-client.js?v=6ec16a66";
import {
  generatePassword,
  passwordStrength,
  isPasskeySupported,
...
} from "/lib/client-auth.js?v=20bd67e9";
import {
  initI18n,
  mountLangPicker,
  t as i18n_t,
...
async function removeAuthorizedDevice(id) {
  ...
  const { removeCredentialFromEnvelopeUI } = await import("/lib/vault-client.js?v=6ec16a66");
    ...
    while (Date.now() - startedAt < TIMEOUT_MS && !cancelled) {
      ...
      try {
        ...
        if (grant) {
          ...
          const { eciesDecrypt } = await import("/lib/vault-crypto.js?v=67358613");
...
function startBalancePolling(packKey = null) {
  ...
  try {
    import("/lib/vault-client.js?v=6ec16a66").then(({ invalidateBalanceCache }) => {
...
async function _auditPushEvent(eventObj) {
  try {
    const { eciesEncrypt } = await import("/lib/vault-crypto.js?v=67358613");
...
document.getElementById("admin-audit-refresh-btn")?.addEventListener("click", async () => {
  ...
  try {
    ...
    const { currentSigningPrivateKeyRaw } = await import("/lib/vault-client.js?v=6ec16a66");
    const adminPrivKeyRaw = await currentSigningPrivateKeyRaw();
...
async function _renderAdminNetPolicy() {
  ...
  try {
    ...
    if (!info?.admin) {
      try {
        const { corpInfoUI } = await import("/lib/vault-client.js?v=6ec16a66");
