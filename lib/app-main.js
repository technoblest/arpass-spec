...
import { enableEmergencyPad } from "/lib/vault-client.js?v=adb3cd17";
import {
  generatePassword,
  passwordStrength,
  isPasskeySupported,
...
} from "/lib/client-auth.js?v=5d3f1f4f";
import {
  initI18n,
  mountLangPicker,
  t as i18n_t,
...
async function removeAuthorizedDevice(id) {
  ...
  const { removeCredentialFromEnvelopeUI } = await import("/lib/vault-client.js?v=adb3cd17");
...
function startBalancePolling(goal = null) {
  ...
  try {
    import("/lib/vault-client.js?v=adb3cd17").then(({ invalidateBalanceCache }) => {
...
document.getElementById("admin-audit-refresh-btn")?.addEventListener("click", async () => {
  ...
  try {
    ...
    const { currentSigningPrivateKeyRaw } = await import("/lib/vault-client.js?v=adb3cd17");
    const adminPrivKeyRaw = await currentSigningPrivateKeyRaw();
...
async function _renderAdminNetPolicy() {
  ...
  try {
    ...
    if (!info?.admin) {
      try {
        const { corpInfoUI } = await import("/lib/vault-client.js?v=adb3cd17");
