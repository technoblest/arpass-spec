...
export async function setRecoverySecretInSession(recoveryString) {
  ...
  if (!_session.rMatHkdfKey) {
    ...
    try {
      ...
      console.warn("[setRecoverySecretInSession] rMatHkdfKey derive failed, raw fallback:", e?.message);
      _session.recoveryMaterial = new Uint8Array(rMatRaw);  // fallback で raw 保持
    }
    rMatRaw.fill(0);
  }
}
...
/** メモリ上の秘密を全消去。localStorage は残す (outerKey 等のキャッシュ)。 */
...
async function _extractK2FromBusinessEnvelope(envelope, factors) {
  ...
  throw new Error("K2 extract failed: no wrap opened");
}
...
export async function refreshTierQualifier() {
  ...
  if (_session.rMatHkdfKey) {
    try {
      const { deriveAppNameTagFromHkdf } = await import("./vault-crypto.js");
...
export async function createVault(password, userDisplayName, captchaToken = null, opts = {}) {
  ...
  // 2. Passkey user.id (WebAuthn 内部識別子) — appNameTag.value (rMat 派生 22 文字) を流用
  const userId = appNameTag.value;
  // 3. Passkey 作成 → PRF
  ...
  if (mode === "business" && enc.signingKey) {
    const { importSigningKeyPair: _isk } = await import("./vault-crypto.js");
  ...
  const writeResult = await writeEnvelope(
    ...
    { tier: businessK2 ? "corp" : "free" },  // Phase 7.2-B v2.6: 初回 write の tier 申告
  );
  // 7. localStorage メタ (Phase 7.0w-AR: vault-id 廃止 → outerKey 直接保存)
  const publicKeyHash = await hashPublicKey(signingState.publicKeyRaw);
  writeMeta({
    outerKey: b64uEncode(enc.outerKeyBytes),
...
export async function unlockWithPasswordAndPasskey(password, options = {}) {
  const meta = readMeta();
  if (!meta?.outerKey || !meta?.appNameTag) {
    throw new Error("この端末で初めて利用する場合は「すでにアカウントがある」から復元してください");
  }
  const outerKeyBytes = b64uDecode(meta.outerKey);
  ...
  const { envelope } = await fetchEnvelope(txid, outerKeyBytes);
...
export async function unlockWithPasswordAndRecovery(password, recoveryString, opts = {}) {
  ...
  const appNameTag = deriveAppNameTag(recoveryMaterial);  // legacy {name,value}
  ...
  const tagList = [tags.legacy, tags.free, tags.paid, tags.private];
  ...
  if (!envelope) {
    ...
    if (metaTxId) {
      try {
        const r = await fetchEnvelope(metaTxId, outerKeyBytes);
        if (r?.envelope) {
          txid = metaTxId;
          matchedAppName = readMeta()?.currentAppNameTag || readMeta()?.appNameTag || tags.legacy;
          envelope = r.envelope;
  ...
  if (!envelope) {
    ...
    for (const cand of candidates) {
      try {
        const r = await fetchEnvelope(cand.txid, outerKeyBytes);
  ...
  writeMeta({
    outerKey: b64uEncode(outerKeyBytes),
...
export async function unlockWithPasskeyAndRecovery(recoveryString, options = {}) {
  ...
  writeMeta({
    outerKey: b64uEncode(outerKeyBytes), appNameTag,
    currentAppNameTag: matchedAppName,
...
export async function saveVault(updatedVault, opts = {}) {
  ...
  const newEnvelope = {
    ..._session.lastEnvelope,  // m, cid, kv, ws, wsa などすべて保持
    v: VAULT_FORMAT_V5,
  ...
  const writeResult = await writeEnvelope(
    newEnvelope, (_session.outerKey ?? _session.outerKeyBytes), writeAppName, _session.signingState,
    {
...
export async function addCredentialOnThisDevice(password, userDisplayName, opts = {}) {
  ...
  // Phase 7.0w-AR: userId は appNameTag.value (rMat 派生 22 文字) を流用
  const userId = _session.appNameTag.value;
  const { credentialId, prfOutput } = await createPasskey(userId, userDisplayName);
  const credIdHash = await credentialIdToHash(credentialId);
...
export async function changePasswordUI(newPassword, recoveryString) {
  ...
  try {
    updatedEnv = await changePassword(
      _session.lastEnvelope, mekForReWrap, _session.currentCredIdHash,
  ...
  const writeResult = await writeEnvelope(
    updatedEnv, (_session.outerKey ?? _session.outerKeyBytes), _session.appNameTag, _session.signingState,
...
export async function reissueRecovery_caseA(password) {
  ...
  _recordWrittenTxId(writeResult.txid);  // Phase 7.1-AI
  writeMeta({
    outerKey: b64uEncode(result.newOuterKeyBytes),
...
export async function addRecordUI(input) {
  ...
  // Phase 7.3-A.5: BEK は CryptoKey (extractable=true for wrap)、 raw bytes が JS heap に出ない
  const bek = await generateBlobKey({ forWrapping: true });
  const { ciphertext, iv: dataIv } = await encryptBlob(bek, fileBytes);

  // 3) BEK を MEK で wrap (CryptoKey BEK → wrapped bytes)
  const { wrapped: wrappedBEK, iv: wrapIv } = await wrapKey(mek, bek);
  // bek は CryptoKey、 GC 任せ (= fill(0) 不可)
...
export async function fetchRecordFileUI(recordId, attachmentIdx = 0) {
  ...
  // 2) BEK を MEK で unwrap
  const wrappedBEK = b64uDecode(att.encryption.wrappedBEK);
  const wrapIv = b64uDecode(att.encryption.wrapIv);  // v2.5 hotfix 5: 復元漏れ
  const blobWrapKey = await _getMekKeyForVersion(att.encryption.k1Version);  // Phase 7.3-A.7c: CryptoKey 返却
  // Phase 7.3-A.5: bek は CryptoKey (non-extractable)
  const bek = await unwrapKey(blobWrapKey, wrappedBEK, wrapIv);

  // 3) BEK で decrypt
  const dataIv = b64uDecode(att.encryption.dataIv);
  const bytes = await decryptBlob(bek, ciphertext, dataIv);
  // Phase 7.3-A.5: bek は CryptoKey なので fill(0) 不要 (= non-extractable)
...
export async function sealOldestChunkUI({ threshold = DEFAULT_CHUNK_THRESHOLD, keepRecent = DEFAULT_CHUNK_KEEP } = {}) {
  ...
  // 2) CEK 生成 → encrypt
  // Phase 7.3-A.5: CEK は CryptoKey
  const cek = await generateBlobKey({ forWrapping: true });
  const { ciphertext, iv: dataIv } = await encryptBlob(cek, chunkBytes);

  // 3) CEK を MEK で wrap
  // Phase 7.2-B (α) fix: Business mode は K2 で CEK wrap
  const blobWrapKey2 = _session.mek;  // Phase 7.2-B v2 #99
  const { wrapped: wrappedCEK, iv: wrapIv } = await wrapKey(blobWrapKey2, cek);
  // Phase 7.3-A.5: cek は CryptoKey
...
export async function loadChunkUI(chunkId) {
  ...
  const blobUnwrapKey = await _getMekKeyForVersion(chunk.encryption.k1Version);  // Phase 7.3-A.7c: CryptoKey 返却
  // Phase 7.3-A.5: cek は CryptoKey
  const cek = await unwrapKey(blobUnwrapKey, wrappedCEK, wrapIv);

  // 3) decrypt
  const dataIv = b64uDecode(chunk.encryption.dataIv);
  const bytes = await decryptBlob(cek, ciphertext, dataIv);
  // Phase 7.3-A.5: cek は CryptoKey
...
export async function refreshFromServerLatest() {
  ...
  for (let i = 0; i < delays.length; i++) {
    ...
    try {
      const r = await fetchEnvelope(serverTxId, (_session.outerKey ?? _session.outerKeyBytes));
...
/**
 * Phase 7.2-B (α): admin が自社の corpKeypair を保持しているか確認、 未保持なら生成 + upload + vault escrow。
 *
 * 呼び出し条件: _session が admin mode、 vault.companyId が判明。
 * 副作用: vault.serverPrivateKeys[cid][version] に privateKeyJwk を保存し saveVault される。
 *
 * @returns {Promise<{ ensured: boolean, version?: number }>}
 */
...
export async function corpAdminSetIpPolicyUI({ ipAllowlist, blockPersonalFromOurNetwork, autoRotateOnOffboard }) {
  if (!_session) throw new Error("locked");
  if (!isAdminMode()) throw new Error("not admin mode");
  const body = {};
...
export async function currentSigningPrivateKeyRaw() {
  if (!_session) return null;
  // signingState.signingPrivateKey は CryptoKey (extractable: false で import 済) → raw 取り出し不可
  // 代わりに deriveSigningKey の戻り値 privateKeyJwk.d を MEK から再 derive
  const { deriveSigningKey } = await import("./vault-crypto.js");
  const sk = deriveSigningKey(_session.mek);
...
export async function removeCredentialFromEnvelopeUI(credIdHash) {
  ...
  const writeResult = await writeEnvelope(
    newEnvelope, (_session.outerKey ?? _session.outerKeyBytes), _session.appNameTag, _session.signingState,
    { tier: _resolveCurrentTier() }
  );
  _session.lastEnvelope = newEnvelope;
  _session.latestTxId = writeResult.txid;
 ...
 */
export async function distributeK1ToAllPending() {
  if (!_session?.signingState) throw new Error("locked — unlock first");
...
export async function distributeK1ToAllPendingSafe() {
  if (!_session) throw new Error("locked — unlock first");
  if (_session.vault?.mode !== "admin") throw new Error("admin mode のみで実行可能");
  ...
  return distributeK1ToAllPending();
}
