...
import {
  ...
  unpadPlaintext,
  VAULT_FORMAT_V5,
} from "/lib/vault-crypto.js?v=67358613";
...
export async function refreshTierQualifier() {
  ...
  if (_session.rMatHkdfKey) {
    try {
      const { deriveAppNameTagFromHkdf } = await import("/lib/vault-crypto.js?v=67358613");
...
export async function createVault(password, userDisplayName, captchaToken = null, opts = {}) {
  ...
  const { generateRecoverySecret } = await import("/lib/vault-crypto.js?v=67358613");
  ...
  if (mode === "business" && enc.signingKey) {
    const { importSigningKeyPair: _isk } = await import("/lib/vault-crypto.js?v=67358613");
...
export async function currentSigningPrivateKeyRaw() {
  ...
  const { deriveSigningKeyFromHkdf } = await import("/lib/vault-crypto.js?v=67358613");
