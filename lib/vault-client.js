...
import {
  ...
  deriveSigningKey,
  deriveSigningKeyFromHkdf,            // Stage2 B incr2: K2 handle → signing (admin pk 不変)
  importSigningKeyHandleFromSeed,      // 署名鍵 handle一本化: seed → SigningKey opaque handle
  unwrapEmpPrivAsHandleWithK2Handle,   // Stage2 B incr2: K2 handle → EmpPrivKey handle
  eciesUnwrapToK1Handle,               // Stage2 B incr2: ECIES → K1Key handle (生 K1 不在)
  deriveOuterKeyBytes,
  encodeUserIdV7,
  encodeUserIdV8,
  decodeUserIdV7,
...
// 2026-07-12: 緊急パディングの singleton を app-main へ (ES module identity 罠回避のため
//   vault-client 経由で公開。re-export 構文はローカル束縛を作らないため import + export)
import { enableEmergencyPad, isEmergencyPadActive } from "/lib/vault-crypto.js?v=6bb1e228";
export { enableEmergencyPad, isEmergencyPadActive };
...
import {
  ...
  fetchKeyslotBlob,
  lookupKeyslotTxid,
} from "/lib/client-auth.js?v=5d3f1f4f";  // sec-2026-07: 絶対パス化で cache-bust(?v=)対象にし、SW キャッシュに旧署名が残る罠を解消（app-main と同一モジュール実体に統一）
...
export async function getDecryptedRecoveryFromVault() {
  ...
  // 修正(recovery K1非依存化): encryptedRecovery は K2 由来鍵で保護される。
  //   business は K2 (k2Handle/k2HkdfKey)、 personal は mek(=K2) で復号。
  //   (旧: business で mek=HKDF(K1,K2) を使い、 transition/rotation で K1 が変わると tag mismatch で
  //    再表示不能だった。)
  const _isBiz = (_session.lastEnvelope?.m === "business") || !!_session.k2Handle;
  const mekArg = _isBiz
    ? _session.k2Handle
    : (_session.mekKey ?? _session.mekHkdfKey ?? _session.mek);
  if (!mekArg) throw new Error("session に Recovery 保護鍵 (K2/mek) が無い");
...
export async function setRecoverySecretInSession(recoveryString) {
  ...
  if (!_session.rmat) {
    ...
    try {
      // appNameTag/outer は下の RMatKey handle で派生する (CryptoKey は廃止)。
...
export function currentRestoredFromHistory() { return _session?.restoredFromHistory ?? null; }
...
// ── 2026-07-13: history フォールバック セッションの「最新を再試行」を賢くする ──
//   従来の retry ボタンは location.reload() だけで、 この端末の wrap が最新版に
//   無い場合は「reload→ロック→解錠→また同じフォールバック」の無限ループになった。
//   ここで「本当の最新」を取得し outer 復号して、 この端末の wrap 有無を判定する:
//     ・wrap あり  → 一時的な取得失敗だった (= Turbo 伝播待ち等)。 reload で解錠可能。
//     ・wrap なし  → reload しても永久に開けない。 Master + Recovery 再登録を案内。
//   ※ 暗号化 blob の中身 (エントリ本文) は復号しない。 wrap ハッシュ (w.b[].h / w.c[].h)
//     は outer 層だけで読めるため、 この判定に Master/Passkey は不要。
function _wrapHashSet(env) {
  const s = new Set();
  try { (env?.w?.b || []).forEach((w) => { if (w && w.h) s.add(w.h); }); } catch (_) {}
  try { (env?.w?.c || []).forEach((w) => { if (w && w.h) s.add(w.h); }); } catch (_) {}
  return s;
}

export async function retryLatestForRestoredSession() {
  if (!_session) return { status: "locked" };
  const restored = _session.restoredFromHistory;
  if (!restored) {
    // フォールバック中でない → 通常の refresh を試みる
    try {
      const r = await refreshFromServerLatest();
      return { status: r?.refreshed ? "refreshed" : "nochange" };
    } catch (e) { return { status: "fetch_failed", message: e?.message }; }
  }
  const latestTxId = restored.latestTxid;
  let envelope = null;
  try {
    const r = await fetchEnvelope(latestTxId, (_session.outerKeyHandle ?? _session.outerKey ?? _session.outerKeyBytes));
    envelope = r?.envelope;
  } catch (e) {
    // outer 復号すら失敗 = 別 outerKey を指す cross-talk か、 まだ伝播していない。
    return { status: "fetch_failed", latestTxId, message: e?.message };
  }
  if (!envelope) return { status: "fetch_failed", latestTxId };
  const present = _wrapHashSet(envelope).has(_session.currentCredIdHash);
  // wrap あり → 今なら開ける (reload で再解錠)。 wrap なし → ループ回避、 復旧導線へ。
  return present ? { status: "retryable", latestTxId } : { status: "device_missing", latestTxId };
}
...
export function lockSession() {
  if (_session) {
    ...
    _session.mek = null;
    // Phase 2-H2-b: mekKey が MekKey handle ならば free() 必須 (= WASM heap 解放)
    if (_session.mekKey && typeof _session.mekKey.free === "function") {
      try { _session.mekKey.free(); } catch (_) { /* swallow */ }
    }
    _session.mekKey = null;        // Phase 7.3-A.7b: CryptoKey は GC 任せ、 handle は上で free 済
    // Phase 2-H4-full F4: k2Handle (Rust MekKey handle) も free
    if (_session.k2Handle && typeof _session.k2Handle.free === "function") {
      try { _session.k2Handle.free(); } catch (_) { /* swallow */ }
    }
    _session.k2Handle = null;
    // Phase 2-H4-full F7-C step 1: mekHkdfKey が MekKey opaque handle の場合 free
    //   (= refreshFromServerLatest 経由で handle が入る可能性。 CryptoKey の場合は no-op)
    if (_session.mekHkdfKey && typeof _session.mekHkdfKey.free === "function") {
      try { _session.mekHkdfKey.free(); } catch (_) { /* swallow */ }
    }
    ...
    // Phase 2-F8 step 3a: EmpPrivKey handle の free
    if (_session.empPrivHandle && typeof _session.empPrivHandle.free === "function") {
      try { _session.empPrivHandle.free(); } catch (_) {}
    }
    _session.empPrivHandle = null;
    ...
    if (_session._oldRealMekCache) {
      for (const m of _session._oldRealMekCache.values()) {
        if (m && typeof m.free === "function") { try { m.free(); } catch (_) {} }
        else if (m && typeof m.fill === "function") m.fill(0);
      }
...
async function buildSigningState(mek) {
  ...
  // fallback 撤去: SigningKey opaque handle 必須 (CryptoKey/importSigningKeyPair 廃止)。
  if (!sk.handle || typeof sk.handle.ecdsa_sign !== "function")
    throw new Error("buildSigningState: SigningKey handle 必須 (Rust)");
  return {
    signingPrivateKey: sk.handle,
...
// Stage2 Business Phase B incr2: K2 MekKey opaque handle から signing state を構築。
//   deriveSigningKeyFromHkdf(k2Handle) → SigningKey opaque handle。 publicKeyRaw は raw 経路と
//   bit-identical (Rust test test_mek_derive_signing_key_equivalent_to_explicit_hkdf_path 担保)
//   = admin pk 不変 (F7 トラップ回避)。 buildSigningState(mek) の handle 入力版。
async function buildSigningStateFromHandle(k2Handle) {
  const sk = await deriveSigningKeyFromHkdf(k2Handle);  // {privateKeyRaw, publicKeyRaw, handle}
  // fallback 撤去: SigningKey opaque handle 必須。
  if (!sk.handle || typeof sk.handle.ecdsa_sign !== "function")
    throw new Error("buildSigningStateFromHandle: SigningKey handle 必須 (Rust)");
  return {
    signingPrivateKey: sk.handle,
    signingPublicKey: null,
    publicKeyRaw: sk.publicKeyRaw,
    publicKeyJwk: sk.publicKeyJwk ?? null,
  };
}
...
export async function generateEphemeralSigningState() {
  // 署名鍵 handle一本化: ephemeral 署名鍵を Rust SigningKey opaque handle で生成 (CryptoKey 不使用)。
  //   signingPrivateKey = SigningKey handle (signRequest が ecdsa_sign で署名)。
  //   eciesPrivateRaw = handle.private_key_raw() (= 32-byte scalar、 admin ECIES payload 復号用、
  //   publicKeyRaw と同一 keypair なので ECDH 整合)。 Rust 未ロード時のみ WebCrypto fallback。
  const seed = crypto.getRandomValues(new Uint8Array(48));  // ランダム seed → ephemeral keypair
  const handle = importSigningKeyHandleFromSeed(seed);     // SigningKey opaque handle or null
  seed.fill(0);
  if (!handle) {
    throw new Error("generateEphemeralSigningState: Rust crypto core (SigningKey) 必須");
  }
  const publicKeyRaw = new Uint8Array(handle.public_key_raw());   // 65-byte uncompressed
  const eciesPrivateRaw = new Uint8Array(handle.private_key_raw()); // 32-byte scalar
  return { signingPrivateKey: handle, publicKeyRaw, eciesPrivateRaw };
}
...
async function decryptVaultAuto(envelope, factors) {
  ...
  if (envelope?.m === "business") {
    ...
    // (1) K2 を取り出す。 Stage2 B incr2: handle 優先 (= 生 K2 を JS heap に出さない)、
    //   WASM/handle 不在時のみ raw fallback。 以後 emp_priv / signing / K1 を handle 経由で導出。
    const k2Handle = await _extractK2HandleFromBusinessEnvelope(envelope, factors);
    if (!k2Handle) throw new Error("decryptVaultAuto(business): K2 handle 必須 (Rust crypto core)");

    // (2) emp_priv を K2 handle で復号 (= 生 K2 不在、 CryptoKey 不使用)
    let empPrivHandle = null;
    try {
      empPrivHandle = await unwrapEmpPrivAsHandleWithK2Handle(k2Handle, envelope.w_emp);
    } catch (e) {
      throw new Error(`w_emp unwrap (handle) failed: ${e?.message}`);
    }
    if (!empPrivHandle) throw new Error("emp_priv handle could not be derived from k2 handle");

    // (3) K2 由来の signing state を一時構築 (admin pk 不変 = F7 トラップ回避)
    const tempSigning = await buildSigningStateFromHandle(k2Handle);
    ...
    if (envelope.k1Pending) {
      ...
      return {
        ...result,
        empPrivKey: result.empPrivKey ?? null,
        empPrivHandle: empPrivHandle ?? result.empPrivHandle ?? null,
        k1Version: null,
        k1Pending: true,
      };
    }
    ...
    // (5)+(6) ECIES unwrap → K1 → body 復号。
    //   #3 (K1 version 対応): 現行 K1 で本体が開けない場合 (= K1 rotation で server version が
    //   進み、 本体は旧 version の mek で暗号化されている) は、 envelope.k1Version (legacy で
    //   k1Version 不在なら current-1..1) の K1 を deprecated store から fetch して再試行する。
    //   各試行は ECIES unwrap (handle 優先 / raw fallback) → decryptVaultBusiness。
    const _attemptUnwrapDecrypt = async (encK1Blob) => {
      const _k1Handle = await eciesUnwrapToK1Handle(empPrivHandle, encK1Blob);
      try {
        return await decryptVaultBusiness(envelope, factors, _k1Handle);
      } finally {
        if (_k1Handle && typeof _k1Handle.free === "function") { try { _k1Handle.free(); } catch (_) {} }
      }
    };

    const _serverK1Version = encK1.k1Version ?? null;
    let result = null;
    let usedK1Version = _serverK1Version;
    try {
      result = await _attemptUnwrapDecrypt(encK1);
    } catch (bodyErr) {
      // 現行 K1 で本体が開けない → 本体が書かれた version の K1 で再試行。
      let candidates = [];
      if (envelope.k1Version != null && envelope.k1Version !== _serverK1Version) {
        candidates = [envelope.k1Version];
      } else if (envelope.k1Version == null && _serverK1Version != null && _serverK1Version > 1) {
        // legacy (k1Version 不在): current-1 → 1 を降順に総当り (= deprecated に退避された旧 K1)。
        for (let vv = _serverK1Version - 1; vv >= 1; vv--) candidates.push(vv);
      }
      let healed = false;
      for (const ver of candidates) {
        let oldEncK1;
        try {
          oldEncK1 = await _fetchEncK1V2(tempSigning, { version: ver });
        } catch (_) { continue; }  // version_not_available / deprecated_not_found (TTL切れ)
        try {
          result = await _attemptUnwrapDecrypt(oldEncK1);
          usedK1Version = oldEncK1.k1Version ?? ver;
          healed = true;
          console.log("[decryptVaultAuto] K1 version fallback で復号成功: body v" + usedK1Version + " (server current v" + _serverK1Version + ")");
          break;
        } catch (_) { /* try next */ }
      }
      if (!healed) { throw bodyErr; }
    }

    return {
      ...result,
      empPrivKey: result.empPrivKey ?? null,         // handle 経路 (raw empPrivKey 廃止)
      empPrivHandle: empPrivHandle ?? result.empPrivHandle ?? null, // session で保持 (= 機種追加・rotation 等で再利用)
      k1Version: usedK1Version,            // 本体を開いた K1 の version
      _serverK1Version,                    // #3: server 現行 version (< なら transition 対象)
    };
  }
  // Personal / admin: 従来通り
...
// Stage2 Business Phase B incr2: _extractK2FromBusinessEnvelope の handle 返却版。
//   kek.unwrap_mek で K2 を MekKey opaque handle 化 (= 生 K2 を JS heap に出さない)。
//   WASM/handle 不在 (= kek が CryptoKey) または全 wrap open 失敗時は null を返す
//   → caller は raw 版 _extractK2FromBusinessEnvelope に fallback (= 現状動作を完全保持)。
async function _extractK2HandleFromBusinessEnvelope(envelope, factors) {
  const salt = b64uDecode(envelope.s);
  const haveP = !!factors?.password;
  const haveK = factors?.prfOutput instanceof Uint8Array && factors.prfOutput.length >= 16;
  const haveR = factors?.recoveryMaterial instanceof Uint8Array && factors.recoveryMaterial.length >= 32;
  ...
  if (pMat && kMat && envelope.w?.b?.length) {
    ...
    for (const w of candidates) {
      try {
        const kek = await deriveKEK(pMat, kMat, "kek_pk");
        if (!(kek && typeof kek.unwrap_mek === "function")) return null;  // WASM/handle 不在
        return kek.unwrap_mek(b64uDecode(w.c), b64uDecode(w.i));
      } catch { /* try next */ }
    }
  }
  // Path AC → wrap_a
  if (pMat && rMat && envelope.w?.a) {
    try {
      const kek = await deriveKEK(pMat, rMat, "kek_pr");
      if (!(kek && typeof kek.unwrap_mek === "function")) return null;
      return kek.unwrap_mek(b64uDecode(envelope.w.a.c), b64uDecode(envelope.w.a.i));
    } catch { /* try BC */ }
  }
  // Path BC → wrap_c
  if (kMat && rMat && envelope.w?.c?.length) {
  ...
  // どの wrap も開けなかった → null (caller が raw 版で詳細エラーを出す)
  return null;
}

async function aesGcmDecryptRaw(key, iv, ciphertext) {
  // Stage G9 hotfix (2026-06-06): deriveKEK が MekKey opaque handle (Rust) を返すように
  //   なって以降、 CryptoKey 前提の subtle.decrypt 直叩きでは handle を受け付けず壊れる。
  //   handle 検出して Rust path に dispatch。 Business 社員 unlock 全滅の真因 fix。
  if (key && typeof key.aes_gcm_decrypt === "function") {
    return key.aes_gcm_decrypt(iv, ciphertext, new Uint8Array(0));
  }
  throw new Error("aesGcmDecryptRaw: opaque handle 必須 (CryptoKey 廃止)");
}

/**
 * Phase 2-H2 (2026-06-06): aesGcmEncrypt の polymorphic 版。
 *   _session.mekKey の type 切替 (= CryptoKey → MekKey handle) に備え、
 *   saveVault body encrypt 等の raw 直叩きを廃止する。 handle 検出して
 *   Rust path に dispatch、 CryptoKey path はそのまま subtle.encrypt。
 */
async function aesGcmEncryptRaw(key, iv, plaintext) {
  if (key && typeof key.aes_gcm_encrypt === "function") {
    return key.aes_gcm_encrypt(iv, plaintext, new Uint8Array(0));
  }
  throw new Error("aesGcmEncryptRaw: opaque handle 必須 (CryptoKey 廃止)");
}
...
async function _deriveTransientMek(factors) {
  ...
  // Stage 2 Phase B: mutation 用に raw mek が必要。 forceRawMek で handle 産生をスキップさせ raw を得る。
  const result = await decryptVault(env, { ...factors, forceRawMek: true });
  // mek raw のみ抜き出し、 他は破棄
  return new Uint8Array(result.mek);
}

async function openSession({ vault, mek, mekKey, mekHkdfKey, k2Handle, signingKey: signingKeyFromResult,
                             outerKeyBytes, appNameTag, recoveryMaterial,
                             recoverySecret,
                             credIdHash, credentialId, latestTxId, lastEnvelope, restoredFromHistory,
                             currentAppNameTag, currentTierQualifier,
                             // === Phase 7.2-B v2 追加 ===
                             empPrivKey, empPrivHandle, k1Version }) {
  ...
  // Phase 2-H1: Rust SigningKey opaque handle 優先 (= signRequest が duck-type 検出して使用)
  let signingState;
  if (signingKeyFromResult?.handle && typeof signingKeyFromResult.handle.ecdsa_sign === "function") {
    // Rust handle path — signingPrivateKey は SigningKey opaque handle
    signingState = {
      signingPrivateKey: signingKeyFromResult.handle,
      signingPublicKey: null,  // not needed in handle path (= verify は server 側)
      publicKeyRaw: signingKeyFromResult.publicKeyRaw,
      publicKeyJwk: signingKeyFromResult.publicKeyJwk,
    };
  } else {
    // fallback 撤去: handle 不在時は buildSigningState (これも handle 必須)。
    signingState = await buildSigningState(mek);
  }
  ...
  let _rmatHandle = null;
  let _outerKeyHandle = null;
  try {
    if (recoveryMaterial instanceof Uint8Array && recoveryMaterial.length === 32) {
      // appNameTag 用に RMatKey opaque handle を保持 (CryptoKey HKDF base 廃止)。
      _rmatHandle = await importRMatAsHandle(recoveryMaterial);
      // handle 化: outer 鍵を OuterKey opaque handle で保持 (CryptoKey 不使用)。
      //   rMat→outer の raw bytes は importOuterKeyAsHandle 直後に fill(0)。
      const _obytes = deriveOuterKeyBytes(recoveryMaterial);
      _outerKeyHandle = await importOuterKeyAsHandle(_obytes);
      _obytes.fill(0);
    } else if (outerKeyBytes instanceof Uint8Array && outerKeyBytes.length === 32) {
      // AB unlock: user.id 由来の outerKeyBytes を OuterKey handle に。
      _outerKeyHandle = await importOuterKeyAsHandle(outerKeyBytes);
    }
  } catch (e) {
    console.warn("[openSession] rMatHkdfKey / outerKeyHandle derive failed (non-fatal):", e?.message);
  }
  ...
  if (_outerKeyHandle && outerKeyBytes instanceof Uint8Array) {
  ...
  if (_rmatHandle && recoveryMaterial instanceof Uint8Array) {
  ...
  _session = {
    ...
    rmat: _rmatHandle,               // handle 化: RMatKey opaque handle (appNameTag 派生用)
    outerKeyHandle: _outerKeyHandle, // handle 化: OuterKey opaque handle (outer wrap/unwrap の primary)
    outerKey: null,                  // CryptoKey outer 鍵は廃止 (handle 一本)
    outerKeyBytes: _outerKeyHandle ? null : (outerKeyBytes ?? null),  // handle 不在時のみ raw fallback
    appNameTag,
    k2Handle: k2Handle ?? null,      // 増分1: K2 は MekKey opaque handle のみ (k2AesKey/k2HkdfKey/businessK2 廃止)
    recoveryMaterial: _rmatHandle ? null : (recoveryMaterial ?? null),  // handle 派生成功時は raw 不保持
    ...
    restoredFromHistory: restoredFromHistory ?? null,
    ...
    empPrivHandle: empPrivHandle ?? null,  // Phase 2-F8 step 3a: EmpPrivKey opaque handle (dormant)
  ...
  const _isBusinessSession = (lastEnvelope?.m === "business") || !!k2Handle;
  ...
  // #3: business 非 pending session で body の K1 version が server 現行より古ければ、
  //   現行 K1 mek で body を再暗号化保存して自己治癒する (= K1 rotation 後の社員 vault 移行)。
  //   fire-and-forget。 version が一致していれば tryTransitionK1Version 内で no-op。
  //   restoredFromHistory (= 最新が Arweave 未伝播で history にフォールバックして開いた) 時は
  //   移行を発火させない。 さもないと (a) 毎回の解錠で再保存するチャーン、 (b) forceOverwrite で
  //   伝播中の新版 envelope を上書きする事故、 が起きる。 伝播が追いつけば最新を直接開いて安定。
  if (_isBusinessSession && !lastEnvelope?.k1Pending && !restoredFromHistory) {
    tryTransitionK1Version().then((r) => {
      if (r?.transitioned && typeof window !== "undefined") {
        try {
          window.dispatchEvent(new CustomEvent("arpass:k1-version-upgraded", {
            detail: { fromVersion: r.fromVersion, k1Version: r.k1Version }
          }));
        } catch (e) { /* ignore */ }
      }
    }).catch((e) => {
      console.warn("[openSession] K1 version auto-transition failed (non-fatal):", e?.message);
    });
  }
}
...
export async function refreshTierQualifier() {
  ...
  if (!_session || (!_session.rmat && !_session.recoveryMaterial)) return null;
  ...
  if (_session.rmat) {
    try {
      const { deriveAppNameTagFromHkdf } = await import("/lib/vault-crypto.js?v=6bb1e228");
      newTag = await deriveAppNameTagFromHkdf(_session.rmat, useTier);
    } catch (e) {
...
export async function createPasskey(userId, displayName, opts = {}) {
  ...
  // iOS 殻 + security key(hwkey) 登録は native へ (WKWebView は security key の create 不可)。
  if (opts.securityKey && _hasNativeHwkey()) {
    console.log("[createPasskey] iOS native registration 経路");
    return await nativeHwkeyRegister(userIdBytes, displayName);
  }
  ...
  if (!prfOutput) throw _prfUnsupportedError("PRF extension required but not returned by authenticator");
  return { credentialId, prfOutput };
}

// PRF 未対応端末 (iOS/iPadOS < 18.4 等) で投げる共通エラー。 code で UI が
//   「iOS/iPadOS 18.4 以降が必要」の分かりやすい案内に差し替えられるようにする。
function _prfUnsupportedError(msg) {
  const e = new Error(msg || "PRF extension required but not returned");
...
export async function authenticateWithPasskey(credentialIdHint = null, options = {}) {
  ...
  if (!prfOutput && !options.prfOptional) throw _prfUnsupportedError("PRF extension required but not returned");
...
/**
 * Secret Scanning: Recovery 文字列の照合 index (ss:rs:<hash> -> pkHash) を server に登録。
 *   漏洩検出 webhook がユーザを特定するため。 server は hash しか受け取らない (ZK 維持)。
 *   非致命: 失敗しても vault 作成/再発行は続行する。
 */
async function _registerRecoveryHash(newRecovery, oldRecovery, signingState) {
  try {
    if (!newRecovery || !signingState) return;
    const rsHash = await recoveryStringToHashHex(newRecovery);
    const body = { rsHash };
    if (oldRecovery) {
      try { body.oldRsHash = await recoveryStringToHashHex(oldRecovery); } catch (_) {}
    }
    await signedFetch("/api/secret-scanning/register", "POST", body, signingState);
  } catch (e) {
    console.warn("[secret-scanning] recovery hash register failed (non-fatal):", e?.message);
  }
}

// ── iOS 本番アプリ (WKWebView) 連携: autofill 拡張への provisioning ───────────
//   iOS 殻 (window.webkit.messageHandlers.arpassAutofill) でのみ発火。他環境は no-op。
//   iOS の autofill 拡張は自前で WebAuthn assertion 不可 → 本体で得た PRF/userHandle を
//   App Group secret.bin に、entries(domain/user/recordId) を ASCredentialIdentityStore に渡す。
function _iosAutofillMH() {
  try { return globalThis.webkit?.messageHandlers?.arpassAutofill ?? null; } catch { return null; }
}
function _iosHostFrom(sv) {
  if (!sv) return "";
  try { return new URL(sv.includes("://") ? sv : "https://" + sv).host || ""; } catch { return ""; }
}
// 解錠 secret (PRF 32B / userHandle 57B / credIdHash) を native (secret.bin) へ。
function provisionIosAutofill({ prfOutput, userHandle, credIdHash }) {
  const mh = _iosAutofillMH();
  if (!mh) return;
  try {
    if (!prfOutput || !userHandle || !credIdHash) return;
    if (prfOutput.length !== 32 || userHandle.length !== 57) {
      console.warn("[ios-autofill] skip store: 長さ不正 prf=" + prfOutput.length + " uh=" + userHandle.length);
      return;
    }
    mh.postMessage({ action: "store", prf: b64uEncode(prfOutput), userHandle: b64uEncode(userHandle), credIdHash });
    console.log("[ios-autofill] secret store 送信");
  } catch (e) { console.warn("[ios-autofill] store 失敗(non-fatal):", e?.message); }
}
// vault.entries を identity store に登録 (domain/user/recordId のみ。パスワードは渡さない)。
function registerIosAutofillIdentities(vault) {
  const mh = _iosAutofillMH();
  if (!mh) return;
  try {
    const list = Array.isArray(vault?.entries) ? vault.entries : [];
    const entries = [];
    for (const e of list) {
      if (!e || !e.id) continue;
      const domain = _iosHostFrom(e.url || e.site || "");
      if (!domain) continue;
      entries.push({ domain, user: e.user || "", recordId: e.id });
    }
    mh.postMessage({ action: "registerIdentities", entries });
    console.log("[ios-autofill] identities 送信 " + entries.length + " 件");
  } catch (e) { console.warn("[ios-autofill] identities 失敗(non-fatal):", e?.message); }
}
// hwkey(Master 無し)の解錠 secret(PRF/userHandle25B/credIdHash)を native Keychain(Face ID)へ。
function provisionIosAutofillHwkey({ prfOutput, userHandle, credIdHash }) {
  const mh = _iosAutofillMH();
  if (!mh) return;
  try {
    if (!prfOutput || !userHandle || !credIdHash) return;
    if (prfOutput.length !== 32) { console.warn("[ios-autofill] hwkey skip: prf 長さ不正 " + prfOutput.length); return; }
    mh.postMessage({ action: "storeHwkey", prf: b64uEncode(prfOutput), userHandle: b64uEncode(userHandle), credIdHash });
    console.log("[ios-autofill] hwkey secret store 送信 (Keychain)");
  } catch (e) { console.warn("[ios-autofill] hwkey store 失敗(non-fatal):", e?.message); }
}

// Android TWA autofill: 解錠した Vault の credId + userHandle を native の「直行キャッシュ」へ
//   上書き provision する。hwkey / AB を区別せず「最後に TWA 内で解錠した Vault」を autofill が
//   使うための統一経路。autofill 側は userHandle の長さ(25B=hwkey / 57B=AB)でモードを自動判別。
//   credId / userHandle は秘密ではない。TWA(standalone/fullscreen)のときだけ実行し、
//   desktop ブラウザ等では no-op。受け口は arpass-provision://cred?credId=..&uh=.. (iframe)。
// Android TWA: 最後の解錠で作った provision URL。明示的な「📲 自動入力に登録」ボタンから
//   user gesture で発火するために保持する (custom scheme 起動には activation が必須)。
let _androidProvisionUrl = null;
export function hasAndroidProvision() { return !!_androidProvisionUrl; }
// provision 起動は Android 標準の intent:// (package 明示) + top-level navigation で行う。
//   custom scheme(arpass-provision://)の iframe 起動は TWA Chrome では弾かれ Activity が起動しない
//   (実機確認 2026-06-29: ボタンの user gesture でも provisioned cred 出ず)。intent:// は Chrome が
//   特別扱いして package=io.arpass.twa の Activity を確実に起動する。外部 intent 遷移はページを
//   置換しないので TWA はそのまま留まる。
export function fireAndroidProvision() {
  if (!_androidProvisionUrl) return false;
  try {
    window.location.href = _androidProvisionUrl;
    console.log("[android-autofill] provision 手動発火(button, intent://)");
    return true;
  } catch (e) { console.warn("[android-autofill] manual provision 失敗:", e?.message); return false; }
}
function provisionAndroidAutofill({ credentialId, userHandle }) {
  try {
    if (!credentialId || !userHandle) return;
    // Android TWA 限定。iOS は別系統(provisionIosAutofill=webkit messageHandler→App Group)で
    //   provision するため、ここを iOS の WKWebView / ホーム追加 PWA で動かしてはいけない
    //   (arpass-provision:// は Android intent 専用 → iOS では壊れたボタンになる)。UA で Android に限定。
    ...
    const cid = credentialId instanceof Uint8Array ? credentialId : new Uint8Array(credentialId);
    const uh  = userHandle  instanceof Uint8Array ? userHandle  : new Uint8Array(userHandle);
    // intent:// 形式 (Chrome が package 明示で Activity を起動)。data = arpass-provision://cred?...
    const q = "cred?credId=" + encodeURIComponent(b64uEncode(cid))
            + "&uh=" + encodeURIComponent(b64uEncode(uh));
    const intentUrl = "intent://" + q + "#Intent;scheme=arpass-provision;package=io.arpass.twa;end";
    _androidProvisionUrl = intentUrl;  // 「📲 自動入力に登録」ボタン(明示タップ)で発火する
    console.log("[android-autofill] provision URL 準備完了(ボタンで発火) uhLen=" + uh.length);
  } catch (e) { console.warn("[android-autofill] provision 失敗(non-fatal):", e?.message); }
}
...
export async function createVault(password, userDisplayName, captchaToken = null, opts = {}) {
  ...
  const { generateRecoverySecret } = await import("/lib/vault-crypto.js?v=6bb1e228");
  ...
  // v8: per-credential nonce で user.id を一意化 (同期パスキー衝突→置換の防止)
  const userId = await encodeUserIdV8(appNameTag, deriveOuterKeyBytes(recoveryMaterial), password);
  ...
  if (mode === "business") {
    ...
    // 増分1: enc.k2 (raw) / k2AesKey / k2HkdfKey は廃止、 K2 は handle のみ。
    //   encryptedRecovery は encryptVaultBusiness 内で body に inject 済。
    //   in-memory vault には K2 handle 経由で同等の encryptedRecovery を再生成。
    if (recoverySecret && enc.k2Handle) {
      try {
        // 修正(recovery K1非依存化): K2 由来鍵で保護 (transition/rotation 後も再表示可能)。
        vault.encryptedRecovery = await encryptRecoveryWithMek(recoverySecret, enc.k2Handle);
      } catch (e) {
  ...
  // handle 化: business/personal とも enc.signingKey の SigningKey opaque handle を使う。
  let signingState;
  if (!enc.signingKey?.handle || typeof enc.signingKey.handle.ecdsa_sign !== "function")
    throw new Error("createVault: SigningKey handle 必須 (Rust)");
  signingState = {
    signingPrivateKey: enc.signingKey.handle,
    signingPublicKey: null,
    publicKeyRaw: enc.signingKey.publicKeyRaw,
    publicKeyJwk: enc.signingKey.publicKeyJwk ?? null,
  };
  ...
  const writeResult = await writeEnvelope(
    ...
    {
      tier: "free",  // 増分1: businessK2 廃止 (従来も常に null → "free")
  ...
  await openSession({
    ...
    // Phase2: Rust 経路は EmpPrivKey handle (privHandle)、 WebCrypto fallback のみ privKey(CryptoKey)。
    empPrivKey: mode === "business" ? (enc.empKeypair?.privKey ?? null) : null,
    empPrivHandle: mode === "business" ? (enc.empKeypair?.privHandle ?? null) : null,

    k1Version: mode === "business" ? 1 : null,
  });

  // iOS autofill: サインアップ直後に secret.bin を用意 (userHandle = v7 user.id)。
  provisionIosAutofill({ prfOutput, userHandle: userId, credIdHash });
  // Android TWA autofill: 作成直後の Vault を直行キャッシュに登録。
  provisionAndroidAutofill({ credentialId, userHandle: userId });
  registerIosAutofillIdentities(vault);
  ...
  // Secret Scanning: Recovery 文字列の照合 index を登録 (漏洩検出時のユーザ特定用)。
  await _registerRecoveryHash(recoverySecret, null, signingState);
...
export async function createHwkeyVault(keyCount, userDisplayName, captchaToken = null, opts = {}) {
  ...
  for (let i = 0; i < keyCount; i++) {
    ...
    const { credentialId, prfOutput } = await createPasskey(userId, userDisplayName, {
      requireResidentKey: true,
      userVerification: "required", // hwkey: PRF を UV 非依存 (touch のみ) に固定
      followupTransports: ["usb", "nfc", "ble"],   // PRF follow-up get を YubiKey 直行に
      securityKey: true,               // iOS 殻では native registration に回す
    });
    const credIdHash = await credentialIdToHash(credentialId);
  ...
  // 3. 署名鍵 (MEK handle 由来) → サーバ register。 enc.signingKey は SigningKey handle。
  if (!enc.signingKey?.handle || typeof enc.signingKey.handle.ecdsa_sign !== "function")
    throw new Error("createHwkeyVault: SigningKey handle 必須 (Rust)");
  const signingState = {
    signingPrivateKey: enc.signingKey.handle,
    signingPublicKey: null,
    publicKeyRaw: enc.signingKey.publicKeyRaw,
  ...
  await openSession({
    ...
    signingKey: enc.signingKey ?? null,  // handle 化: openSession が SigningKey handle を使う (mek=null 対応)
...
async function _authenticateHwkey({ forcePicker = false, credentialIdHint = null } = {}) {
  ...
  if (!prfOutput) {
    ...
    let second;
    try {
      second = await authenticateWithPasskey(credentialId, { transports: ["usb", "nfc", "ble"], userVerification: "required" });
    } catch (e) {
  ...
  if (!prfOutput) throw _prfUnsupportedError("PRF extension required but not returned");
...
export async function hwkeyAuthenticate({ forcePicker = false, credentialIdHint = null, transports = null, hints = null } = {}) {
  const r = await authenticateWithPasskey(credentialIdHint, {
    ...
    ...(Array.isArray(hints) && hints.length ? { hints } : {}),
  });
...
export async function hwkeyAuthenticateForUnlock(opts = {}) {
  // iOS 本番アプリ内: WKWebView は YubiKey(security key) WebAuthn 不可 →
  //   native assertion(ASAuthorizationSecurityKey+PRF)で {credentialId,prfOutput,userHandle} を取得。
  //   (UI は本関数 → unlockWithHwkeyAuthed を呼ぶ。helper は同ファイル下部で定義済=実行時には初期化済)
  // forcePicker(「別の端末で開く」導線)は meta を無視し discoverable(一覧表示)に強制する
  //   (別端末の YubiKey ではローカル credId は無関係/古く、specific だと一覧が出ず誤選択→hang)。
  //   ただし Android Chrome は discoverable+PRF が不安定(NFC選択後 PIN に進まない)なので、
  //   meta があれば従来どおり specific get を維持(失敗時は picker フォールバック)。
  const _forcePicker = opts.forcePicker === true;
  if (_hasNativeHwkey()) {
    let hint = null;
    if (!_forcePicker) { try { const m = readMeta(); if (m && m.mode === "hwkey" && m.credentialId) hint = m.credentialId; } catch (_) {} }
    console.log("[hwkeyAuthForUnlock] iOS native assertion 経路 hint=" + (hint ? "あり(specific)" : "なし(discoverable)") + " forcePicker=" + _forcePicker);
    return await nativeHwkeyAssert(hint ? [hint] : null);
  }
  let hint = null;
  try {
    const meta = readMeta();
    // forcePicker(「別の端末で開く」)は全環境で discoverable(一覧表示)。Android も一覧を出す
    //   (※Android の一覧は Credential Manager=Google パスワードマネージャで、そこから YubiKey に
    //     到達できるかは端末依存。到達不可なら「端末追加コード」で specific get に切替える)。
    if (!_forcePicker && meta && meta.mode === "hwkey" && meta.credentialId) {
  ...
  // Android 2-tap 特例は撤去。UV=required の単一 discover(+PRF) に全環境統一(PM 起動1回)。
...
export async function unlockWithHwkeyAuthed({ credentialId, prfOutput, userHandle, __onPhase }) {
  if (!prfOutput)
    throw _prfUnsupportedError("PRF extension required but not returned");
  if (!userHandle || !isUserIdHwkey(userHandle))
  ...
  await openSession({
    ...
    signingKey: r.signingKey ?? null,
  });
  console.log("[hwkeyUnlock] openSession OK — unlocked");

  // iOS autofill: hwkey secret を Keychain(Face ID) へ + entries を identity store へ。
  provisionIosAutofillHwkey({ prfOutput, userHandle, credIdHash });
  // Android TWA autofill: 最後に解錠した Vault として直行キャッシュを上書き。
  provisionAndroidAutofill({ credentialId, userHandle });
  registerIosAutofillIdentities(r.vault);

  return { vault: r.vault, latestTxId: txid, path: "K" };
}

// ── iOS 殻: native YubiKey assertion 経由 (WKWebView は security key WebAuthn 不可) ──
//   本体アプリの HwkeyBridge が ASAuthorizationSecurityKey+PRF を実行し、結果を
//   window.__arpassHwkeyResolve(reqId, {credentialId,prfOutput,userHandle}) で返す。
function _hasNativeHwkey() {
  try { return !!globalThis.webkit?.messageHandlers?.arpassHwkey; } catch { return false; }
}
const _hwkeyResolvers = new Map();
function _ensureHwkeyResolveHook() {
  if (globalThis.__arpassHwkeyResolve) return;
  globalThis.__arpassHwkeyResolve = (reqId, result) => {
    const r = _hwkeyResolvers.get(reqId);
    if (!r) return;
    _hwkeyResolvers.delete(reqId);
    r(result);
  };
}
async function nativeHwkeyAssert(allowCredentials = null) {
  _ensureHwkeyResolveHook();
  const reqId = (globalThis.crypto?.randomUUID?.() ) || String(Date.now() + Math.random());
  const prfSalt = b64uEncode(PRF_SALT);
  const msg = { reqId, rpId: RP_ID, prfSalt };
  if (Array.isArray(allowCredentials) && allowCredentials.length) msg.allowCredentials = allowCredentials;
  const result = await new Promise((resolve, reject) => {
    _hwkeyResolvers.set(reqId, resolve);
    try {
      globalThis.webkit.messageHandlers.arpassHwkey.postMessage(msg);
    } catch (e) { _hwkeyResolvers.delete(reqId); reject(e); }
  });
  if (!result || result.error) throw new Error("native hwkey assert 失敗: " + (result?.error || "unknown"));
  return {
    credentialId: b64uDecode(result.credentialId),
    prfOutput: b64uDecode(result.prfOutput),
    userHandle: b64uDecode(result.userHandle),
  };
}
// iOS 殻: native YubiKey 登録 (WKWebView は security key create 不可)。戻り {credentialId, prfOutput}。
async function nativeHwkeyRegister(userIdBytes, displayName) {
  _ensureHwkeyResolveHook();
  const reqId = (globalThis.crypto?.randomUUID?.()) || String(Date.now() + Math.random());
  const prfSalt = b64uEncode(PRF_SALT);
  const msg = { action: "register", reqId, rpId: RP_ID, prfSalt,
                userId: b64uEncode(userIdBytes), displayName: displayName || "YubiKey" };
  const result = await new Promise((resolve, reject) => {
    _hwkeyResolvers.set(reqId, resolve);
    try { globalThis.webkit.messageHandlers.arpassHwkey.postMessage(msg); }
    catch (e) { _hwkeyResolvers.delete(reqId); reject(e); }
  });
  if (!result || result.error) throw new Error("native hwkey register 失敗: " + (result?.error || "unknown"));
  return { credentialId: b64uDecode(result.credentialId), prfOutput: b64uDecode(result.prfOutput) };
}

export async function unlockWithHwkey(options = {}) {
  // iOS 本番アプリ内: WKWebView は YubiKey WebAuthn 不可 → native assertion で材料取得し
  //   既存の unlockWithHwkeyAuthed(WebAuthn を再呼出しない) で解錠を完了する。
  if (_hasNativeHwkey()) {
    console.log("[hwkeyUnlock] iOS native assertion 経路");
    const auth = await nativeHwkeyAssert();
...
export async function addHwkeyDevice(userDisplayName, opts = {}) {
  ...
  const mek = exDec.mekKey ?? exDec.mek;   // MekKey handle 優先 (WASM時は生 mek 無し)、 raw は fallback
  ...
  try {
    ...
    const { credentialId: newCredId, prfOutput: newPrf } = await createPasskey(
      ...
      { requireResidentKey: true, userVerification: "required",
        followupTransports: ["usb", "nfc", "ble"], securityKey: true },
    );
    ...
    else if (mek && typeof mek.free === "function") { try { mek.free(); } catch (_) {} }
...
export async function unlockWithPasswordAndPasskey(password, options = {}) {
  ...
  let vault, mek, path;
  let _restoredTxid = null, _restoredEnvelope = null;   // history フォールバックで使った旧版
  try {
    ...
    try {
      console.error("[unlock-AB][diag] usedCredIdHash=" + credIdHash
        + " | env.m=" + (envelope?.m ?? "(2of3)")
        + " | w.b[].h=" + JSON.stringify((envelope?.w?.b || []).map((x) => x.h))
        + " | w.c[].h=" + JSON.stringify((envelope?.w?.c || []).map((x) => x.h))
        + " | hasW.a=" + !!envelope?.w?.a
        + " | txid=" + txid
        + " | metaCredId=" + (meta?.credentialId ?? "(none)"));
    } catch (_) {}
    // ── 「この factor で開ける最新」を取得する: pk-slot の global 最新が手元の Passkey/Master
    //    で開けない場合、 同 appNameTag の Arweave 履歴を新しい順にさかのぼり、 手元の factor で
    //    開ける直近の版を開く (= ユーザー期待: マスター+パスキーの組で開ける最新を返す)。
    try {
      const _hist = await findVaultTxHistory(appNameTag, 10);
      for (const _oldTxid of (_hist || [])) {
        if (!_oldTxid || _oldTxid === txid) continue;
        try {
          const { envelope: _oldEnv } = await fetchEnvelope(_oldTxid, _abOuterKey ?? outerKeyBytes);
          const _r2 = await decryptVaultAuto(_oldEnv, { password, prfOutput, credIdHash });
          if (_r2?.vault) {
            console.warn("[unlock-AB] history フォールバック: 最新 " + txid + " が読めず " + _oldTxid + " で復旧 (path=" + _r2.path + ", credIdHash=" + credIdHash + ", w.b[].h=" + JSON.stringify((_oldEnv?.w?.b||[]).map((x)=>x.h)) + ")");
            _r = _r2; ({ vault, mek, path } = _r2);
            _restoredTxid = _oldTxid; _restoredEnvelope = _oldEnv;
            break;
          }
        } catch (_) { /* この版もダメ → さらに古い版へ */ }
      }
    } catch (_) { /* GraphQL 失敗 → フォールバック不可 */ }
    if (!_restoredTxid) {
  ...
  await openSession({
    ...
    credIdHash, credentialId,
    latestTxId: _restoredTxid ?? txid,
    lastEnvelope: _restoredEnvelope ?? envelope,
    restoredFromHistory: _restoredTxid ? { restoredTxid: _restoredTxid, latestTxid: txid } : null,
    k2Handle: _r?.k2Handle ?? null,
    signingKey: _r?.signingKey ?? null,
    empPrivKey: _r?.empPrivKey ?? null,
    empPrivHandle: _r?.empPrivHandle ?? null,  // F8 step 3a
    k1Version: _r?.k1Version ?? null,
  });
  patchMeta({ latestTxId: txid });

  // iOS autofill: 解錠 secret(secret.bin) と entries(identity) を native へ。
  provisionIosAutofill({ prfOutput, userHandle, credIdHash });
  // Android TWA autofill: 最後に解錠した Vault として直行キャッシュを上書き。
  provisionAndroidAutofill({ credentialId, userHandle });
  registerIosAutofillIdentities(vault);
...
export async function unlockWithPasswordAndRecovery(password, recoveryString, opts = {}) {
  ...
  // fix(bootstrap/business recovery): 新規作成直後(tier 未確定)や business/corp vault の
  //   最新 tx は null(=tier 無し) タグで書かれる (emergency-restore-main.js と同じ前提)。
  //   Phase 7.4.1 で dev 期 legacy を除外した際に bootstrap/business を巻き添えで漏らして
  //   いたため、 端末追加 Recovery で「vault が見つからない」となっていた。 null タグも探す。
  const bootstrapTag = deriveAppNameTag(recoveryMaterial, null);
  const tagList = [bootstrapTag, tags.free, tags.paid, tags.private];
  ...
  await openSession({
    ...
    k2Handle: _rAC?.k2Handle ?? null,
    signingKey: _rAC?.signingKey ?? null,
    empPrivKey: _rAC?.empPrivKey ?? null,
    empPrivHandle: _rAC?.empPrivHandle ?? null,  // F8 step 3a
...
export async function unlockWithPasskeyAndRecovery(recoveryString, options = {}) {
  ...
  // fix(bootstrap/business recovery): null(=tier 無し) タグも検索 (Path BC も同防御)。
  //   新規作成直後や business/corp vault の最新 tx は null タグで write されるため。
  const bootstrapTag = deriveAppNameTag(recoveryMaterial, null);
  const tagList = [bootstrapTag, tags.free, tags.paid, tags.private];
  ...
  await openSession({
    ...
    latestTxId: txid,
    lastEnvelope: envelope,
    k2Handle: _rBC?.k2Handle ?? null,
    signingKey: _rBC?.signingKey ?? null,
    empPrivKey: _rBC?.empPrivKey ?? null,
    empPrivHandle: _rBC?.empPrivHandle ?? null,  // F8 step 3a
...
export async function saveVault(updatedVault, opts = {}) {
  ...
  // Fix1 (2026-07-17): history フォールバックで開いたセッションからの保存を安全化。
  //   従来は saveVault が restoredFromHistory を検出すると自動で forceOverwrite=true を
  //   立て、楽観ロック(expectedLatestTxId)を黙ってバイパスして「古い(未配信 or 履歴)版＋
  //   この端末の変更」を最新として強制上書きしていた。これが他端末の wrap を落とす恒久
  //   wrap-loss の引き金だった (project_arpass_history_fallback 追記②)。
  //   → 自動 forceOverwrite を廃止し、フォールバックからの保存は既定で安全に中止する。
  //   明示的な救済 (opts.forceOverwrite===true, 例: 409 解決モーダルでユーザーが上書きを選択)
  //   のときのみ従来動作。フォールバックは「未配信の一時状態」か「真に最新を失った状態」の
  //   いずれかで、どちらも Master+Recovery で開き直せば非フォールバックの正規セッションになり
  //   安全に保存できる (recovery wrap は全版に在る)。
  if (_session.restoredFromHistory && opts.forceOverwrite !== true) {
    const err = new Error(
      "この端末は最新版の配信待ち（履歴フォールバック）で開いています。この状態から保存すると" +
      "他の端末の解錠鍵を失う恐れがあるため、保存を中止しました。数分待って「最新を再試行」するか、" +
      "Master + Recovery で開き直してから保存してください。"
    );
    err.code = "fallback_write_blocked";
    throw err;
  }
  ...
  // 修正(recovery K1非依存化): personal は従来通り mek(=K2)。 business は K2 (k2Handle/k2HkdfKey) で
  //   保護し、 recoverySecret が session にあれば inject/再inject 可 (= transition/rotation で
  //   壊れた encryptedRecovery を、 recovery key 再入力 → save で修復できる)。
  const needsInjectPersonal = !isBusinessMode && _session.recoverySecret &&
    (!updatedVault.encryptedRecovery || updatedVault.encryptedRecovery.v !== 2);
  const _bizRecKey = isBusinessMode ? _session.k2Handle : null;
  const needsInjectBusiness = isBusinessMode && !!_session.recoverySecret && !!_bizRecKey;
  if (needsInjectPersonal || needsInjectBusiness) {
    updatedVault = {
      ...updatedVault,
      encryptedRecovery: await encryptRecoveryWithMek(_session.recoverySecret, needsInjectBusiness ? _bizRecKey : _session.mek),
    };
  ...
  // Phase 2-H2 hotfix: aesGcmEncryptRaw 経由で handle/CryptoKey 自動 dispatch
  //   (H2-prep commit 6f8cd76 で漏れていた saveVault path、 H2-b の mekKey handle 化で発覚)
  if (!_session.mekKey) throw new Error("saveVault: _session.mekKey (handle) 必須 (Rust crypto core)");
  const mekKey = _session.mekKey;
  const padded = padPlaintext(new TextEncoder().encode(JSON.stringify(updatedVault)));
  const bodyCt = await aesGcmEncryptRaw(mekKey, ivBody, padded);
  ...
  // #3 (K1 version 自己記述化): business 本体が どの K1 version の mek で暗号化されたかを
  //   envelope に刻む。 解錠側はこれを見て該当 version の K1 を fetch して開く。
  //   _session.k1Version は body 暗号化に使った mek の K1 version (= mekKey の素)。
  if (isBusinessMode && _session.k1Version != null) {
    newEnvelope.k1Version = _session.k1Version;
  } else if (!isBusinessMode && "k1Version" in newEnvelope) {
    // personal/admin に business の k1Version が混入しないよう除去 (spread 由来の保険)。
    delete newEnvelope.k1Version;
  }
  ...
  let writeResult;
  try {
    writeResult = await writeEnvelope(
      newEnvelope, (_session.outerKeyHandle ?? _session.outerKey ?? _session.outerKeyBytes), writeAppName, _session.signingState,
      {
        ...
        forceOverwrite: opts.forceOverwrite === true,   // Phase 7.1-N.3: 救済 escape hatch (明示時のみ)。Fix1(2026-07-17): restoredFromHistory の自動 forceOverwrite は saveVault 冒頭ガードで廃止 (恒久 wrap-loss 防止)
        tier: _resolveCurrentTier(),                    // Phase 7.2-B v2.6: tier 申告
      }
    );
  } catch (e) {
    // 2026-07-12: Turbo Payment Service 障害 (有料アップロードのみ落ちる) の場合、
    //   <100KiB sponsored パディングで 1 回だけ再暗号化して再試行する。
    //   saveVault は salt/鍵を再生成しない (body のみ再暗号化) ため retry 安全。
    const _outage = e?.code === "upload_failed" || e?.status === 502 || e?.status === 503;
    if (_outage && !opts._emergencyPadRetry) {
      enableEmergencyPad();
      console.warn("[saveVault] storage outage — retrying once with emergency (<100KiB sponsored) padding");
      return await saveVault(updatedVault, { ...opts, _emergencyPadRetry: true });
    }
    throw e;
  }
  ...
  // iOS autofill: vault が変わったので QuickType 候補ストア(ASCredentialIdentityStore)を
  //   全置換で最新化する。 これが無いと削除済みエントリが候補に残り、選択→解錠時に
  //   recordNotFound になる。 (iOS 殻以外は no-op。 追加/編集/削除いずれの saveVault でも反映)
  try { registerIosAutofillIdentities(updatedVault); } catch (_) {}
...
export async function addCredentialOnThisDevice(password, userDisplayName, opts = {}) {
  ...
  // task16 (2026-07-17): 呼び出し側 (設定画面の Passkey 追加など、passkey 解錠で
  //   session に recovery が無い状況) が opts.recoverySecret で Recovery を明示的に
  //   渡せるようにする。優先順位: session raw → opts.recoverySecret(ユーザー入力) →
  //   session.recoverySecret → vault.encryptedRecovery 復号。ユーザー入力由来のときは
  //   下で appNameTag 一致を検証し、誤入力で「この Vault で解錠できない孤児パスキー」を
  //   作らないようにする。
  let _rMatFromUserInput = false;
  if (!rMatTransient) {
    let recoveryString = null;
    if (typeof opts.recoverySecret === "string" && opts.recoverySecret.length >= 4) {
      recoveryString = opts.recoverySecret;
      _rMatFromUserInput = true;
    } else if (typeof _session.recoverySecret === "string" && _session.recoverySecret.length >= 4) {
  ...
  // ユーザー入力の Recovery がこの Vault のものか検証 (孤児パスキー防止)。
  //   deriveAppNameTag(rMat, null) = legacy tag。_session.appNameTag も legacy。
  if (_rMatFromUserInput) {
    let _okTag = false;
    try {
      const _tag = deriveAppNameTag(rMatTransient, null);
      _okTag = !!_session.appNameTag && _tag.value === _session.appNameTag.value;
    } catch (_) { _okTag = false; }
    if (!_okTag) {
      if (rMatAllocated) rMatTransient.fill(0);
      const err = new Error("入力された Recovery Secret がこの Vault と一致しません。正しい Recovery Secret を入力してください。");
  ...
  const userId = await encodeUserIdV8(_session.appNameTag, _outerKeyBytesForUserId, password);  // v8: 一意化 nonce で他端末パスキーを上書きしない
...
export async function changePasswordUI(newPassword, recoveryString, userDisplayName) {
  ...
  let newUserId;
  try {
    newUserId = await encodeUserIdV8(_session.appNameTag, _outerKeyBytes, newPassword);  // v8: 一意化 nonce
  } finally {
  ...
  const writeResult = await writeEnvelope(
    ...
    { expectedLatestTxId: _session.latestTxId, tier: _resolveCurrentTier() }  // sec-2026-07 報告3: 楽観ロック必須化（stale session からの credential-rotation 巻き戻し防止）
  );
...
export async function reissueRecovery_caseA(password) {
  if (!_session) throw new Error("locked — unlock first");
  const _oldRecovery = _session?.recoverySecret || null;  // Secret Scanning: 旧 index 削除用
  if (!_session.lastEnvelope) throw new Error("session に直近 envelope がありません");
  if (!_session.currentCredIdHash) {
  ...
  try {
    // appNameTag 用 RMatKey handle を新 rMat に差し替え (古い handle free、 stale 防止)。
    if (_session.rmat && typeof _session.rmat.free === "function") { try { _session.rmat.free(); } catch (_) {} }
    _session.rmat = await importRMatAsHandle(newRMat);
    // handle 化: outer 鍵を新 OuterKey handle に差し替え (古い handle は free、 stale 防止)。
    if (_session.outerKeyHandle && typeof _session.outerKeyHandle.free === "function") {
      try { _session.outerKeyHandle.free(); } catch (_) {}
    }
    _session.outerKeyHandle = await importOuterKeyAsHandle(result.newOuterKeyBytes);
    _session.outerKey = null;
    if (result.newOuterKeyBytes instanceof Uint8Array) result.newOuterKeyBytes.fill(0);
    _session.outerKeyBytes = null;
  } catch (e) { console.warn("[changeRecovery] outerKeyHandle re-derive failed:", e?.message); }
  ...
  // Secret Scanning: 新 Recovery を登録 (旧 index は同一 pkHash なので削除される)。
  await _registerRecoveryHash(newRecovery, _oldRecovery, _session.signingState);
...
export async function reissueRecovery_caseB(password) {
  ...
  try {
    if (_session.rmat && typeof _session.rmat.free === "function") { try { _session.rmat.free(); } catch (_) {} }
    _session.rmat = await importRMatAsHandle(newRMat);  // appNameTag 用 RMatKey handle 差し替え
    if (_session.outerKeyHandle && typeof _session.outerKeyHandle.free === "function") {
      try { _session.outerKeyHandle.free(); } catch (_) {}
    }
    _session.outerKeyHandle = await importOuterKeyAsHandle(result.newOuterKeyBytes);
    ...
    if (result.newMek instanceof Uint8Array) {
      _session.mekHkdfKey = await importMekRawAsHandle(result.newMek);  // handle 化
    }
    if (result.newOuterKeyBytes instanceof Uint8Array) result.newOuterKeyBytes.fill(0);
    _session.outerKeyBytes = null;
  } catch (e) { console.warn("[changeRecovery_caseB] outerKeyHandle re-derive failed:", e?.message); }
  _session.recoverySecret = newRecovery;  // Phase 7.0w-AP
  // (outer 鍵 / rMatHkdfKey の再派生は直前のブロックで outerKeyHandle として実施済 — 重複撤去)
  ...
  // Secret Scanning: 新アカウント (新 pkHash) で新 Recovery を登録。
  await _registerRecoveryHash(newRecovery, null, _session.signingState);
  return { newRecovery, txid: writeResult.txid };
}
...
async function _getRealMekForVersion(k1Version) {
  ...
  // #3後続: old K1 fetch には K2 handle + emp_priv (handle 優先、 CryptoKey fallback) が必要。
  if (!_session.k2Handle || (!_session.empPrivHandle && !_session.empPrivKey)) {
    throw new Error("business mode の old K1 取得には K2 handle + emp_priv が必要 (= session 状態不正)");
  }
  let encK1;
  try {
  ...
  // K2 handle + old K1 から old real_MEK を MekKey opaque handle として派生。
  //   bit-equivalence: いずれの経路も K1Key.derive_business_mek_v2(k2Handle, "mek-business-v2")
  //   = HKDF(salt=K1, ikm=K2, info) で同一。
  let oldRealMek;
  if (_session.empPrivHandle) {
    // handle 経路 (= 生 K1 不在): ECIES → K1Key handle → derive_business_mek_v2(k2Handle)。
    const oldK1Handle = await eciesUnwrapToK1Handle(_session.empPrivHandle, encK1);
    try {
      const _info = new TextEncoder().encode("mek-business-v2");
      oldRealMek = oldK1Handle.derive_business_mek_v2(_session.k2Handle, _info);
    } finally {
      if (oldK1Handle && typeof oldK1Handle.free === "function") { try { oldK1Handle.free(); } catch (_) {} }
    }
  } else {
    throw new Error("_getRealMekForVersion: empPrivHandle 必須 (Rust crypto core、 raw empPrivKey 経路廃止)");
  }
...
async function _getMekKeyForVersion(k1Version) {
  ...
  if (currentVaultMode() !== "business") {
    ...
    throw new Error("personal session に mekKey (handle) が無い (Rust crypto core 必須)");
  }
  // Business mode
  const curVer = _session.k1Version ?? 1;
  ...
  // 増分1: _getRealMekForVersion は MekKey opaque handle を返す (wrapKey/unwrapKey が duck-type)。
  const oldMek = await _getRealMekForVersion(k1Version);
  if (oldMek && typeof oldMek.aes_gcm_encrypt === "function") {
    _session._oldMekKeyCache.set(k1Version, oldMek);
    return oldMek;
  }
  // raw が来た場合 (= Rust 不在) は不正: old MEK も handle 必須。
  throw new Error("_getMekKeyForVersion: old MEK は MekKey handle 必須 (Rust crypto core)");
}
...
export async function addRecordUI(input) {
  ...
  // handle 化: MekKey handle (_session.mekKey、 personal/business 共通) で BekKey 経路に統一。
  const _bekWrapMek = _session.mekKey;
  if (_bekWrapMek && typeof _bekWrapMek.wrap_bek === "function") {
    let bekHandle = null;
    try {
      ...
      const w = await wrapBekWithMekHandle(_bekWrapMek, bekHandle);
    ...
    throw new Error("addRecordUI: Rust crypto core (MekKey/BekKey handle) 必須");
  }
  ...
  const record = {
    ...
    attachments: [{
      ...
      referenceUsdMicro: writeRes.referenceUsdMicro ?? null,   // trial 参考額 (請求なし)
...
export async function fetchRecordFileUI(recordId, attachmentIdx = 0) {
  ...
  // handle 化: 該当 version の MekKey handle (_getMekKeyForVersion) で BekKey unwrap に統一。
  //   wrap 形式は CryptoKey 経路と同一 (AES-GCM(mek,bek)) のため既存 record とも互換。
  const _bekUnwrapMek = await _getMekKeyForVersion(att.encryption.k1Version);
  if (_bekUnwrapMek && typeof _bekUnwrapMek.unwrap_bek === "function") {
    let bekHandle = null;
    try {
      bekHandle = await unwrapBekWithMekHandle(_bekUnwrapMek, wrappedBEK, wrapIv);
    ...
    throw new Error("fetchRecordFileUI: Rust crypto core (MekKey/BekKey handle) 必須");
  }
...
export async function sealOldestChunkUI({ threshold = DEFAULT_CHUNK_THRESHOLD, keepRecent = DEFAULT_CHUNK_KEEP } = {}) {
  ...
  // handle 化: _session.mekKey (handle) で CEK を BekKey 経路に統一。
  const _cekWrapMek = _session.mekKey;
  if (_cekWrapMek && typeof _cekWrapMek.wrap_bek === "function") {
    let cekHandle = null;
    try {
      ...
      const w = await wrapBekWithMekHandle(_cekWrapMek, cekHandle);
    ...
    throw new Error("chunk write: Rust crypto core (MekKey/BekKey handle) 必須");
  }
...
export async function loadChunkUI(chunkId) {
  ...
  let bytes;
  // handle 化: 該当 version の MekKey handle で CEK を BekKey unwrap に統一 (既存 chunk 互換)。
  const _cekUnwrapMek = await _getMekKeyForVersion(chunk.encryption.k1Version);
  if (_cekUnwrapMek && typeof _cekUnwrapMek.unwrap_bek === "function") {
    let cekHandle = null;
    try {
      cekHandle = await unwrapBekWithMekHandle(_cekUnwrapMek, wrappedCEK, wrapIv);
    ...
    throw new Error("loadChunkUI: Rust crypto core (MekKey/BekKey handle) 必須");
  }
...
export async function refreshFromServerLatest() {
  if (!_session) throw new Error("locked");
  // history フォールバックで開いた session は、 サーバ最新(= 手元の Passkey/Master では
  //   開けない別系列の版)を採用してはいけない。 採用すると body は同 MEK で復号できてしまう
  //   ため lastEnvelope が別系列の wrap に差し替わり、 次の save がその wrap を流用して
  //   永久に開けない版を書き続ける。 復旧 session は据え置き、 save (forceOverwrite) が
  //   pk-slot を奪取して canonical 化する。
  if (_session.restoredFromHistory) {
    console.warn("[refresh] restoredFromHistory session — サーバ最新は手元 factor で開けないので採用しない (save が奪取する)");
    return { refreshed: false, latestTxId: _session.latestTxId, skippedForRestored: true };
  }
...
async function _decryptBodyWithSessionMek(envelope) {
  ...
  // incr3: handle session 対応 — k2Handle のみ保有する session でも K1 rotation 検知を有効化。
  // handle 一本化: k2Handle + empPrivHandle 必須 (CryptoKey/raw 経路は廃止)。
  if (envelope?.m === "business" && envelope?.w_emp && _session.k2Handle && _session.empPrivHandle) {
    const cid = envelope.cid || _session.vault?.companyId;
    if (cid) {
      try {
        // handle 一本化: empPrivHandle で ECIES → K1Key handle、 mek は WASM 内派生 (生 K1/K2 不在、 fallback 無し)。
        const encK1 = await _fetchEncK1V2(_session.signingState);
        const _newK1Handle = await eciesUnwrapToK1Handle(_session.empPrivHandle, encK1);
        if (_session.k1Handle && typeof _session.k1Handle.free === "function") {
          try { _session.k1Handle.free(); } catch (_) {}
        }
        _session.k1Handle = _newK1Handle;
        const _enc = new TextEncoder();
        const _info = _enc.encode("mek-business-v2");
        const newMekKey = _session.k1Handle.derive_business_mek_v2(_session.k2Handle, _info);
        const newMekHkdfKey = _session.k1Handle.derive_business_mek_v2(_session.k2Handle, _info);
        if (_session.mek) _session.mek.fill(0);
        _session.mek = null;
  ...
  // Phase 2-H2: aesGcmDecryptRaw 経由で handle/CryptoKey 自動 dispatch
  if (!_session.mekKey) throw new Error("decrypt body: _session.mekKey (handle) 必須 (Rust crypto core)");
  const mekKey = _session.mekKey;
  const paddedBuf = await aesGcmDecryptRaw(mekKey, b64uDecode(envelope.i), b64uDecode(envelope.c));
 ...
 */
// IAP(アプリ内課金) 検証: 解錠後の署名リクエストで native ストアのレシートを
//   サーバへ送り、検証＋クレジット付与してもらう。Stripe の checkoutSessionUI と同じ
//   signedFetch(_session.signingState) 認証を使う。
export async function iapVerifyApple(signedTransaction) {
  const r = await signedFetch("/api/iap/apple", "POST", { signedTransaction }, _session.signingState);
  const j = await r.json().catch(() => ({}));
  if (!r.ok || !j.ok) throw new Error(j.error || `iap/apple ${r.status}`);
  return j;
}
export async function iapVerifyGoogle(productId, purchaseToken) {
  const r = await signedFetch("/api/iap/google", "POST", { productId, purchaseToken }, _session.signingState);
  const j = await r.json().catch(() => ({}));
  if (!r.ok || !j.ok) throw new Error(j.error || `iap/google ${r.status}`);
  return j;
}

/**
 * Postpaid (docs/postpaid-billing-design.md): Stripe Checkout セッション作成。
 * kind: "registration" (登録料 ¥300) | "settlement" (未払額の精算 — 金額はサーバ計算)
 * 旧 pack 指定は廃止 (サーバが 410 pack_discontinued を返す)。
 */
export async function checkoutSessionUI(kind, opts = {}) {
  if (!_session) throw new Error("locked — unlock first");
  const body = { kind };
  if (opts.locale) body.locale = opts.locale;
  if (opts.currency) body.currency = opts.currency;   // ja=jpy / 他=usd (表示と請求通貨を一致させる)
  const r = await signedFetch("/api/checkout", "POST", body, _session.signingState);
  const j = await r.json().catch(() => ({}));
  if (!r.ok || !j.ok) {
    const err = new Error(`checkout failed: ${j.error || r.status}`);
    err.code = j.code || null;
    err.status = r.status;
    throw err;
  }
  return j;  // { ok: true, url, sessionId, kind, chargeJpy, settleUsdMicro, publicKeyHash }
}
...
export async function currentSigningPrivateKeyRaw() {
  if (!_session) return null;
  // Phase 2-H1: Rust SigningKey opaque handle path — handle が session に居れば直接取り出し
  //   (= handle 内部 raw bytes を 32-byte big-endian で返す。 ECIES 復号 caller のみが使う)。
  //   修正: 署名鍵は _session.signingState.signingPrivateKey 配下にある (トップレベルではない)。
  //   旧コードは _session.signingPrivateKey (= 常に undefined) を読んでおり、 admin の handle
  //   経路 (mekHkdfKey=null) で監査ログ復号が常に失敗していた。
  const sp = _session.signingState?.signingPrivateKey ?? _session.signingPrivateKey;
  if (sp && typeof sp.private_key_raw === "function") {
    try {
      const raw = sp.private_key_raw();
      if (raw && raw.length === 32) return new Uint8Array(raw);
    } catch (e) {
      console.warn("[currentSigningPrivateKeyRaw] handle.private_key_raw failed:", e?.message || e);
    }
  }
  // Legacy path: signingPrivateKey が CryptoKey (extractable: false) のため raw 取り出し不可。
  ...
  const hkdfBase = _session.k2Handle || _session.mekHkdfKey;
  if (!hkdfBase) {
    console.warn("[currentSigningPrivateKeyRaw] session に HKDF base key が無い");
    return null;
  }
  const { deriveSigningKeyFromHkdf } = await import("/lib/vault-crypto.js?v=6bb1e228");
...
export async function decryptBusinessVaultWithK1(envelope, factors, k1Bytes) {
  ...
  // recovery fix: 災害復旧ツールは raw K1 をファイルから渡す。 forceRawMek で K2 を raw 抽出するが、
  //   増分1 (K2 handle 一本化) 以降は raw K2 も即 MekKey handle に収束され、 mek 派生は
  //   K1Key.derive_business_mek_v2 (= 旧 deriveBits HKDF と bit-equivalent) で行われる。
  const result = await decryptVaultBusiness(envelope, { ...factors, forceRawMek: true }, k1Bytes);
...
export async function rotateOrCreateAdminK1() {
  ...
  // #3: rotation は active 社員全員への再配布 + server version 同期まで一括で行う。
  //   1) active 社員を取得 → 各社員 emp_pubkey 宛てに newK1 を ECIES wrap
  //   2) /api/corp/rotate-k1 に distributions を POST → server が version++ +
  //      旧版 deprecated(14日TTL) + 各社員 enc_K1 を current に差し替え
  //   3) server の newVersion を権威として vault.k1Version に採用
  //   active 社員が 0 名 (= admin のみ) の場合は配布対象なし → ローカルのみ rotate
  //   (server rotate-k1 は空 distributions を拒否するため呼ばない)。
  ...
  try {
    let members = [];
    try {
      members = await listActiveEmployees();
    } catch (e) {
      console.warn("[admin-k1-rotate] active 社員取得失敗 (ローカルのみ rotate):", e?.message || e);
    }
    activeMemberCount = members.length;
    const distributions = [];
    for (const m of members) {
      if (!m?.pkHash || !m?.emp_pubkey) { skippedNoPubkey++; continue; }
      try {
        const encK1Blob = await eciesWrapForRecipient(m.emp_pubkey, newK1);
        distributions.push({ pkHash: m.pkHash, encK1Blob });
      } catch (e) {
        console.error("[admin-k1-rotate] ECIES wrap 失敗 for", m.pkHash, e?.message || e);
        skippedNoPubkey++;
      }
    }
    if (distributions.length > 0) {
      // server-first: 配布 + version bump を commit してから local vault を更新する
      //   (= 配布が成功した K1 を admin の vault.k1Current に確定させる)。
      const r = await signedFetch("/api/corp/rotate-k1", "POST", { distributions }, _session.signingState);
      const body = await r.json().catch(() => ({}));
      if (!r.ok || !body.ok) throw new Error(body?.error || "rotate-k1 failed");
      newVersion = body.newVersion ?? newVersion;     // server を権威に
      distributedCount = body.distributedCount ?? distributions.length;
    }
    // else: 配布対象なし → ローカルのみ rotate (newVersion = oldVersion + 1)
    ...
    // Phase 7.3-A.7e: _session.k1 は raw 保持を廃止。 k1Version のみ session に保持。
    _session.k1Version = newVersion;

    await saveVault(_session.vault);
  } finally {
    newK1.fill(0);  // raw を即破棄 (vault.k1Current に b64u で永続化済)
  }
  return {
    ok: true,
    k1Version: newVersion,
    wasRotation: !!oldCurrent,
...
export async function tryTransitionFromPending() {
  ...
  // handle 一本化: k2Handle + empPrivHandle が必須 (CryptoKey/raw 経路は廃止、 fallback 無し)。
  if (!_session.k2Handle || !_session.empPrivHandle) {
  ...
  // handle 一本化: empPrivHandle で ECIES → K1Key handle を直接生成 (生 K1 不在、 fallback 無し)。
  const _newK1Handle = await eciesUnwrapToK1Handle(_session.empPrivHandle, encK1);
  if (_session.k1Handle && typeof _session.k1Handle.free === "function") {
    try { _session.k1Handle.free(); } catch (_) {}
  }
  _session.k1Handle = _newK1Handle;
  // mek 派生: K1Key.derive_business_mek_v2(k2Handle) を WASM 内で実行 (生 K1/K2 不在)。
  const _enc = new TextEncoder();
  const _info = _enc.encode("mek-business-v2");
  const realMekKey = _session.k1Handle.derive_business_mek_v2(_session.k2Handle, _info);
  const realMekHkdfKey = _session.k1Handle.derive_business_mek_v2(_session.k2Handle, _info);  // 同 bytes、 別 handle
  if (_session.mek) _session.mek.fill(0);
  _session.mek = null;
  _session.mekKey = realMekKey;
  _session.mekHkdfKey = realMekHkdfKey;
  _session.k1 = null;
...
/**
 * #3: K1 rotation 後の社員 vault 自己治癒 transition。
 *   現在 session の body は (version-aware unlock により) 旧 K1 mek で開かれている場合がある。
 *   server 現行 version が body version より新しければ、 現行 K1 で mek を再導出し body を
 *   再暗号化して保存する (= envelope.k1Version も現行に更新)。 records (attachments) の
 *   再暗号化は別途 migrateAllRecordsToCurrentK1 (移行バナー) が担当する。
 *   handle 一本化前提 (k2Handle + empPrivHandle 必須、 fallback 無し)。
 */
export async function tryTransitionK1Version() {
  if (!_session) throw new Error("locked — unlock first");
  if (_session.lastEnvelope?.m !== "business") return { transitioned: false, reason: "not_business" };
  if (_session.lastEnvelope?.k1Pending) return { transitioned: false, reason: "pending" };  // pending は別経路
  if (!_session.k2Handle || !_session.empPrivHandle) return { transitioned: false, reason: "missing_keys" };
  let encK1;
  try {
    encK1 = await _fetchEncK1V2(_session.signingState);  // current
  } catch (e) {
    if (e.code === "pending_k1_distribution") return { transitioned: false, reason: "k1_pending" };
    throw e;
  }
  const currentVer = encK1.k1Version ?? 1;
  const bodyVer = _session.k1Version ?? 1;
  if (currentVer <= bodyVer) return { transitioned: false, reason: "already_current", k1Version: bodyVer };
  // 現行 K1 で mek を再導出 → body を現行 mek で再暗号化保存。
  const newK1Handle = await eciesUnwrapToK1Handle(_session.empPrivHandle, encK1);
  if (_session.k1Handle && typeof _session.k1Handle.free === "function") {
    try { _session.k1Handle.free(); } catch (_) {}
  }
  _session.k1Handle = newK1Handle;
  const _info = new TextEncoder().encode("mek-business-v2");
  const newMekKey = newK1Handle.derive_business_mek_v2(_session.k2Handle, _info);
  const newMekHkdfKey = newK1Handle.derive_business_mek_v2(_session.k2Handle, _info);  // 同 bytes、 別 handle
  if (_session.mekKey && typeof _session.mekKey.free === "function") { try { _session.mekKey.free(); } catch (_) {} }
  if (_session.mekHkdfKey && typeof _session.mekHkdfKey.free === "function") { try { _session.mekHkdfKey.free(); } catch (_) {} }
  _session.mekKey = newMekKey;
  _session.mekHkdfKey = newMekHkdfKey;
  _session.k1Version = currentVer;
  // saveVault が body を _session.mekKey (= 現行 K1 mek) で再暗号化し、 envelope.k1Version=currentVer を刻む。
  await saveVault(_session.vault);
  return { transitioned: true, fromVersion: bodyVer, k1Version: currentVer };
}
...
export async function migrateAllRecordsToCurrentK1() {
  ...
  for (const rec of _session.vault.records || []) {
    for (const att of rec.attachments || []) {
      ...
      try {
        ...
        // handle 化: BekKey handle で unwrap → re-wrap (CryptoKey 不使用)。
        const bekH = await unwrapBekWithMekHandle(oldRealMek, oldWrapped, oldIv);
        const { wrapped: newWrapped, iv: newIv } = await wrapBekWithMekHandle(newRealMek, bekH);
        if (bekH && typeof bekH.free === "function") { try { bekH.free(); } catch (_) {} }
  ...
  for (const chunk of _session.vault.recordChunks || []) {
    ...
    try {
      ...
      const oldIv = b64uDecode(enc.wrapIv);
      const cekH = await unwrapBekWithMekHandle(oldRealMek, oldWrapped, oldIv);
      const { wrapped: newWrapped, iv: newIv } = await wrapBekWithMekHandle(newRealMek, cekH);
      if (cekH && typeof cekH.free === "function") { try { cekH.free(); } catch (_) {} }
