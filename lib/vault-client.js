...
export async function createPasskey(userId, displayName, opts = {}) {
  ...
  if (!prfOutput) {
    console.log("[createPasskey] PRF が登録時に返らず — 直後の get() で取得 (セキュリティキー経路)");
...
export async function createVault(password, userDisplayName, captchaToken = null, opts = {}) {
  ...
  const userId = encodeUserIdV7(appNameTag, deriveOuterKeyBytes(recoveryMaterial));
...
export async function unlockWithPasswordAndPasskey(password, options = {}) {
  const meta = readMeta();
  const hasMeta = !!(meta?.outerKey && meta?.appNameTag);
  ...
  // outer 鍵 / appNameTag の取得元:
  //   - 既知端末 (localStorage meta あり): meta から。
  //   - 新端末 (meta 無し) + envelope v7 user.id: WebAuthn userHandle から。
  let outerKeyBytes, appNameTag, freshDevice = false;
  if (hasMeta) {
    outerKeyBytes = b64uDecode(meta.outerKey);
    appNameTag = meta.appNameTag;
  } else if (userHandle && isUserIdV7(userHandle)) {
    const _v7 = decodeUserIdV7(userHandle);
    outerKeyBytes = _v7.outerKey;
    appNameTag = _v7.appNameTag;
    freshDevice = true;
    console.log("[unlock-AB] envelope v7: 新端末 — outer 鍵 / appNameTag を user.id から取得");
  } else {
...
export async function currentSigningPrivateKeyRaw() {
  ...
  const { deriveSigningKeyFromHkdf } = await import("./vault-crypto.js");
