...
export async function hwkeyAuthenticateForUnlock() {
  ...
  if (hint) {
    try {
      ...
      console.warn("[hwkeyAuthForUnlock] specific get 失敗 — picker に fallback:",
        e?.name, e?.message);
    }
  }
  console.log("[hwkeyAuthForUnlock] picker get");
...
async function _retryArweaveFetch(fn, label) {
  const delays = [0, 3000, 5000, 8000, 12000, 15000, 18000];  // 合計 ~61s
  let lastErr;
...
export async function unlockWithHwkeyAuthed({ credentialId, prfOutput, userHandle }) {
  if (!prfOutput)
  ...
  // keyslot blob を GraphQL で発見 → 取得 → この YubiKey の PRF で復号
  const ksTxid = await findLatestVaultTx(keyslotTag);
  console.log("[hwkeyUnlock] findLatestVaultTx(keyslot) -> " + ksTxid);
  if (!ksTxid) {
...
export async function addHwkeyDevice(userDisplayName, opts = {}) {
  ...
  const exKsTxid = await findLatestVaultTx(exKeyslotTag);
...
export async function unlockWithPasswordAndPasskey(password, options = {}) {
  ...
  let envelope;
  try {
    ({ envelope } = await fetchEnvelope(txid, outerKeyBytes));
  } catch (e) {
...
export async function getBalanceUI() {
  if (!_session) throw new Error("locked");
  // TTL 内の近接呼び出しは進行中/直近の promise を使い回す (= /api/balance を間引く)。
  //   成功・失敗とも TTL キャッシュする (失敗キャッシュは 429 storm の抑制にもなる)。
  const now = Date.now();
  if (_balanceCache && now - _balanceCache.ts < _BALANCE_CACHE_MS) {
