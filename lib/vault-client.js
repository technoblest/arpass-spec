...
import {
  ...
  unpadPlaintext,
  VAULT_FORMAT_V5,
} from "/lib/vault-crypto.js?v=b28eca23";
...
async function buildSigningState(mek) {
  ...
  // Phase 2-H1: Rust SigningKey handle 優先 — signRequest が duck-type で使用
...
async function openSession({ vault, mek, mekKey, mekHkdfKey, k2AesKey, k2HkdfKey, signingKey: signingKeyFromResult,
  ...
  } else if (signingKeyFromResult?.privateKeyJwk && signingKeyFromResult?.publicKeyJwk) {
...
export async function refreshTierQualifier() {
  ...
  if (_session.rMatHkdfKey) {
    try {
      const { deriveAppNameTagFromHkdf } = await import("/lib/vault-crypto.js?v=b28eca23");
...
export async function createVault(password, userDisplayName, captchaToken = null, opts = {}) {
  ...
  const { generateRecoverySecret } = await import("/lib/vault-crypto.js?v=b28eca23");
  ...
  if (mode === "business" && enc.signingKey) {
    ...
    const { importSigningKeyPair: _isk } = await import("/lib/vault-crypto.js?v=b28eca23");
...
export async function currentSigningPrivateKeyRaw() {
  ...
  const sp = _session.signingPrivateKey;
  ...
  const { deriveSigningKeyFromHkdf } = await import("/lib/vault-crypto.js?v=b28eca23");
