 ...
 * @param {string|null} [tier=null] - "free" | "paid" | "private" | "corp::<companyId>" | null (legacy)
...
export function deriveAllAppNameTags(recoveryMaterial, currentCompanyId = null) {
  return {
    legacy:  deriveAppNameTag(recoveryMaterial, null),
 ...
 */
export async function changePassword(envelope, secretToWrap, currentCredIdHash, newPassword, currentPrfOutput, recoveryMaterial) {
...
/**
 * Admin recovery 用 K1 wrap (= wsa)。Master + Recovery 由来の KEK で AES-GCM。
 * Arpass 廃業時など server 不在の状況で admin が K1 を取り戻すための保険経路。
 *
 * @param {Uint8Array} k1Bytes 32 byte
 * @param {Uint8Array} pMat   32 byte (Master Password から PBKDF2 で派生)
 * @param {Uint8Array} rMat   32 byte (Recovery Secret から HKDF で派生)
 * @returns {Promise<{iv: Uint8Array, ciphertext: Uint8Array}>}
 */
export async function adminWrapK1(k1Bytes, pMat, rMat) {
  const wrapKeyRaw = hkdfBytes(
    concatBytes(pMat, rMat),
    HKDF_SALT_KEK_K1ADMIN,
    HKDF_INFO_KEK_K1ADMIN,
    32
  );
  const wrapKey = await crypto.subtle.importKey(
    "raw", wrapKeyRaw, { name: "AES-GCM" },
    false, ["encrypt"]
  );
  wrapKeyRaw.fill(0);
  const iv = crypto.getRandomValues(new Uint8Array(AES_IV_LEN));
...
/**
 * Admin recovery 用 K1 unwrap (= wsa を開く)。
 * Master + Recovery を入力に取る。
 *
 * @param {{iv: Uint8Array, ciphertext: Uint8Array}} wsa
 * @param {Uint8Array} pMat
 * @param {Uint8Array} rMat
 * @returns {Promise<Uint8Array>} 32 byte K1
 */
export async function adminUnwrapK1(wsa, pMat, rMat) {
  const wrapKeyRaw = hkdfBytes(
    concatBytes(pMat, rMat),
    HKDF_SALT_KEK_K1ADMIN,
    HKDF_INFO_KEK_K1ADMIN,
  ...
  wrapKeyRaw.fill(0);
  const k1 = new Uint8Array(
    await crypto.subtle.decrypt({ name: "AES-GCM", iv: wsa.iv }, wrapKey, wsa.ciphertext)
  );
  return k1;
}
...
/**
 * Phase 7.2-B 用 envelope の ws フィールドを base64u 文字列化 (Arweave 保存用)。
 *
 * @param {{ephPubRaw: Uint8Array, iv: Uint8Array, ciphertext: Uint8Array}} wsRaw
 * @param {number} kekVersion
 * @returns {object} envelope.ws shape
 */
export function encodeWsField(wsRaw, kekVersion) {
  return {
    e: b64uEncode(wsRaw.ephPubRaw),
    i: b64uEncode(wsRaw.iv),
    c: b64uEncode(wsRaw.ciphertext),
    v: kekVersion,
  };
}

/**
 * envelope.ws を decode して raw bytes に戻す (server に送る前 or server 側受信時)。
 */
export function decodeWsField(wsField) {
  return {
    ephPubRaw: b64uDecode(wsField.e),
    iv: b64uDecode(wsField.i),
    ciphertext: b64uDecode(wsField.c),
    version: wsField.v,
  };
}

/**
 * envelope の wsa field encode/decode。
 */
export function encodeWsaField(wsaRaw) {
  return {
    i: b64uEncode(wsaRaw.iv),
    c: b64uEncode(wsaRaw.ciphertext),
  };
}
export function decodeWsaField(wsaField) {
  return {
    iv: b64uDecode(wsaField.i),
    ciphertext: b64uDecode(wsaField.c),
  };
}
 ...
 * 緊急復旧 (admin): k1 は adminUnwrapK1(envelope.wsa, pMat, rMat) で取得
 *
 * caller が k1 を準備して渡す (= server fetch も admin unwrap も vault-client.js 側)。
 *
...
export async function decryptVaultBusiness(envelope, factors, k1Bytes) {
  ...
  // Phase 7.3-A.9 part 3: V1 (legacy) と V2 (kdfV2) の両 path をサポート
  const isKdfV2 = envelope.kdfV2 === true;
  let k2AesKey = null;
  let k2HkdfKey = null;
  let empPrivKey;
  let mekKey;
  ...
  if (isKdfV2) {
    // V2 path: K2 を CryptoKey 化、 mekKey は K2 HKDF base + K1 salt から派生
    ({ k2AesKey, k2HkdfKey } = await importK2AsKeys(k2));
    empPrivKey = await unwrapEmpPrivWithK2Key(k2AesKey, envelope.w_emp);
    mekKey = await deriveBusinessMekKeyV2(k1Bytes, k2HkdfKey, {
      usages: ["encrypt", "decrypt", "wrapKey", "unwrapKey"],
    });
    mekHkdfKey = await deriveBusinessMekHkdfKeyV2(k1Bytes, k2HkdfKey);
  } else {
    // V1 legacy path: raw k2 ベースの HKDF chain
    empPrivKey = await unwrapEmpPrivWithK2(k2, envelope.w_emp);
    const realMek = deriveBusinessMek(k1Bytes, k2);
    mekKey = await crypto.subtle.importKey(
      "raw", realMek, { name: "AES-GCM" }, false, ["encrypt", "decrypt"]
    );
    mekHkdfKey = await deriveBusinessMekHkdfKey(k1Bytes, k2);
  }
  ...
  const signingKey = deriveSigningKey(k2);
  // Phase 7.3-A.9 part 3: V2 path では k2 raw は importK2AsKeys 内で消費済。 ここで fill(0)。
  //   V1 path は legacy caller が k2 raw を期待する可能性あり、 戻り値に含める。
  const k2CopyForLegacy = isKdfV2 ? null : new Uint8Array(k2);
  k2.fill(0);
  return {
    vault,
    mek: null,                   // Phase 7.3-A.9 part 3: raw mek 不在
    mekKey, mekHkdfKey,
    k2AesKey, k2HkdfKey,          // Phase 7.3-A.9 part 3: K2 CryptoKey 群 (V2 path のみ)
    k2: k2CopyForLegacy,          // V1 legacy 後方互換用
...
// ----------------------------------------------------------------------------
// 増分1: outer 鍵 (32 byte) を WebAuthn user.id に *生* で格納する。新端末でも
//   localStorage / Recovery 無しに outer 鍵へ到達できる。
// 設計判断 (案A): WebAuthn の順序制約により user.id をその credential 自身の
//   PRF で暗号化することは不可能。user.id は Passkey ハードウェアでゲートされ
//   Arweave 公開オブジェクトには載らないため anti-fingerprint は無傷、生で許容。
//   標準モードの outer 鍵は既存の deriveOuterKeyBytes(rMat) をそのまま使うため、
//   新たな鍵導出も envelope 形式変更も不要。詳細は envelope-v7-spec.md §3-4。
// 本セクションは純関数のみ。createVault / unlock への結線は後続ステップ。
// ============================================================================
...
/**
 * envelope v7 の WebAuthn user.id (userHandle) ペイロードを構築 (案A)。
 * レイアウト (57 byte): [1B version=7][8B name][16B value][32B outerKey(生)]
 ...
 * @returns {Uint8Array} 57 byte (<=64)
 */
export function encodeUserIdV7(appNameTag, outerKeyBytes) {
  ...
  const out = concatBytes(new Uint8Array([USERID_V7_VERSION]), nameB, valB, outerKeyBytes);
  if (out.length > 64) throw new Error(`encodeUserIdV7: user.id ${out.length} byte exceeds WebAuthn 64-byte limit`);
  return out;
}

/**
 * encodeUserIdV7 の逆。v7 形式でない (旧 string user.id 等) 場合は例外。
 * @param {Uint8Array} userIdBytes
 * @returns {{version:number, appNameTag:{name:string,value:string}, outerKey:Uint8Array}}
 */
export function decodeUserIdV7(userIdBytes) {
  ...
  const outerKey = userIdBytes.slice(off);
  return {
