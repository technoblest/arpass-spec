...
// 設計判断 (案A + ハードニング): WebAuthn の順序制約により user.id をその
//   credential 自身の PRF で暗号化することは不可能 (user.id は credential 作成
//   *前* に確定、PRF は作成 *後*)。代わりに outer 鍵を Master パスワード由来鍵で
//   AES-256-CTR ラップして user.id に格納する (v7 ハードニング 2026-05-24)。
//   Master は credential 作成順序と独立なので順序制約に抵触しない。これにより
//   user.id が将来想定外の場所 (パスキー export 規格・OS 変更・フォレンジック等)
//   へ漏れても、Master を知らない者には暗号文でしかない。AES-CTR は非膨張なので
//   user.id は 57 byte を維持。誤 Master の検出は独自タグを持たず下流の外側
//   AES-GCM 層に委譲する (誤 Master → 誤 outer 鍵 → envelope 復号がそこで失敗)。
//   appNameTag は秘密ではない (Arweave 上の匿名タグそのもの) ため平文で載せる。
//   詳細は envelope-v7-spec.md §3-4。
// ============================================================================
...
//   vault ごとに一意。user.id は vault 作成時に 1 度だけ書かれ以後不変なので
//   (鍵, カウンタ) の組が別平文で再利用されることはない)。
...
//   ラップ鍵 = PBKDF2-SHA256(Master, salt=appNameTag.value, 600k)。
async function _wrapOuterForUserId(outerKeyBytes, password, nameB, valB, mode) {
  const kekBytes = await derivePMat(password, valB);
...
/**
 * envelope v7 の WebAuthn user.id (userHandle) ペイロードを構築 (案A + ハードニング)。
