...
export async function encryptVault(vault, password, prfOutput, recoveryMaterial, credIdHash, recoverySecret = null) {
  ...
  // Stage 2c Stage G7 (2026-06-06): body encryption を MekKey handle 経由に。
  ...
  let _g7BodyKey = mekKey;
  let _g7TmpHandle = null;
  try {
    _g7TmpHandle = await importMekRawAsHandle(mek);
    if (_g7TmpHandle) _g7BodyKey = _g7TmpHandle;
  } catch (_) { /* fallback to mekKey CryptoKey */ }
  const bodyCt = await aesGcmEncrypt(_g7BodyKey, ivBody, padded);
  if (_g7TmpHandle && typeof _g7TmpHandle.free === "function") {
    try { _g7TmpHandle.free(); } catch (_) {}
  }
...
export async function decryptVault(envelope, factors) {
  ...
  // Stage 2c Stage G7 (2026-06-06): body decryption を MekKey handle 経由に。
  let _g7DBodyKey = mekKey;
  let _g7DTmpHandle = null;
  try {
    _g7DTmpHandle = await importMekRawAsHandle(mek);
    if (_g7DTmpHandle) _g7DBodyKey = _g7DTmpHandle;
  } catch (_) { /* fallback to mekKey CryptoKey */ }
  const padded = await aesGcmDecrypt(_g7DBodyKey, b64uDecode(envelope.i), b64uDecode(envelope.c));
  if (_g7DTmpHandle && typeof _g7DTmpHandle.free === "function") {
    try { _g7DTmpHandle.free(); } catch (_) {}
  }
