...
//
// 最小バケット 120 KiB は Turbo の無料枠 (107520 B = 105 KiB) を確実に超える
// よう設計。これにより以下を同時に達成する:
//   (a) フィンガープリント耐性: tx サイズで Arpass vault を一発抽出できない
//   (b) フリーライド回避: 全 write が Turbo の有料 tier に入り、AUP 違反リスクを排除
//   (c) サイズ秘匿: エントリ数の増減が外部に漏れない
//
// 過去 (v5 cutover 直後) の `[4 * 1024, 16 * 1024, ...]` は (a)(b) を同時に
// 破っていた重大バグ。改修履歴は v5.2 PR を参照。
const PAD_BUCKETS = [120 * 1024, 240 * 1024, 480 * 1024, 960 * 1024, 4 * 1024 * 1024];
