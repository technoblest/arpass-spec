 ...
 *
 * Phase 6.4: tier qualifier 対応
 * ----------------------------------
 *   外部観測者が Free pool wallet を全列挙して App-Name X を harvest した後、
 *   user が Paid に upgrade すると、同じ App-Name X が Paid pool wallet にも
 *   出現してしまう (= tier transition の bridge)。
 *
 *   対策: tier ごとに HKDF info を変える。
 *
 *   tier = null (legacy)            : info = "App-Name"             (現行 / 後方互換)
 *   tier = "free"                   : info = "App-Name::free"
 *   tier = "paid"                   : info = "App-Name::paid"
 *   tier = "private"                : info = "App-Name::private"
 *   tier = "corp::<companyId>"      : info = "App-Name::corp::<companyId>"
 *
 *   同じ Recovery でも tier が違えば全く異なる App-Name になる。
 *   Vault discovery 時は全 tier の App-Name を並列検索 (deriveAllAppNameTags 参照)。
 *
 * @param {Uint8Array} recoveryMaterial
 * @param {string|null} [tier=null] - "free" | "paid" | "private" | "corp::<companyId>" | null (legacy)
 * @returns {string} base64url 16 文字
 */
export function deriveAppNameTag(recoveryMaterial, tier = null) {
  const info = tier ? `${HKDF_INFOS.app_name_tag}::${tier}` : HKDF_INFOS.app_name_tag;
  const bytes = hkdfBytes(recoveryMaterial, HKDF_SALTS.app_name_tag, info, 12);
  return b64uEncode(bytes);
}

/**
 * 並列 vault 発見用に全 tier の App-Name を一括計算。
 ...
 * @returns {{ legacy: string, free: string, paid: string, private: string, corp: string|null }}
 */
