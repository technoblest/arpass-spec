...
async function _getRustCrypto() {
  ...
  _rustCryptoLoading = (async () => {
    try {
      const mod = await import("/lib/rust-crypto/arpass_crypto.js?v=3305b85d");
...
export async function eciesEncrypt(recipientPublicKeyRaw, plaintext) {
  ...
  // 3. HKDF -> 32-byte KEK (Stage 2c: dispatcher 経由で raw bytes path)
  const kekBytes = hkdfBytes(sharedX, ECIES_HKDF_SALT, ECIES_HKDF_INFO, 32);

  // 4. AES-GCM encrypt (Stage 2c: aesGcmEncrypt dispatcher、 kek が Uint8Array なので
  //    Rust path で動作。 import to CryptoKey は不要)
  const iv = crypto.getRandomValues(new Uint8Array(AES_IV_LEN));
  const ct = await aesGcmEncrypt(kekBytes, iv, plaintext);

  // 5. Zeroize ephemeral private key + kek bytes (= raw bytes だったので明示 zeroize)
...
export async function eciesDecrypt(myPrivateKeyRaw, payload) {
  ...
  // 2. HKDF -> 32-byte KEK (Stage 2c: dispatcher 経由で raw bytes path)
  const kekBytes = hkdfBytes(sharedX, ECIES_HKDF_SALT, ECIES_HKDF_INFO, 32);

  // 3. AES-GCM decrypt (Stage 2c: aesGcmDecrypt dispatcher、 kek が Uint8Array なので
  //    Rust path で動作。 import to CryptoKey は不要)
  let pt;
  try {
    pt = await aesGcmDecrypt(kekBytes, iv, ciphertext);
  } catch (e) {
    sharedX.fill(0);
    kekBytes.fill(0);
...
export async function deriveKEK(material1, material2, saltKey) {
  ...
  // Stage 2c Stage G9 (2026-06-06): Rust handle 優先、 fallback CryptoKey。
  //   全 callers (= encryptVault, decryptVault, addCredential, changePassword,
  //   changeRecovery_caseA/B, encryptVaultBusiness, decryptVaultBusiness 等) が
  //   aesGcmEncrypt/Decrypt dispatcher 経由なので handle/CryptoKey どちらでも動く。
  //   handle return 時は raw bytes WASM 内に隠蔽、 wasm-bindgen の FinalizationRegistry で
  //   GC 時 auto free (= 明示 .free() 不要、 但し可能なら caller 側 free 推奨)。
  try {
  ...
  } catch (_) { /* rust-crypto 未ロード or import 失敗 → CryptoKey fallback */ }
...
export async function deriveKEKHandle(material1, material2, saltKey) {
  ...
  try {
    const h = await importMekRawAsHandle(raw);
    return h;
  } finally {
    raw.fill(0);
  }
...
async function aesGcmEncrypt(key, iv, plaintext, aad) {
  ...
  // CryptoKey path (= WebCrypto)
  const opts = { name: "AES-GCM", iv };
  if (aad instanceof Uint8Array) opts.additionalData = aad;
  return new Uint8Array(await crypto.subtle.encrypt(opts, key, plaintext));
}
async function aesGcmDecrypt(key, iv, ciphertext, aad) {
  ...
  const opts = { name: "AES-GCM", iv };
  if (aad instanceof Uint8Array) opts.additionalData = aad;
  return new Uint8Array(await crypto.subtle.decrypt(opts, key, ciphertext));
}
...
export async function encryptVault(vault, password, prfOutput, recoveryMaterial, credIdHash, recoverySecret = null) {
  ...
  // Stage 2c Stage G7 (2026-06-06) + Phase 2-H2-b (= 同日): body encryption を MekKey handle 経由に。
  //   aesGcmEncrypt は duck-type detection で handle.aes_gcm_encrypt を呼ぶ → Rust AES-GCM path。
  //   H2-b 変更: 一時 handle を free せず 返却 mekKey として promotion。 caller (= openSession)
  //   が _session.mekKey に格納 → session 終了時にまとめて free。
  //   handle が取れない (= rust-crypto 未ロード) なら mekKey CryptoKey で fallback。
  let _h2bHandle = null;
  try {
    _h2bHandle = await importMekRawAsHandle(mek);
  } catch (_) { /* fallback to mekKey CryptoKey */ }
  const _bodyKey = _h2bHandle || mekKey;
  const bodyCt = await aesGcmEncrypt(_bodyKey, ivBody, padded);
  // mekKey 返却値: Rust loaded なら handle 優先、 fallback で CryptoKey
  const _returnedMekKey = _h2bHandle || mekKey;
  ...
  return { envelope, mek, mekKey: _returnedMekKey, mekHkdfKey, outerKeyBytes, appNameTag, signingKey };
}
...
export async function decryptVault(envelope, factors) {
  ...
  // Stage 2c Stage G7 (2026-06-06) + Phase 2-H2-b (= 同日): body decryption を MekKey handle 経由に。
  //   H2-b 変更: 一時 handle を free せず 返却 mekKey として promotion。
  let _h2bDHandle = null;
  try {
    _h2bDHandle = await importMekRawAsHandle(mek);
  } catch (_) { /* fallback to mekKey CryptoKey */ }
  const _bodyKey = _h2bDHandle || mekKey;
  const padded = await aesGcmDecrypt(_bodyKey, b64uDecode(envelope.i), b64uDecode(envelope.c));
  // mekKey 返却値: Rust loaded なら handle 優先、 fallback で CryptoKey
  const _returnedDMekKey = _h2bDHandle || mekKey;
  ...
  const mekHkdfKey = await derivePersonalMekHkdfKey(mek);
  return { vault, mek, mekKey: _returnedDMekKey, mekHkdfKey, signingKey, path };
}
...
export async function signRequest(privateKey, message) {
  ...
  // Legacy CryptoKey path (= subtle.sign).
...
export async function wrapKey(mek, blobKey) {
  const iv = crypto.getRandomValues(new Uint8Array(AES_IV_LEN));
  // Phase 2-H2-a (2026-06-06): MekKey opaque handle (Rust) も受付ける。 unwrapKey と対称。
  //   handle 検出 → blobKey raw を export → handle.aes_gcm_encrypt で wrap → bytes 廃棄。
  if (mek && typeof mek.aes_gcm_encrypt === "function") {
    let bekBytes;
    if (blobKey instanceof CryptoKey) {
      bekBytes = new Uint8Array(await crypto.subtle.exportKey("raw", blobKey));
    } else if (blobKey instanceof Uint8Array) {
      bekBytes = blobKey;
    } else {
      throw new Error("wrapKey: blobKey must be CryptoKey or Uint8Array");
    }
    const wrapped = new Uint8Array(mek.aes_gcm_encrypt(iv, bekBytes, new Uint8Array(0)));
    if (blobKey instanceof CryptoKey) bekBytes.fill(0);
    return { wrapped, iv };
  }
  let mekKey;
...
export async function unwrapKey(mek, wrapped, iv, opts = {}) {
  ...
  // Phase 2-H2-a (2026-06-06): MekKey opaque handle (Rust) も受付ける。
  //   将来 _session.mekKey が handle 化されても record file path が壊れないよう
  //   handle 検出 → handle.aes_gcm_decrypt → 結果 raw を CryptoKey に import。
  //   既存 CryptoKey / Uint8Array path はそのまま保持。
  if (mek && typeof mek.aes_gcm_decrypt === "function") {
    const bekBytes = new Uint8Array(mek.aes_gcm_decrypt(iv, wrapped, new Uint8Array(0)));
...
export async function encryptBlob(blobKey, plaintext) {
  // Stage 2c: dispatcher 経由で raw bytes / CryptoKey 両対応。
  //   blobKey Uint8Array → Rust path、 CryptoKey → WebCrypto path。
  const iv = crypto.getRandomValues(new Uint8Array(AES_IV_LEN));
  const ciphertext = await aesGcmEncrypt(blobKey, iv, plaintext);
...
export async function decryptBlob(blobKey, ciphertext, iv) {
  // Stage 2c: dispatcher 経由で raw bytes / CryptoKey 両対応。
  return await aesGcmDecrypt(blobKey, iv, ciphertext);
}
