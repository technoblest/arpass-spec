...
export async function signedFetch(url, method, bodyObject, signingState, fetchOpts = {}) {
  ...
  // sec-2026-07 報告2+5 (v6 署名): 署名対象に METHOD + PATH を含めて署名をエンドポイントに
  //   拘束し(旧 `${ts}.${rawBody}` は endpoint 非拘束だった=報告2)、改行区切りの明示フィールドに
  //   して timestamp 等の正規化揺れを排除する(報告5)。ユーザー不在の now-or-never 切替なので
  //   サーバは v6 のみ受理(後方互換なし)。path は query を除いた pathname のみ署名する。
  const _sigPath = new URL(url, "https://arpass.io").pathname;
  const message = ["v6", method.toUpperCase(), _sigPath, ts, rawBody].join("\n");
