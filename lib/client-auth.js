...
import { activeMetaKey, refreshActiveLabel } from "/lib/profiles.js?v=69cb395d";
...
export function writeMeta(meta) {
  ...
  // Phase 7.1-AE: drive 識別子が判明したら active profile のラベルを自動更新。
  try { refreshActiveLabel(); } catch { /* label 更新失敗は致命的でない */ }
}
 ...
 */
export async function signedFetch(url, method, bodyObject, signingState, fetchOpts = {}) {
  ...
  // 2026-07-12: timeoutMs 指定時は AbortSignal を付ける (応答輸送路が死んでいる端末対策)。
  const init = { method, headers, body: rawBody || undefined };
  if (fetchOpts.timeoutMs) init.signal = AbortSignal.timeout(fetchOpts.timeoutMs);
  return fetch(url, init);
}

// 2026-07-12: /api/write 専用の signedFetch。server 側の最大処理は ~31s (15s×2+α) なので、
//   45s で応答が無ければ輸送路の問題 (省電力のプロセス凍結・TLS ストール等)。ブラウザ既定の
//   数分待ちを避けて打ち切り、storage outage (upload_failed) と同じ扱いに変換する
//   (= saveVault の縮小リトライ・16言語の障害メッセージがそのまま効く)。
async function _signedWriteFetch(body, signingState) {
  // 2026-07-12: timeout はリクエスト転送 + server 処理 (envelope ~20.5s / large ~31s) を含む。
  //   envelope (b64u ~160K chars) は 35s、大きいファイルは転送時間も要るので 75s。
  const _payloadChars = (body && typeof body.data === "string") ? body.data.length : 0;
  const _timeoutMs = _payloadChars > 400_000 ? 75_000 : 35_000;
  // 2026-07-12: 書き込み中は Screen Wake Lock を保持する。アグレッシブな省電力
  //   (Motorola moto_freezer 等) は画面アイドル数秒で Chrome プロセスごと凍結し、
  //   応答を取り損ねて数分待ちになる (実機確認済み: タップで解凍すると即応答)。
  //   非対応ブラウザ / 拒否時は no-op。完了・失敗どちらでも必ず解放する。
  let _wl = null;
  try { _wl = navigator.wakeLock?.request ? await navigator.wakeLock.request("screen") : null; } catch (_) { _wl = null; }
  try {
    return await signedFetch("/api/write", "POST", body, signingState, { timeoutMs: _timeoutMs });
  } catch (e) {
    if (e?.name === "TimeoutError" || e?.name === "AbortError") {
      const err = new Error(`write timed out after ${_timeoutMs / 1000}s (no response from server)`);
...
export async function getBalance(signingState) {
  // 2026-07-12: timeout なし。refreshHeader は非ブロッキング化済みなので、遅くても
  //   届いた時点で描画される方が良い (10s で打ち切ると利用額行が空のままになる)。
  //   遅さの根本 (ar-price 上流ハング) は server 側 4s timeout で解消済み。
...
export async function fetchEnvelope(txid, outerKeyBytes, options = {}) {
  ...
  if (cached) {
    // 2026-07-12: 旧実装は background probe で Turbo 配信開始を検知したら cache を
    //   破棄していたが、Arweave tx は immutable なので「txid 一致 = 内容同一」が保証
    //   される。破棄する理由がなく、保持すれば自端末書き込み後の unlock は常に
    //   network ゼロで返せる。他端末が書けば server の txid が変わり cache miss で
    //   自然に network へ行く (getEnvelopeCache は txid 照合済み)。probe は廃止。
...
async function readBlobWithFallback(txid, options = {}) {
  ...
  // sec-2026-07 報告8 (privacy): 以前は proxy と arweave.net を毎回「並列」で叩いていたため、
  //   全 envelope 読み取りで txid + user IP + timing が第三者 GW (arweave.net) に露出していた。
  //   → same-origin proxy を「優先・逐次」にし、arweave.net 直叩きは proxy が blob を返せ
  //     なかった時のみの保険にする。proxy は server-side で Turbo + arweave.net を並列 fetch
  //     するので、通常はここで取れて第三者への露出はゼロ。信頼性は現状同等 (proxy が取れない
  //     時は必ず arweave も試す)。
  // proxy は same-origin なので通常は即応。ハング時に arweave フォールバックまで
  //   FETCH_TIMEOUT_MS(30s) 待つと全 unlock 読み取りが遅延するため、proxy 試行は短め。
  const PROXY_TIMEOUT_MS = 10000;
  const fetchOnce = async (url, timeoutMs = FETCH_TIMEOUT_MS) => {
    ...
    try {
      ...
      if (r.ok) {
        return {
          source: url.startsWith("/api/") ? "proxy" : "arweave-fallback",
  ...
  // 1) same-origin proxy を優先 (短めタイムアウト)
  let winner = await fetchOnce(`/api/arweave/${txid}`, PROXY_TIMEOUT_MS);
  // 2) proxy が取れなかった時のみ arweave.net 直叩き (privacy より unlock 可用性を優先する degraded path)
  if (!winner) winner = await fetchOnce(`${ARWEAVE_GATEWAY}/${txid}`);
...
export async function fetchServerVaultLatest(appNameTagValue) {
  ...
  try {
    // 2026-07-12: timeout 5s。ストールする回線で unlock 全体が待たされるのを防ぐ。
    //   失敗時は caller (resolveLatestTxIdForUnlock) が meta.latestTxId / GraphQL に fallback。
...
export async function fetchServerVaultSlots(pkHash) {
  ...
  try {
    // 2026-07-12: timeout 5s (fetchServerVaultLatest と同理由)。
...
/**
 * findLatestVaultTx の履歴版: 同 appNameTag の txid を新しい順に最大 limit 件返す。
 * 最新版が壊れている時に過去版へさかのぼる history フォールバック用。
 */
export async function findVaultTxHistory(appNameTag, limit = 10) {
  if (!appNameTag?.name || !appNameTag?.value) return [];
  const safeName = String(appNameTag.name).replace(/[^A-Za-z0-9_-]/g, "");
  const safeVal = String(appNameTag.value).replace(/[^A-Za-z0-9_-]/g, "");
  if (!safeName || !safeVal) return [];
  const n = Math.max(1, Math.min(50, limit | 0));
  const query = `
    query {
      transactions(
        tags: [{ name: "${safeName}", values: ["${safeVal}"] }]
  ...
  try {
    ...
    if (!resp.ok) return [];
    const data = await resp.json();
    return (data?.data?.transactions?.edges ?? []).map((e) => e?.node?.id).filter(Boolean);
  } catch (e) {
    clearTimeout(timer);
    return [];
  }
...
export async function writeEnvelope(envelope, outerKeyBytes, appNameTag, signingState, options = {}) {
  ...
  const r = await _signedWriteFetch(body, signingState);
  const j = await r.json().catch(() => ({}));
  if (!r.ok || !j.ok) {
  ...
  // Phase 5.3 → 2026-07-12 改訂: 書き込み成功直後に local cache に保存。
  //   旧設計は Turbo bundling 完了までのブリッジ目的で配信開始後に破棄していたが、
  //   現在は保持し続け、unlock 時に server txid と一致すれば network ゼロで復号する
  //   (単一 slot なので次回書き込みで自然に上書きされる)。
  try {
