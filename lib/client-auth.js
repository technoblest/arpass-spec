...
import { activeMetaKey } from "/lib/profiles.js";
...
/**
 * v5 メタ (Phase 7.0w-AR で vault-id を outerKey に置換、tag を {name,value} に):
 *   {
 *     outerKey:    "<base64url 32-byte AES-GCM key>",
 *     appNameTag:  { name: "<11 chars>", value: "<22 chars>" },  // 両方 rMat 派生
 ...
 *   }
 * outerKey 含めて localStorage 内であり、HTTPS 越しサーバや Arweave に送らない。
 */
...
export async function findLatestVaultTxAcrossTiers(appNameTags) {
  ...
  async function queryGateway(gateway, tag) {
    const ctrl = new AbortController();
    const timer = setTimeout(() => ctrl.abort(), GRAPHQL_TIMEOUT_MS);
    try {
      const resp = await fetch(`${gateway}/graphql`, {
  ...
  for (const tag of tags) {
    jobs.push(queryGateway(TURBO_GATEWAY, tag));
    jobs.push(queryGateway(ARWEAVE_GATEWAY, tag));
  }
...
export async function fetchRecordFileBytes(txid) {
  ...
  for (const gw of gateways) {
    try {
      const r = await fetch(`${gw}/${encodeURIComponent(txid)}`, { method: "GET" });
      if (r.ok) {
        const buf = await r.arrayBuffer();
        return new Uint8Array(buf);
      }
      lastErr = new Error(`${gw} returned HTTP ${r.status}`);
    } catch (e) {
      lastErr = e;
    }
  }
  throw lastErr ?? new Error("record file fetch failed: all gateways unreachable");
}
...
async function _probeDataReachable(txid) {
  ...
  const cb = `?t=${Date.now()}`;
  // Phase 7.2-B v2.6 hotfix: same-origin proxy も probe 候補に加える (= CORS 回避)。
  const candidates = [
