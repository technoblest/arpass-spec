...
export async function fetchRecordFileBytes(txid) {
  // Phase 7.5h: /api/arweave proxy 経由で取得 (= server-side で Turbo + arweave 並列)。
  //   turbo-gateway.com 直叩きは CORS で失敗。 arweave.net 直叩きは Turbo 期間中の
  //   tx を取れない。 proxy が両方の良いとこ取り。
  const candidates = [
    `/api/arweave/${encodeURIComponent(txid)}`,
    `${ARWEAVE_GATEWAY}/${encodeURIComponent(txid)}`,  // 保険: proxy 落ち時
  ];
  let lastErr;
